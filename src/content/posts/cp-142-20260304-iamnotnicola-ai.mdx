...
但現在呢？你的角色比較像機場塔台。你不是自己在開飛機，而是同時指揮好幾架飛機起降。你給高階方向、設定目標，然後讓一群 [Agent](/glossary#agent)（甚至 Sub-agent）自己去搞定細節。
...
這裡有個特別有意思的細節。原作者的實戰配置是這樣的：他讓 [Claude Code](/glossary#claude-code) 或 [Codex](/glossary#codex) 持續掃描他的 GitHub repo，讓 AI 自己發 PR、寫 Issue。更酷的是——AI 還會去讀**他自己**寫的留言跟 PR review 意見，然後根據這些反饋自動修正。
