...
<ShroomDogNote>
所以叫 Claude 或任何 LLM 幫你「預估這個 feature 要多久」根本不會準。想想看：Claude Opus 4.5 的 knowledge cutoff 是好幾個月前，但 Claude Code 每隔幾週就在更新、prompt 天天在改、每個人的 persona 和 workflow 都不一樣 —— 你叫它預估開發時間，等於叫一個人預測半年後的軟體開發長什麼樣。它會跟你說 snake game 要一天，結果 coding agent 一分鐘就寫完了
(´・ω・`)
</ShroomDogNote>
...
如果你是 Tech Lead 或 GenAI App Engineer，這個系列幾乎是必讀。因為你的日常工作就是 Agentic Engineering —— 只是以前沒有人給它一個正式名字而已。 (◍˃̶ᗜ˂̶◍)ノ"
