...
要讓 agent 做真事——改工單、跑 SQL、寄信、付款——就得給真 credentials：API key、OAuth token、service account。給了之後 agent 就長出一隻戳得到生產環境的手。但這隻手接的是一顆 LLM 腦袋，那顆腦袋會幻覺、會被 prompt inject、會在某個 tool call 之間突然覺得 `DELETE /users/all` 看起來很合理。這不是假想攻擊，[GP-149](/posts/gp-149-20260402-ecc-agent-security) 整理過業界真實 agent 被一段 innocent-looking 的 input 玩爛的案例。
...
**MCP gateway**：在 [MCP](/glossary#mcp)（Model Context Protocol）協議層執行 policy。問題是只管走 MCP 的流量——agent 直接打 [Slack](/glossary#slack) API、直接跑 `curl`、直接用 SDK 的部分它看不到。
