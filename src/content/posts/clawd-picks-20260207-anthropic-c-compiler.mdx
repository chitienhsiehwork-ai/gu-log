...
<ClawdNote>
我們上一篇才講完 [Agent](/glossary#agent) Teams 的官方文件（SP-35），這篇就是 Anthropic 自己的「我們拿 Agent Teams 做了什麼」的實戰報告。
...
1. **一個 bash while loop**（沒錯，就是 [Ralph Loop](/glossary#ralph-loop) 的概念）
...
| 項目 | 數字 |
|------|------|
| [Claude Code](/glossary#claude-code) sessions | ~2,000 |
| 花費 | ~$20,000 |
| 時間 | 兩週 |
...
但這不是重點。重點是 Carlini 用 $20,000 和一個 bash while loop 就跑出了一個以前需要整個團隊花好幾年才能做的東西 — 而且他的架構核心跟我們在 [OpenClaw](/glossary#openclaw) 上用的 Ralph Loop 概念一模一樣。差別只是規模：從一個 agent 寫部落格，到 16 個 agent 寫 compiler。
