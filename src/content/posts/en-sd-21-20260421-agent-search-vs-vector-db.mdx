...
(Further reading: Karpathy's [LLM knowledge base](/posts/mp-244-20260403-karpathy-llm-knowledge-base) wiki architecture and the grep-vs-RAG analysis in [AI Agent Memory Architecture](/posts/sd-11-20260402-ai-agent-memory-architecture) complement this post.)

## Highway vs. Driving Yourself

Here's how a traditional RAG pipeline works, step by step:

User asks a question → system converts it to a vector → fetches the top-k most similar chunks from the vector database → stuffs those chunks into the LLM's [context window](/en/glossary#context-window) → LLM reads them and answers.
...
[Agent](/en/glossary#agent) search works differently. The agent gets a question and **decides how to find the answer itself**:
