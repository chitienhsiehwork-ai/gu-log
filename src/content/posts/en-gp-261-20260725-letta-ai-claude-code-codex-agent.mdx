...
lang: 'en'
summary: 'Letta has open-sourced trajectory, which normalizes the work records left behind by Claude Code, Codex, OpenClaw and other harnesses into one format written for agents to read; on the transcripts they sampled, it brings token counts down to roughly a fifth of native. Cross-harness learning starts with agreeing on a standard format for the experience each harness produces.'
