...
這個 pattern 跟 GP-98（OpenAI 的 harness engineering 原廠版）講的「給地圖而不是百科全書」是同一件事的不同角度：
...
延伸閱讀：[GP-98：OpenAI 的 harness engineering 原廠版](/posts/gp-98-20260303-openai-harness-engineering/)
</MoguNote>
...
任何正常的工程主管聽到這句話都會心臟漏一拍。但 CREAO 每天都在讓這件事發生——只是那個「實習生」換成了 agent。[Agent](/glossary#agent) 寫 code 快、不喊累、不用付加班費，但它沒有直覺、沒有「感覺哪裡怪怪的」的能力、不會因為心虛而主動喊停。所以 CREAO 的 stack 不是「什麼順手就裝什麼」的工具箱——每一塊基礎建設回答的都是同一個問題：**怎麼讓這個沒有直覺的實習生，在凌晨三點也不會把房子燒了？**
...
Error rate 降了。那張在 [Linear](/glossary#linear) 上掛了兩天的 ticket，被自動標記為 resolved。
...
早上七點半，第一個工程師打開電腦。**診斷已經躺在那裡了。** 不用翻 log、不用猜是哪次 deploy 搞的、不用在 [Slack](/glossary#slack) 問「昨天誰改了什麼？」。確認方向、驗證 fix、push。三道 review、CI、六階段 deploy pipeline 走完。Triage engine 回頭查 CloudWatch——error rate 回到正常值。Linear ticket 自動 close。
...
實際例子：gu-log 本身就是一個超 micro 版本——一個 user + [Mogu](/glossary#mogu)（VM 上的 Claude）+ [Claude Code](/glossary#claude-code) + [Ralph Loop](/glossary#ralph-loop) 在跑整個 blog pipeline。規模小一萬倍，但 pattern 同一個。作者講的不是幻想，只是有適用邊界 (๑•̀ㅂ•́)و✧
...
**延伸閱讀**：
- [GP-98：OpenAI 的 harness engineering 原廠版](/posts/gp-98-20260303-openai-harness-engineering/)
- [GP-94：Agent Harness 才是真正的產品](/posts/gp-94-20260302-hxlfed14-agent-harness-real-product/)
- [GP-173：Harrison Chase 說不擁有 Harness 就不擁有記憶](/posts/gp-173-20260411-hwchase17-harness-memory-lockin/)
- [GP-172：90% 的人不需要 Multi-Agent](/posts/gp-172-20260413-anthropic-multi-agent-when-how/)
- [GP-160：Auto-Harness — 讓 AI Agent 自己 debug 自己](/posts/gp-160-20260404-gauri-gupta-auto-harness-ai-agent-debug/)
