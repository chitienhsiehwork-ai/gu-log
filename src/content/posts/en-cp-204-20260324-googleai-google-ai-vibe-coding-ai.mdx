...
Now the intern got an upgrade. Same request, but the intern searches, calls your function, and comes back with the final result in one trip. One fewer round trip means one fewer chance for bugs, and one less layer of orchestration code to maintain. For developers building [agents](/en/glossary#agent), this kind of "less glue code" upgrade often has more real-world impact than any flashy new model release.
