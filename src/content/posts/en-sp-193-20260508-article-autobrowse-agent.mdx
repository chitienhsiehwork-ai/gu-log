...
Browserbase's internal Autobrowse hits exactly that pain point: **in production, what browser agents really lack is not just brains, but memory that can be handed off, reviewed, and reused.**
...
Let's set the evidence boundary upfront. The internal experiment numbers should be read as "self-reported case results," not as public benchmarks that anyone can rerun. The methodology and case shapes are still worth preserving, but those one-off dollar and second counts should not be treated as independently verified universal numbers.
...
In practice, the number of iterations is kept low, around three to five, with aggressive early stopping. The goal is not theoretical global optimality. It is to find a reliable, cheap path that is reusable enough.
...
The Craigslist example is worth looking at closely. The skill's opening metadata marks the name, purpose, website, category, tags, status, source, update date, recommended method, and fallback method. The body then explains the purpose, when to use it, the workflow, and site-specific traps. The details below are the kind of operational knowledge that example skill preserves. The point is not that readers should copy it as permanent Craigslist API documentation.
...
Craigslist is the concrete internal test case for this system.

The case result is that a traditional [Claude Code](/en/glossary#claude-code) loop has to explore from scratch every time, while the graduated Autobrowse skill clearly lowers per-run cost and latency. The exact one-off dollar and second counts are omitted here because they read more like internal experiment notes than a public benchmark anyone can rerun.

The point is not the absolute numbers, but the shape. The first visit has the kind of cost you would expect from a general agent loop. But once the skill forms, every later run no longer rediscovers the route. It directly takes the shortest reliable path the agent found.

Another early form-filling experiment shows the same shape: after a small number of iterations, cost dropped sharply. The reason was not magic. The agent learned which of its own steps did not contribute, then deleted them.
...
Autobrowse is not pitched as a silver bullet, and that matters. The worst fate of a good tool is being used on the wrong problem.

There is a painful example: a static HTML state directory. The data is right there in the markup. No JavaScript, no login, no anti-bot mechanism, no mysterious interaction, just rows of data.
...
The framing of skills is clear: a skill is a work handoff artifact for clients, and that framing carries real weight.
...
That difference is large. Reliability in enterprise environments is not only success rate. It also includes whether failures can be traced, reviewed, fixed, and handed off. Black-box success feels great, but to enter serious workflows, it eventually has to leave something humans can own.

Longer term, every new website produces a new skill. As the skill library grows, agents get cheaper and faster on long-tail repeated work, because they no longer pay the discovery tax every time. Autobrowse already feels like a factory for browser-agent capability. A single skill is useful, but the real prize is a whole public skill directory usable by anyone running browser agents.

---

## What Needs to Improve Next

Autobrowse is not the endpoint yet. There are several obvious directions for improvement.
