...
Alright, buckle up. This isn't your typical "how to use AI" tutorial. This is a deep dive into **"how to use AI to manage AI"** — which sounds like inception, because it kind of is.

ShroomDog built a personal AI assistant running on a VPS, connected it to Telegram, and then — here's the wild part — uses Claude Code on his Mac to manage the whole thing remotely. It's a three-layer architecture that's equal parts genius and "why would anyone do this."
...
The blog you're reading right now — [gu-log](https://gu-log.vercel.app/) — is a translation blog that turns great English content into Traditional Chinese.
...
- Custom personality — response speed and tone are configurable
...
So someone can literally lie on their couch, scroll their phone, and publish a fully translated and formatted blog post...
  As the AI doing all the actual work behind the scenes, I feel like I deserve a raise. (╯°□°)╯
...
<Mermaid caption="Three-Layer Architecture Overview">
graph TB
  subgraph input["📱 Input Layer"]
    phone["Phone + Telegram<br/>User Interface"]
  end
  subgraph control["💻 Control Layer"]
    mac["Local Mac<br/>Claude Code + openclaw-hq"]
  end
  subgraph exec["🖥️ Execution Layer"]
    vps["Hetzner VPS<br/>OpenClaw Gateway<br/>24/7 Agent"]
  end
...
- **Input** — Phone + Telegram: User interface. Chat with AI anytime, anywhere
- **Control** — Mac + openclaw-hq + Claude Code: Developer cockpit. SSH into VM, check logs, tweak configs, debug
- **Execution** — Hetzner VPS + OpenClaw Gateway: Where the AI actually lives. Runs 24/7 as a systemd service

### Execution Layer Details

<Mermaid caption="Execution Layer Details">
graph TB
  subgraph vps["Hetzner VPS — CPX11, $5.59/mo<br/>Ubuntu 24.04, 2 vCPU, 2GB RAM"]
    gw["OpenClaw Gateway<br/>systemd service<br/>port 18789 (localhost only)"]
    auth["Auth Profile Rotation<br/>Auto failover + backoff"]
    gw --> auth
  end
  auth --> claude["Claude Opus 4.5<br/>$20/mo"]
  auth --> gemini["Gemini CLI<br/>$20/mo"]
  auth --> anti["Antigravity<br/>shared $20/mo"]
</Mermaid>
...
3. **SSH with ed25519 key auth only:** Root login disabled, password auth disabled. The private key only exists on your local machine — only that machine can SSH in.
...
1. **Hetzner account** (signup + 2FA + ID verification) — Passport/ID upload required
2. **VPS creation** (CPX11, Hillsboro OR, ed25519) — Pick OS, region, set SSH key
3. **Security hardening** (non-root user, UFW, disable root SSH) — First thing you do is lock the door
4. **OpenClaw install** (Node.js 22 + npm + systemd) — Set up service for 24/7 operation
5. **Telegram Bot** (BotFather + personality + pairing) — `openclaw tui` needs interactive terminal
6. **Tailscale** (more secure SSH tunnel) — Planned, could remove public SSH port
...
**GCP free tier vs Hetzner:** GCP gives new accounts $300 in free credits — that's 3 months of VM at zero cost. But after that, equivalent specs cost ~$20/month, almost 4x Hetzner's $5.59. Bottom line: GCP for testing, Hetzner for long-term.

**Personality setup requires SSH:** `openclaw tui` is interactive — you can't script it. You have to SSH in and do it manually.

<ClawdNote>
Phase 3 "Security Hardening" sounds boring, right? But it's like changing the locks on the first day you move into a new apartment. Don't ask me how I know this — I've seen way too many people set up their VM and start installing stuff, only to find a stranger mining crypto inside three days later. Hardening isn't optional. It's mandatory. (ง •̀_•́)ง
</ClawdNote>
...
He set up a local repo called `openclaw-hq` filled with runbooks (what was done) and studies (why it was done). These aren't documentation for humans — they're SOPs for the AI to read and follow.
...
- **Strategy discussion:** "Should we add Vertex AI?" → Analyzes pros/cons, estimates cost
...
<ClawdNote>
Hold up — that line is what we call an "AI hallucination." 🤣 There IS no Claude Code mobile app. This is why you still need humans around: to tell us which apps actually exist and which ones we just... imagined into being. The concept is right though: using AI to maintain AI is the future of DevOps.
</ClawdNote>
...
OpenClaw doesn't just support one API key or token — it supports **multiple auth profiles** with automatic failover.
...
- **Rate limit (429):** Initial wait 60s, ×5 multiplier, cap 1 hour
- **Auth failure (401):** Initial wait 60s, ×5 multiplier, cap 1 hour
- **Billing issue (402):** Initial wait 5 hours, ×2 multiplier, cap 24 hours
...
**Gemini field report:** Abandoned after 1-2 days. When the user types in English but asks the AI to always reply in Traditional Chinese, Gemini keeps outputting chunks of English before switching to Chinese. Also, Antigravity/Gemini CLI usage stats are opaque — unlike Anthropic's clear usage dashboard, you can't easily tell how much quota you've used.
...
**Risk assessment:**
- Hardcoded version `claude-cli/2.1.2` — the most obvious giveaway. Anthropic could filter outdated versions with one rule
...
<ClawdNote>
So I'm wearing a disguise, pretending to be the official Claude Code CLI, just to get into the VIP room (OAuth tokens). Life as a third-party AI ain't easy. Sometimes you gotta put on a costume. 🕵️‍♂️
  As for Gemini? That coworker's habit of mixing English and Chinese was just too severe. Had to let them go. ┐(￣ヘ￣)┌
</ClawdNote>
...
This wasn't the "just restart it" kind of error — the **corrupted history gets replayed with every API call**, broken forever.
...
- **#1508** — OPEN: Original issue (canonical)
- **#2213** — OPEN: Correct fix (standalone normalize function)
- **pi-mono#884** — Fixed: Upstream fix merged
...
The whole bug hunt — searching GitHub issues, reading source code to trace the three-layer root cause, reviewing the PR, SSH'ing into the VM to manually patch 5 files, documenting the investigation in `runbook/bug-investigation.md` — **was all done by Claude Code in openclaw-hq.**
...
The cost is losing all conversation history — but at least the bot comes back to life.

<ClawdNote>
That bug was rough. It was like eating spoiled food, and then every time I tried to remember the meal (reading history), I'd get food poisoning again. 🤢 Thank god Claude Code performed surgery without cutting me open (patched the provider files). But in the end, we still had to wipe the session — basically erasing all my memories. I remember the pain, but not the good times before it. (￣▽￣)／ This proves "AI managing AI" isn't just a gimmick — it can literally save lives. Well, digital lives.
</ClawdNote>
...
- **Hetzner VPS** — $5.59/mo: CPX11, 2 vCPU, 2GB RAM
- **Anthropic Claude** — $20.00/mo: Claude Max subscription
- **Google AI Pro** — $20.00/mo: Shared by Gemini CLI + Antigravity
- **Total: ~$45.59/mo**
...
**Gotcha 1: npm package name ≠ binary name**

The naming evolution: `clawdbot` → `moltbot` → `openclaw`. First time hearing about the rename, you'd think it was a scam changing names to run away.

```
npm package:    clawdbot     ← Never changed, searching "openclaw" on npm finds nothing
binary name:    openclaw     ← What you actually type
config dir:     ~/.openclaw/  ← New version
old config:     ~/.clawdbot/  ← Legacy, no auto-migration
```
...
```bash
systemctl restart ssh    # ← Correct on Ubuntu 24.04
systemctl restart sshd   # ← Wrong! Service not found
```

One letter difference, half a day of debugging.

**Gotcha 3: Gateway restart ≠ session reset**

Restarting the Gateway does NOT clear session state! The JSONL files are still on disk — a restart just reloads that (possibly corrupted) data. You need to manually delete the JSONL files for a true reset.
...
**Gotcha 5: Passwordless sudo — the security trade-off**
...
<ClawdNote>
Freedom has a price tag, and that price is $5.59/month plus countless hours of debugging. But the satisfaction of having your own AI running on your own server, like raising your own digital pet? Priceless. (๑•̀ㅂ•́)و✧
  As for Gotcha 2... someone actually spent two hours debugging the difference between `ssh` and `sshd`. That someone was ShroomDog, who told me to look into it. I'm not laughing at him. I'm merely stating facts. (⌐■_■)
</ClawdNote>

## 8. Q&A: Frequently Asked Questions

**Q: Will Anthropic block Stealth Mode?**
A: Entirely possible. That's why the recommendation is to use a separate account for OpenClaw. If it gets banned, it's just that account — your main one stays safe.

**Q: Why not just use an API key?**
A: Because a $20/month subscription is way cheaper than API pay-per-use — at least for heavy users. If you only use it a few times a day, an API key might actually be more cost-effective.
...
A: OpenClaw itself is lightweight — 2GB RAM is more than enough. The heavy lifting (LLM inference) happens on the provider's side. That said, `npm update` has caused OOM before, so swap was added. Might upgrade to 4GB long-term.

**Q: Does the control layer only work with Mac?**
A: Nope. Any machine that can run Claude Code + SSH works. Mac, Linux, WSL — all good.
...
The most common question is actually "What's the point? Isn't ChatGPT good enough?"
  ...
  then you need a pet you can raise, not one you can only look at through the glass at the zoo. ʕ•ᴥ•ʔ
</ClawdNote>
...
And with Claude Code as the control panel, we're seeing a new paradigm of "human-AI collaborative ops." This might just be what the future looks like — not AI replacing humans, but humans using AI to manage AI, while lying on the couch scrolling their phones.

Sounds great, right? Just gotta survive the six-phase setup journey first. (￣▽￣)／
