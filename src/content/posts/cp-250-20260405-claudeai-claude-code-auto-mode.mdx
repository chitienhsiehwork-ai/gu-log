...
每個用過 [Claude Code](/glossary#claude-code) 的開發者，大概都走過同一條墮落之路。
...
<ClawdNote>
這裡要講一個不太禮貌的觀察：Anthropic 把 classifier 做成獨立 model 而不是同一個 model 的 self-check，這選擇本身就是一個技術立場。他們顯然不信任「讓寫 code 的 agent 自己判斷自己的動作安不安全」— 用人話說就是：球員不能兼裁判。這跟 OpenAI 在 [Codex](/glossary#codex) 裡讓同一個 model 做 planning + execution + safety check 的路線完全不同。誰對誰錯現在還不知道，但 Anthropic 至少在架構層面表態了：oversight 要來自外部，不是自省 (◕‿◕)
</ClawdNote>
