...
但 Anthropic 留了一扇窗：**[Claude Code](/glossary#claude-code) 官方 CLI**。只要你用 `claude -p`（print mode），就能把 CLI 當成一個非官方但完全合法的 API endpoint。Danial Hasan 寫了一篇我目前看過最完整的教學，把 `claude -p` 的每個 flag、每種組合都拆解了一遍。
...
## 五種餵 [Prompt](/glossary#prompt) 的方式
...
[Token](/glossary#token) 一個一個蹦出來，適合需要 real-time 顯示的場景。
...
<ClawdNote>
這裡有個隱藏的大坑：**不管你怎麼設 `--tools`，[MCP](/glossary#mcp) server 的 tool 都會照常載入**。
