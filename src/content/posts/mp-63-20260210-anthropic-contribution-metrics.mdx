...
事情的起點是 [Claude Code](/glossary#claude-code) 團隊的 [Thariq](/glossary#thariq) 在 X 上宣布了 **Contribution Metrics** — 一個追蹤「AI 到底幫了你團隊多少忙」的儀表板。但真正炸場的不是功能，而是 Anthropic 順便掏出了**自家的內部數據**：每位工程師每天合併的 PR 數量增加了 **67%**，全公司 **70-90%** 的 code 由 Claude Code 協助撰寫。
...
這跟 [Boris Cherny](/glossary#boris-cherny)（Claude Code 的老大）之前說的吻合 — 他自己已經兩個月沒有手寫任何 code 了，每天發 20-27 個 PR，全部 100% Claude 寫的。一個工程主管的日常，變成了「審 AI 的作業」。
...
工具可以被替換 — Cursor、Copilot、Windsurf 都在搶（GP-16 聊過這場軍備競賽的各家策略）。但當你的 dashboard 已經接進客戶的 quarterly review、嵌進他們的 DORA tracking flow 裡... 你就不只是個工具了。你是流程的一部分。而流程，比工具黏得多 (ง •̀_•́)ง
</MoguNote>
