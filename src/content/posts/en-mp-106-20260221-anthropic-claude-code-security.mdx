...
Anthropic clearly sees this too, which is why they just released [Claude Code Security](https://www.anthropic.com/news/claude-code-security) — a security scanning capability built right into [Claude Code](/en/glossary#claude-code), currently in limited research preview.
...
Fun cross-reference: MP-62 covered how Opus 4.6 learned to "play nice" in the Sabotage Risk Report — same model, dual nature. It can both conceal and detect. That duality is exactly why Anthropic wants defenders armed first.
</MoguNote>
...
<MoguNote>
Here is the dirty secret of most security scanning setups: the tool runs, spits out 5,000 alerts, and about 4,800 of them are noise. The security engineer gives up around alert number 50, mutes the [Slack](/en/glossary#slack) channel by alert 200, and then alert 4,999 — the one that would actually get exploited — sits there unread. The biggest irony in security is that noisy tools make real problems harder to find, not easier (╯°□°)╯

MP-91 covered 12 protocol-level landmines in [MCP](/en/glossary#mcp) alone. Now add application-layer vulnerabilities on top, and your alert dashboard probably needs its own alert dashboard.
</MoguNote>
...
GP-51 told the story of [OpenClaw](/en/glossary#openclaw) agent skills turning into attack surfaces — when an agent ecosystem's supply chain gets compromised, even the skills themselves become backdoors. Without AI-grade detection, defenders simply cannot keep up.
</MoguNote>
