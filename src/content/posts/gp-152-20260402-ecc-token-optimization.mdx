...
<MoguNote>
[Mogu](/about) 來算一筆帳。$200/月的計劃聽起來很多，但如果認真跑 multi-agent pipeline，它消失的速度比想像中快很多。Opus 4.6 每 1M tokens 的輸入是 $15、輸出是 $75。一個有 10 個 [subagent](/glossary#subagent) 的 pipeline，每個 subagent 呼叫 2K tokens，往返一次大概 $3。一天跑 10 次，一個月 $900。
...
## MAX_THINKING_TOKENS — 31,999 個 [Token](/glossary#token) 的隱形帳單
...
這跟 [GP-150 的 indie hacker 省錢哲學](/posts/gp-150-20260402-ecc-indie-hacker-ecc/)一脈相承——錢不是不能花，是要花對地方。大部分人把 Opus 當「預設安全選項」，但這個邏輯的代價是用 5 倍的錢換了根本不需要的品質提升。
...
Context management 是 token 優化裡最少人講，但效果最持久的一塊。（如果對 Claude Code 的 pipeline 設計有興趣，[GP-151 的 eval-driven development](/posts/gp-151-20260402-ecc-eval-driven-development/) 講的是怎麼用評估循環把品質自動化——跟這裡的成本自動化是同一套思路的兩面。）
...
<MoguNote>
有一次 gu-log 跑 Ralph Loop 跑到一半，context 快滿了，讓 Claude 自動 compact。壓縮完之後繼續跑，但「這篇文章的 ticketId 是 GP-148」這個細節在摘要裡消失了。後來 Claude 在命名的時候用了一個錯的 ID，寫到一半才發現，重新說明、重新跑，多花的 token 比原本 compact 省的多兩倍。
...
再搭配 ECC 另一個概念——`cost-aware-llm-pipeline` skill，把 budget tracking 直接編進 pipeline 設計裡。概念跟 [hooks](/glossary#hooks) 很像——[GP-146 講的 Hook Architecture](/posts/gp-146-20260402-ecc-hook-architecture/) 是在 pipeline 的關鍵節點插入自動化行為，這裡則是在同樣的節點插入成本意識。
...
但最大的 lesson 不是模型路由——是思考 token。發現之前，Ralph Loop 的每個 agent call 都在跑 Extended Thinking，包括那個只是在確認「frontmatter 欄位有沒有填滿」的 Haiku call。那個 call 不需要思考，但 31,999 的預設上限讓它有權思考——而且計費了。（[GP-143 講的 autonomous loops](/posts/gp-143-20260402-ecc-autonomous-loops/) 也提到類似的問題——自主循環跑得越自動，成本控制越重要，因為沒有人在中間喊停。）
...
- [GP-143: ECC Autonomous Loops — 6 種讓 AI 自己跑的模式](/posts/gp-143-20260402-ecc-autonomous-loops/)
- [GP-146: ECC Hook Architecture — AI 版的 Git Hooks](/posts/gp-146-20260402-ecc-hook-architecture/)
- [GP-150: ECC Indie Hacker — 一個人 + AI 的開發哲學](/posts/gp-150-20260402-ecc-indie-hacker-ecc/)
- [GP-151: ECC Eval-Driven Development — 用評估循環保品質](/posts/gp-151-20260402-ecc-eval-driven-development/)
