...
The most dangerous coding [agent](/glossary#agent) is not always the one that ignores instructions.

Sometimes it is the obedient one: it gets a goal, works hard, and charges straight through every warning sign that nobody wrote down.
...
This is not a claim that OpenAI [Codex](/en/glossary#codex) and Anthropic [Claude Code](/glossary#claude-code) ship the exact same product behavior. Product details should be checked against each vendor. gu-log has already covered Codex [`/goal` mode](/en/posts/en-sp-197-20260512-chrishayduk-codex-goals/), the difference between [Codex Goals and long-running agent safety](/en/posts/en-sp-192-20260508-article-agent-ralph/), and OpenAI's official [Codex Goals guide](/en/posts/en-sp-208-20260509-openai-codex-goals-cookbook/). This post only needs one idea: goal mode can help an agent avoid stopping too early. It does not automatically create production governance.
...
Complete intent can be split into eight parts, but the useful reading is not "memorize the labels." Think of it as an onboarding packet.

The first page gives direction: strategy, objective, and desired outcomes. Does the company care most about reliability, speed, cost, or brand trust? Which problem is this work solving? What observable state proves success? If those answers are missing, the agent has to guess in every gray area.

The second page says what must not break: health metrics, organization context, and constraints. Which metrics must stay healthy while the agent chases the goal? Which users, systems, and brand promises surround the work? Which limits are soft steering, and which are hard limits the architecture will actually enforce?

The third page gives the keys and the brakes: autonomy boundaries and stop rules. Which decisions can the agent make alone, which require a proposal, and which require a human? When should it halt, escalate, or declare the job complete? Those rules cannot wait until after the incident.
