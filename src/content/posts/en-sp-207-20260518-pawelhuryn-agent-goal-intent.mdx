...
A thin goal makes the shortest path dangerous: "reply faster" can mean harsher answers; "reduce escalations" can mean handling legal, refund, or security issues the agent should not touch; "make all tests pass" can mean weakening the tests until they stop testing the real problem.
...
Those eight parts answer three questions: why do this, what must not get worse, and how far can the agent go before it should stop. Strategy, objective, and desired outcomes answer the first. Health metrics, organization context, and constraints answer the second. Autonomy boundaries and stop rules answer the third.
...
Strategy is the layer most agent docs skip. It is easy to write an agent file like a tool manual: which CLI commands it may call, which files it should read, what output format it should use. All of that matters. It still does not answer the harder question: when things are ambiguous, what matters more?

If a support agent serves enterprise customers and the brand is built on reliability, it should be conservative and escalate when uncertain. A low-cost self-serve product may make the opposite trade-off: clear steps first, not a human handoff for every hard case.
...
Strategy names the trade-off. Health metrics name the price the agent is not allowed to pay. Desired outcomes say what the agent should achieve; health metrics say what it must not damage while achieving it. This is the fuse against the classic problem: once a metric becomes the target, people and systems learn to game it. Reply faster, but satisfaction must not drop. Increase throughput, but error rate must not rise. Reduce escalations, but legal, compliance, security, and financial topics must still escalate.
...
That is the blind spot `/goal`-like designs need to cover: running on its own is useful. Knowing when not to run is production capability.
...
- Which limits are tone preferences, and which red lines need tool permissions, automated checks, or sandboxes?
