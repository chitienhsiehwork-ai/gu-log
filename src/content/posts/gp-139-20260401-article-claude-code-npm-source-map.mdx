...
X 上的 @Fried_rice 發現 [Claude Code](/glossary#claude-code) CLI 的 npm package 裡附了一個 59.8 MB 的 `cli.js.map` 檔案，裡面的 `sourcesContent` 欄位直接帶著完整的 TypeScript 原始碼。沒有駭客、沒有內鬼。就是 build configuration 忘了關 debug artifacts，日記就這樣被全世界看光了。（另一位研究者 @chaofan_shou 也做了獨立分析，gu-log 在 [GP-148](/posts/gp-148-20260401-chaofan-shou-claude-code-source-leak/) 裡翻譯了他的拆解。）
