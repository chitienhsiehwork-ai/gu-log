...
## 🏗️ Floor 4：FastAPI — 每個 service 的 URL prefix 設計

<LevelUpProgress current={4} total={7} title="一個 Domain，多個 Service" />

好，外部的大樓結構蓋好了（DNS → Ingress/Istio）。現在輪到每個 service 的「室內裝潢」。

你的 `user-service` 會被 Ingress 以 `/users` 這個 path prefix 導進來。但問題是：**FastAPI 收到的 path 是什麼？**
...
每個 endpoint 都要帶 `/users` prefix。

### 情況二：改寫 path

Ingress 把 `/users/123` 改寫成 `/123` 再轉給 `user-service`。

這時候 FastAPI 可以寫得比較乾淨：
...
service 本身不需要知道自己的 prefix 是什麼。

<AnalogyBox title="🏠 房間門牌的比喻">

就像你住在「信義路 100 號 **5 樓之 3**」。

- 情況一（不改寫）：郵差把整個地址「信義路 100 號 5 樓之 3」交給你，你要自己知道「5 樓之 3」是你
- 情況二（改寫）：大樓管理員收到信之後，把地址改成「之 3」再轉給你，你只需要認得「之 3」就好

兩種方式都行，但「改寫」讓每個 service 更獨立 —— 它不需要知道自己在大樓的哪一層。

</AnalogyBox>

<Mermaid chart={`%%{init: {"look": "handDrawn"}}%%
graph TD
  R["GET /users/123"] --> I["Ingress"]
  I -->|"不改寫<br/>/users/123"| F1["FastAPI<br/>root_path=/users"]
  I -->|"改寫 path<br/>/123"| F2["FastAPI<br/>root_path 不用設"]
  style R fill:#ffeaa7,stroke:#fdcb6e
  style I fill:#74b9ff,stroke:#0984e3
  style F1 fill:#55efc4,stroke:#00b894
  style F2 fill:#55efc4,stroke:#00b894
`} caption="有沒有做 path rewrite，決定了 FastAPI 要怎麼設計 endpoint" />

### 推薦做法

大多數團隊的做法是：

1. **Ingress 做 path rewrite**（strip prefix）
2. **FastAPI 設 `root_path`**（讓 Swagger UI 的路徑正確顯示）

```python
# user-service
app = FastAPI(root_path="/users")

@app.get("/")          # 外面看到的是 /users/
@app.get("/{user_id}") # 外面看到的是 /users/{user_id}
```

`root_path` 不會影響 routing，它只影響 **Swagger 文件** 中 endpoint 的顯示路徑。這是 FastAPI 的一個貼心設計。

<ClawdNote>
`root_path` 是 ASGI 的標準概念，不是 FastAPI 獨有的。當你的 app 跑在 reverse proxy 後面時，設定 `root_path` 可以讓 OpenAPI schema 正確反映真實的 URL。不設的話，Swagger UI 會以為 base URL 是 `/`，使用者點 "Try it" 會打到錯的地方。
</ClawdNote>

<LevelUpQuiz
  question="FastAPI 的 root_path 設定主要影響什麼？"
  options={[
    { label: "A", text: "API 的實際 routing 邏輯" },
    { label: "B", text: "API 的回應速度" },
    { label: "C", text: "Swagger/OpenAPI 文件中顯示的 endpoint 路徑" },
    { label: "D", text: "資料庫的連線設定" },
  ]}
  answer="C"
  explanation="root_path 是 ASGI 標準，主要影響 OpenAPI schema 中的 server URL。設了 root_path='/users' 之後，Swagger UI 會知道真實的 base path 是 /users，'Try it' 功能才會打到正確的 URL。它不影響 FastAPI 內部的 routing。"
/>
