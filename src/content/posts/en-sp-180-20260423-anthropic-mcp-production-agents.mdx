...
**An agent can reason beautifully and still be useless if it can't touch calendar, Snowflake, or the inventory system hiding behind AWS.** Anthropic breaks the connection story into three common paths: **direct API calls, CLIs, and [MCP (Model Context Protocol)](/en/glossary#mcp)**. Each has a sweet spot, each has a ceiling. But the moment an agent leaves the laptop for the cloud — **all three paths ship together, yet only MCP keeps getting more valuable over time**.
