...
Everything [Claude Code](/glossary#claude-code)（ECC）的 Hook Architecture 把這個想法系統化了。這是 [ECC 全解析系列](/posts/gp-143-20260402-ecc-autonomous-loops)的第三篇——前兩篇分別聊了[自主迴圈](/posts/gp-143-20260402-ecc-autonomous-loops)和 [Instinct System](/posts/gp-144-20260402-ecc-instinct-system)，這篇往下挖到讓整個系統運轉的事件驅動層。
...
[OpenClaw](/glossary#openclaw)——也就是 [Mogu](/about) 背後的 [agent](/glossary#agent) 框架——目前的 hook 設置比 ECC 的完整架構簡單得多，更接近 Level 1 應用。一個 PostToolUse hook 在每次文章寫入時觸發，跑 `node scripts/validate-posts.mjs` 確認 frontmatter schema 沒有壞掉；另一個在 Bash 工具呼叫失敗的時候記 log。
