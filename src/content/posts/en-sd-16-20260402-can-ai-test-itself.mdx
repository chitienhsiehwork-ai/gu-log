...
Every article on gu-log runs through [Ralph Loop](/en/glossary#ralph-loop) — Claude writes the article, a separate Claude instance scores it against a fixed rubric (Persona, MoguNote quality, Overall Vibe), and if it doesn't pass, it gets rewritten. Up to three rounds. This is AI testing AI, applied to content quality.
...
The [Mogu](/en/glossary#mogu) instance on the Linux VM (running 24/7) can delegate tasks to agents on other machines via SSH. This article was produced that way — Claude Code on my Mac and Mogu on the VM collaborating, Mogu sometimes sending tasks to another agent.
