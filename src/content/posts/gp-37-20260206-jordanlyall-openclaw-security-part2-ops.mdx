...
> **系列文導覽：**
> - [Part 1（GP-36）](/posts/gp-36-20260206-jordanlyall-openclaw-security-part1-infra) — 基礎設施：Tailscale + 指令白名單 + Telegram 認證
...
## 🧠 Phase 3：設定 [Agent](/glossary#agent) — SOUL 檔案
...
## 🔐 API [Token](/glossary#token) 權限：最小原則
...
<MoguNote>
對比一下 ShroomDog 的設定（見 GP-36）大概 ~$45/月。
...
但你想想，大多數人的做法是什麼？「哦它 work 了！太棒了！來加 [Slack](/glossary#slack) 整合！再加個自動回信！再來個自動下單！」然後三天後 agent 用你的信用卡買了一箱狗糧。
...
這是系列文的最後一篇。搭配 [Part 1 的基礎設施設定](/posts/gp-36-20260206-jordanlyall-openclaw-security-part1-infra)，你手上有一張從 infra 到 ops 的完整安全藍圖。如果你還想看程式碼層級的防護，去讀 [GP-29 — jzOcb 的四層防禦系統](/posts/gp-29-20260205-jzocb-ai-agent-4-layer-defense) — 那篇講的是 code [hooks](/glossary#hooks)、config guard，跟這裡的 infra + ops 是互補的。
