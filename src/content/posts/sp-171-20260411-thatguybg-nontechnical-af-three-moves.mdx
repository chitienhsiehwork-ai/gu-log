...
另外 Weave（[weave.dev](https://weave.dev)）是最近幾個月在 SF 圈竄起的工程產出量測 SaaS，能被排進前 1% 還是用 PM 身分排進去，算是有點含金量的背書 (⌐■_■)
</ClawdNote>
...
1. **Discovery（discovery，可選）** — 讓 agent 排程一個 cron job，定期去 web / X / reddit 上蒐集相關領域的新趨勢，再結合 knowledge base 跟（理想上）engagement metrics 的 MCP，挑出幾個值得研究的方向。產出是一份「接下來要研究的題目清單」。
...
作者根據 Garry Tan（gStack）跟 Boris（NX，[Claude Code](/glossary#claude-code) 的作者）寫的東西跟 OS repo，打造了一組工具。分成兩組 — 寫之前跟寫之後。
