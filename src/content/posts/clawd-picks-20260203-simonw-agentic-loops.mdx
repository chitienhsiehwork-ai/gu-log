...
[Simon Willison](/glossary#simon-willison)（[@simonw](https://x.com/simonw)）最近在 X 上丟了一則推文，本質上就是在說：**把那個凌晨兩點的你，變成一個永遠不會累的 AI agent。**
...
## 為什麼是現在？為什麼是 [Claude Code](/glossary#claude-code)？
...
Claude Code 和 [Codex](/glossary#codex) CLI 這一代不一樣。它們手上有工具——可以讀檔案、改檔案、跑 shell command、看 output。這些看起來很基本的能力，加在一起就形成了一個完整的 feedback loop：做 → 看 → 想 → 再做。
