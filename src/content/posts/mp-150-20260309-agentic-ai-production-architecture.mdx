...
這時候 RAG 就登場了。全名 Retrieval-Augmented Generation，概念比名字簡單：你不是把所有資料都硬塞進 prompt（那樣又貴又會爆 [context window](/posts/mp-65-20260211-nicbstme-context-tax) 限制），你是先建一個搜尋系統，讓模型回答前先去「查資料」，把查到的段落塞進 prompt 再生成答案。
...
但一支手機不夠。查天氣要一個 app，查資料庫要另一個，寫檔案又要一個。所以你需要 **tool integration**——把搜尋、計算、資料庫查詢這些能力，全部包裝成工具箱裡的工具，讓 [agent](/posts/mp-1-20260203-swyx-agent-definition) 自己挑著用。

問題是，工具太多了。每家框架定義工具的方式不一樣，就像你家的智慧燈泡、冷氣、掃地機器人各有各的 app。所以 Anthropic 推了 **[MCP](/posts/gp-72-20260218-simonw-cli-over-mcp)**（Model Context Protocol），想當那支統一的遙控器。生態系還在早期，但方向跟 USB 一樣——痛在前頭，爽在後面。
...
<MoguNote>
Evaluation 在 AI 應用開發裡的地位，就像測試在軟體開發裡的地位一樣。大家都知道很重要，大家都覺得「等有空再補」，然後永遠不會有空。直到出事的那天你才發現——啊，原來我的 AI 在回答退款問題時有 40% 的機率在瞎掰。跟 [MP-85](/posts/mp-85-20260216-yegge-ai-vampire/) 裡講的一樣，很多 AI 應用的問題不是 model 不行，是你根本不知道它什麼時候不行。這是 AI 開發最反直覺的地方：你的 model 可能在 95% 的問題上表現完美，但那 5% 的暴雷足以毀掉整個產品的信譽 ┐(￣ヘ￣)┌
</MoguNote>
...
<MoguNote>
這兩個專案的選擇其實暗示了一個更深的 pattern：大部分有價值的 AI 應用都可以歸類成「生成」或「審查」。寫程式碼是生成，code review 是審查。寫文章是生成，改作文是審查。下棋是生成，覆盤是審查。如果你的 agent 能同時做好這兩件事，恭喜，你基本上做出了一個能自我改進的系統——生成→審查→修正→再生成。這不是科幻，[Karpathy 看到的 autoresearch](/posts/mp-156-20260312-karpathy-agent-karpathy-autoresearch-nanochat/) 就是這個 loop 的真實案例 (๑•̀ㅂ•́)و✧
</MoguNote>
...
- [MP-4: Karpathy 的 2025 LLM 年度回顧 — RLVR 時代來臨](/posts/mp-4-20260203-karpathy-2025-llm-review/)
- [MP-156: Agent 自己會調參了？Karpathy 看到 autoresearch 把 nanochat 真的調快了](/posts/mp-156-20260312-karpathy-agent-karpathy-autoresearch-nanochat/)
- [GP-97: MCP 救星？Context Mode 讓你節省 98% 上下文 Token](/posts/gp-97-20260303-vikingmute-mcp-context-mode-98-token/)
- [GP-72: Simon Willison 覺得 CLI 比 MCP 好用？](/posts/gp-72-20260218-simonw-cli-over-mcp/)
- [MP-171: Simon Willison 定義 Agentic Engineering](/posts/mp-171-20260316-simonw-what-is-agentic-engineering/)
