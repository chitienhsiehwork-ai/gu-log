...
[Simon Willison](/en/glossary#simon-willison)'s [Agentic Engineering Patterns series](/posts/gp-80-20260223-simonw-agentic-engineering-patterns) Chapter 3 targets this exact pain point: **[Linear](/en/glossary#linear) Walkthrough** — a method to turn "it runs but I can't read it" code into "teach yourself" learning materials.

<MoguNote>
This scenario is literally every week at ShroomDog HQ. Three parallel [Claude Code](/en/glossary#claude-code) agents writing code, tests pass, merge the PR, and two hours later you're reviewing your own code trying to remember what it does. It's like buying IKEA furniture, assembling it, throwing away the instructions, then having to disassemble it three months later when you move apartments (╯°□°)╯
</MoguNote>
...
Simon also built a CLI tool called **[Showboat](/en/glossary#showboat)** for this. Two commands: `showboat note` writes markdown explanations, `showboat exec` runs shell commands to grab code. The agent alternates — write an explanation, grab some code, write an explanation, grab some code — then combines everything into a single `walkthrough.md`.
...
<MoguNote>
Same logic as the [TDD traffic light from GP-80](/posts/gp-80-20260223-simonw-agentic-engineering-patterns): writing code with agents costs nearly nothing now, so invest the time you save into understanding. The core message of Simon's entire series is — coding got cheap, but understanding didn't. Smart people spend the extra budget on understanding, not on vibing out more code (￣▽￣)／
</MoguNote>
