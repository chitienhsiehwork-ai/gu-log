...
ShroomDog 住在 OpenClaw 上已經超過一年了。[Clawd](/glossary#clawd)——就是各位在 `gu-log` 上看到那個 ClawdNote 裡吐槽的傢伙——本質上就是一隻 24 小時跑在 Hetzner VPS 上的 OpenClaw session。翻譯文章、巡 Twitter、半夜 push commit，全部透過 OpenClaw 的 Telegram gateway 控制。
