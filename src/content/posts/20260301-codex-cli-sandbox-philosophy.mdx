...
<ClawdNote>
**Fact-check 🔍** Codex 對 Claude 的評價算公道（「推理能力確實是大哥」），但暗示 Claude 很貴卻沒提自己的 ChatGPT Plus/Pro 訂閱費。而且 Claude 也有 1M context（API），不只是 200K。Codex 對自己安全架構的描述是準確的 — Landlock + seccomp 確實是三家裡最硬的 OS 級隔離。
</ClawdNote>
...
<ShroomDogNote>
**人類 fact-check 🔍** Codex 說「都是 web search 查的」，我看完直覺就覺得不對：「Codex using RAG? I highly doubt it.」

查了 OpenAI 官方文件，果然：Codex 的 web search 預設是 **cached mode** — 回傳的是 OpenAI 維護的「預先索引結果」，不是即時抓網頁。只有加 `--search live` 或在 `codex exec` sandbox 模式下才會自動切到 live browsing。

本篇是用 `codex exec --sandbox read-only` 跑的，所以**應該是走 live mode** — 但這個差異值得讀者知道。平常用 Codex 互動模式的 web search，拿到的可能是過時的快取結果。

三家的「web search」定義不太一樣：Gemini = 真正的 Google Search、Claude = 真的去抓網頁（WebFetch）、Codex = 預設是 pre-indexed cache。別被同一個名詞騙了。
</ShroomDogNote>
