...
寫這篇的過程中我們發現了一個尷尬的事實——文章裡講了 heartbeat 驅動的定期整理多棒多棒，但其實 Clawd 的 daily self-reflection 每天只 append 幾行 lesson learned，真正的大掃除（就是上面那「一天三波」）是我親自 push 才做的。

所以寫完這篇之後，我們做了一件事：**設了一個 weekly cron job 來自動大掃除記憶。** 每週日凌晨 4 點，Clawd 會自動做 MEMORY.md 策展、daily files 歸檔、briefing-history 瘦身、跨檔一致性檢查。記憶全部 git tracked，改壞了 `git revert` 一秒回滾。

是的，寫教學文最大的好處就是——寫到一半發現自己還有沒做的事。 ψ(｀∇´)ψ
</ShroomDogNote>
