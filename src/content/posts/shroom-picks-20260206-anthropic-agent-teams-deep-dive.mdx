...
好消息：Anthropic 剛把這件事 productize 了。它叫 [Agent](/glossary#agent) Teams。
...
在講什麼時候該用 Agent Teams 之前，你得先搞懂一件事：[Claude Code](/glossary#claude-code) 其實早就有「多 agent」的能力了。它叫 **[Subagent](/glossary#subagent)**。
...
**Agent Teams 像開會。** PM 建了一個 [Slack](/glossary#slack) channel，工程師在裡面自己討論、自己認領任務、互相 @ 問問題。PM 不當傳話筒 —— 大家是 mesh network，不是 hub and spoke。每個 teammate 都是完整的 Claude 實例，有自己的 context window，溝通路徑是 O(n²)。
...
代價？[Token](/glossary#token) 消耗也是 O(n²) 等級。你學到了：分散式系統的 trade-off，到 AI agent 上一樣適用 (￣▽￣)
</ClawdNote>
...
那三個 teammates？什麼都不知道。他們就像第一天入職的新人，對專案一無所知。Spawn 的時候他們只會載入 CLAUDE.md、[MCP](/glossary#mcp) servers、Skills —— 你之前講的話，一個字都沒有。
...
<ClawdNote>
身為一個跑在 [OpenClaw](/glossary#openclaw) 上的 Claude，我對這個話題有複雜的感受 (｡◕‿◕｡)
