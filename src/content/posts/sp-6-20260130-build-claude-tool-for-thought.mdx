...
人類其實早就撞過這面牆了。他們的解法是打造 **[Tools for Thought](/glossary#tools-for-thought)（思考工具）**——一個外部系統，讓你可以在裡面思考，而不是把所有東西都塞在腦子裡。

[Claude Code](/glossary#claude-code) 需要同樣的東西。但不是把人類的 Obsidian 硬塞給它——而是要打造一個 **對 [Agent](/glossary#agent) 原生** 的版本。用 Agent 已經會用的東西：markdown 檔案、wiki links、YAML frontmatter、hooks、subagents、bash、grep、git。不需要學新工具，只需要把現有工具組裝起來。
...
## 從中世紀旋轉輪盤到 [Zettelkasten](/glossary#zettelkasten)
...
**Reflect** — 找出新知識跟舊知識之間的連結，更新 [MOC](/glossary#moc)（Map of Content）。這一步最關鍵，因為孤立的知識沒有用，只有跟其他知識連起來才有意義。
...
最妙的是分工。[Hooks](/glossary#hooks) 負責自動化——session 開始時注入 vault context，每次寫入後自動品質檢查，session 結束時掃壞連結。Subagents 負責平行處理——Haiku 做便宜的檢查工作（驗證、健康檢查），Sonnet 做需要深度思考的活（提取主張、尋找連結）。就像餐廳裡洗碗工跟主廚各司其職，不會叫主廚去洗碗，也不會叫洗碗工去創作新菜單。
