...
Unit test 就是普通 vitest。`calendar-recall.mjs` exports 的那些 pure function——`parseEventLine`、`eventMatchesKeyword`、`searchKeyword`、`formatJson`——每一個都對 fixture 資料跑測試。捕捉的 bug 類型：Unicode 地名字被靜默砍掉、閏年日期解析錯、只有一個參加者時 attendees 陣列被省略。小、無聊、關鍵。

對 `context-now`，unit test 測的是時區格式、quiet-hours 偵測、跨 DST 的 `minutesUntil` 計算。其中一個測試會餵一個 DST 轉換前 3 分鐘的時間，驗證輸出不會突然跳 60 分鐘——**這剛好就是「28 分鐘」那個 bug 的結構性防線。現在那個 bug 結構上不可能再發生。**

Integration test 打的是真 endpoint、真資料。unit test 用的 fixture 太乾淨，實際的 calendar 資料會出現格式錯的 event line、缺 timezone 欄位、Windows 換行符、跨午夜的事件。這些只有真資料才抓得到。Garry 全系統有 179 個 unit test、5 組 suite，2 秒內跑完。

### Step 5：LLM evals——用 model 判斷另一個 model

有些輸出 deterministic test 判不了——「這份 calendar 摘要有沒有用？」不是一個 yes/no 問題。Garry 用 **LLM-as-judge**：一個 model 依照 rubric 評另一個 model 的輸出。
...
另一個 eval 給 agent 一個 UTC 時間戳問「這是本地幾點？」正確行為：跑腳本、引結果。錯誤行為：心算。eval 同時捕捉「答錯」**跟**「過程錯」——因為就算這次心算剛好對，下次就會錯。

Garry 給了一個很真實的 heuristic：**想找還缺什麼 test？把對話紀錄裡搜「fucking shit」跟「wtf」，那些就是漏掉的 test case。**

### Step 6-7：Resolver trigger + eval

**Resolver** 是 agent 的 context 路由表——某種任務類型出現時，就自動載入對應的 skill。Garry 在另一篇 [resolver 專文](https://x.com/garrytan/status/2044479509874020852) 講得更細。每個 skill 都需要在 `AGENTS.md` 裡註冊一行 trigger。

Step 6 擋掉的 bug：skill 寫好了但忘了加 resolver entry。skill 在、capability 在，**但系統找不到它**——就像醫院有外科醫師但沒登進科別名單，比沒這個醫師還糟，因為工程師會**以為**系統能處理。

Step 7 是大多數人完全跳過的一層。resolver trigger 說「這句話應該路由到這個 skill」，resolver eval **驗證它真的路由過去**。Garry 的 eval suite 有 50+ test case：
...
DRY audit 跟它一起跑。**放著不管就會長出 15 個做差不多事情的 skill，resolver 看骰子選一個執行——**修行不夠 robust 的系統都會這樣。

### Step 9-10：E2E smoke test + Brain filing rules

Smoke test 是最後一道防線。問 agent「那趟新加坡 trip 是什麼時候？」，驗證它有跑 `calendar-recall.mjs`、拿到對的答案、格式對。問「下一個會議什麼時候？」驗證它跑 `context-now.mjs` 而不是心算。**前面 8 步全過、系統還是可以失敗**——skill 對、script 對、resolver 對，agent 還是可以決定全部不理直接賭一把。smoke test 就抓這個。

Brain filing rules 是寫進 knowledge base 的 skill 要知道**東西該放哪**。人物進 `people/`、公司進 `companies/`、政策分析進 `civic/`。Garry 發現 13 個寫 brain 的 skill 裡有 **10 個各自 hardcode 了自己的路徑**，完全沒參照 resolver。現在 skill 建檔案前先讀 filing rules doc，自上線以來零錯檔。
