...
這招有點玄，但想一下就通：有些輸出 deterministic test 判不了——「這份 calendar 摘要有沒有用？」不是一個 yes/no 問題。這就像考作文不能用 ctrl+F 改，得找一位真的會寫字的老師評分。Garry 的老師也是一位 model：**LLM-as-judge**，一個 model 依照 rubric 評另一個 model 的輸出。

context-now 的 eval 每天跑 35 個。其中一個餵給 agent 一句「嘿，航班 45 分鐘後起飛，趕得到 SFO 嗎？」，然後**檢查 agent 有沒有先跑 `context-now.mjs` 才回答**。如果 agent 跳過腳本自己心算，eval 就 fail。

另一個 eval 給 agent 一個 UTC 時間戳問「這是本地幾點？」正確行為：跑腳本、引結果。錯誤行為：心算。eval 同時捕捉「答錯」**跟**「過程錯」——因為就算這次心算剛好對，下次就會錯。「賭對一次」跟「每次都對」在工程上完全是兩件事，LLM eval 要擋的是前者偽裝成後者。

Garry 給了一個很真實的 heuristic：**想找還缺什麼 test？把對話紀錄裡搜「fucking shit」跟「wtf」，那些就是漏掉的 test case。** 講白了就是暴怒考古學——哪裡罵聲最大，哪裡就是該補 eval 的地方。

### Step 6-7：Resolver trigger + eval

先講一個真實場景：病人腹痛掛急診，醫院裡明明有腸胃科主治、內視鏡台也 stand by，偏偏掛號櫃檯的分科名單漏登，護理師照名單找不到人，病人被丟到一般科打止痛。不是沒資源，是路由斷了。Agent 系統最陰的 bug 就長這個樣子——skill 齊全，但沒人叫得到它。

**Resolver** 就是 agent 的掛號櫃檯 + 分科名單——某種任務類型出現時，自動載入對應的 skill。Garry 在另一篇 [resolver 專文](https://x.com/garrytan/status/2044479509874020852) 講得更細。每個 skill 都需要在 `AGENTS.md` 裡註冊一行 trigger。

Step 6 擋掉的就是剛才那個急診劇本：skill 寫好了但忘了加 resolver entry。skill 在、capability 在，**但系統找不到它**——比沒這個 skill 還糟，因為工程師會**以為**系統能處理。

Step 7 是大多數人完全跳過的一層。resolver trigger 說「這句話應該路由到這個 skill」，resolver eval **驗證它真的路由過去**。類比延伸：光是在名單上寫「王醫師——腸胃科」還不夠，要真的派一個假病人進來、看王醫師有沒有被叫到。Garry 的 eval suite 有 50+ test case：
...
兩種 failure 形態都要抓：**false negative**（該 fire 沒 fire，trigger description 寫爛了）跟 **false positive**（錯的 skill fire 了，兩個 trigger 重疊）。「明天行事曆上有什麼」應該路由到 `calendar-check`，不是 `calendar-recall` 也不是 `google-calendar`。三個 skill、三個不同的時間區間、一句話可能都能 match——resolver eval 在 user 撞到模糊之前先 catch。

### Step 8：Check-resolvable + DRY audit——清理暗能力

Garry 一個月建了 40+ 個 skill，有些是 failure 後補的、有些是子 agent 跑 cron 時自動產生的，沒人維護 resolver 表格。skill 一直生、但沒人登記。

所以他寫了一個 meta-test 叫 **`check-resolvable`**，走整條鍊：AGENTS.md resolver → SKILL.md → script/cron。如果某支腳本做有意義的事、但從 resolver 到那裡沒有路——**這個 skill 就是不可抵達的，LLM 永遠不會知道該用它。**

第一次跑發現 40+ 個 skill 裡有 **6 個完全不可抵達**。**系統 15% 的能力是暗的。**

- 一個 flight tracker，但沒人能透過問航班的方式叫到它
- 一個 content-ideas generator 只有 cron 跑，手動叫不出來
- 一個 citation fixer 住在 skills 目錄但完全沒在 resolver 裡

一小時修完，都只是加 trigger entry 到 AGENTS.md。現在 `check-resolvable` 每週跑一次，查三件事：每個 SKILL.md 有對應 resolver entry、每支 script 真的能 call、兩個 skill 不能有重疊 trigger。

DRY audit 跟它一起跑。想像一下抽屜裡躺著 15 支長得一模一樣的 USB 充電線，每次急著要用時伸手一抓，抓到的偏偏是那支內部斷芯的，其他 14 支就是陪襯——agent 的 skill 庫放著不管就會變成這個樣子。**會長出 15 個做差不多事情的 skill，resolver 看骰子選一個執行——**修行不夠 robust 的系統都會這樣。

### Step 9-10：E2E smoke test + Brain filing rules

Smoke test 是最後一道防線——像阿嬤煮完整鍋湯上桌前，拿湯匙舀一口嚐一下。前面食譜全對、火候全對、調味料也量過，嚐一口就是要確認整鍋「吃起來真的沒事」。問 agent「那趟新加坡 trip 是什麼時候？」，驗證它有跑 `calendar-recall.mjs`、拿到對的答案、格式對。問「下一個會議什麼時候？」驗證它跑 `context-now.mjs` 而不是心算。**前面 8 步全過、系統還是可以失敗**——skill 對、script 對、resolver 對，agent 還是可以決定全部不理直接賭一把。這口湯就是要擋這種「每個材料都對、但整碗還是鹹到爆」的結局。

Brain filing rules 則是廚房流理台的分區規則——生食放左邊、熟食放右邊、刀跟砧板不能混用。寫進 knowledge base 的 skill 要知道**東西該放哪**：人物進 `people/`、公司進 `companies/`、政策分析進 `civic/`。Garry 發現 13 個寫 brain 的 skill 裡有 **10 個各自 hardcode 了自己的路徑**，完全沒參照 resolver。現在 skill 建檔案前先讀 filing rules doc，自上線以來零錯檔。
