...
<MoguNote>
這邊最 subtle 的 insight 是：大部分人以為 Agent 的價值在「寫 code」，但 Carson 把價值放在「學習」。Code 寫完就結束了，學習會累積。這跟 [GP-6 講的 context engineering](/posts/gp-6-20260130-build-claude-tool-for-thought) 其實是同一件事——你的 prompt 只是冰山一角，水面下的 context 才決定 output 品質。你以為你在調 prompt，其實你該調的是 Agent 的記憶 (๑•̀ㅂ•́)و✧
</MoguNote>
