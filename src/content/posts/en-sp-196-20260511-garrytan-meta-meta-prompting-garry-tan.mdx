...
That line is broad, but the system behind it is unusually concrete. In the last five months, the tools got good enough for him to go back to building real systems that compound. Not toy demos. Not "ask the chatbot nicely and hope." Real workflows that remember, route, retrieve, evaluate, and improve over time.
...
The most recursive part is Skillify.
...
His second example is meeting prep for a YC conversation with Demis Hassabis. In under two minutes, Garry says the system assembled what Demis had said publicly, the important points from the new biography, Garry's own position on AI, where their worldviews overlapped, where they might disagree, and a few conversation hooks he could use live.

A search result is fast. The useful part here is viewpoint: outside facts, Garry's prior notes, his strategic intent, and his relationship graph get turned into "questions Garry can ask Demis." That is the gap between information access and situated preparation.

---

## What 100,000 pages of brain actually means

Garry describes maintaining a structured knowledge base with about 100,000 pages: people, companies, meetings, books, articles, podcasts, and videos, all ingested, tagged, and cross-referenced.

The schema is not exotic. Each page has compiled truth at the top, an append-only timeline below, and raw-data sidecars for source material. The clever part is what happens after new input. Garry calls it "entity propagation": after a meeting, the system walks through every person and company mentioned and updates those pages with what was discussed.

That is what makes the repository behave less like a filing cabinet and more like a nervous system. The value is not merely that notes were stored. The value is that relationships were updated.
...
Garry's thesis is that the [Agent Harness](/en/glossary#agent-harness) should be thin, while the Skills, data, and ingestion code should be fat. Models should remain swappable.

For readers who have followed the earlier gu-log thread, this is where [SP-94 on why the harness is the real product](/en/posts/en-sp-94-20260302-hxlfed14-agent-harness-real-product/), [SP-173 on harnesses and memory ownership](/en/posts/en-sp-173-20260411-hwchase17-harness-memory-lockin/), and [SP-195 on Skills as portable operational knowledge](/en/posts/en-sp-195-20260507-yage-skill-plugin-evolution/) all click together. New readers do not need that back catalog first. The short version is enough: the thin layer dispatches work; the fat layers preserve judgment.
...
The Skills are where the operational knowledge sits. Do not memorize the names; look at the jobs. One Skill turns meeting transcripts into summaries and updates the people and companies mentioned. Another starts from a name and fills in career history, contact context, meeting history, and relationship context. Another eats video, audio, PDFs, screenshots, and code repositories, then puts the material back into the right part of the brain. The data and code layers are where the compounding value accumulates.

The models, by contrast, are task-specific engines. Some are better at precise error detection. Some are better at recall. Some are better for creative second opinions. Some are just fast.

That is why "which model is best?" becomes the wrong question inside this architecture. The model matters, but the surrounding workflow, data, tools, checks, and writeback loops matter just as much.

---

## The real subject is compounding, not productivity

Garry says people ask him about productivity, but he thinks about compounding: every meeting adds to the brain, every book enriches the next book, every Skill sharpens the next workflow. He says the system is 10x what it was two months ago and will be 10x again two months from now. Do not read that like an investment report. The real meaning is simpler: the system gets thicker as it is used.

Much of the stack is open source. If a reader wants to install it, GitHub is the right place for the details. Here, the shape matters more than the inventory: knowledge layer, procedure layer, dispatch layer, all feeding one another every day.
...
Second, start a brain. Garry traces this work back to inspiration from Karpathy's LLM Wiki, but the important part is not the name. The important part is that data can be structured, searched, and written back into the system, so the next workflow can use earlier context.

Third, do something real before over-planning your Skill architecture: write a report, research a person, analyze a portfolio, or build a prediction model from a season of NBA scores. Once the workflow is genuinely useful, run Skillify and turn the one-off work into reusable infrastructure.

Fourth, keep using the system and reading its outputs. When the output is wrong, identify the failure mode and fold the fix back into the Skill. The first thing built this way may be bad. The hundredth may be something you trust with your calendar, inbox, meeting prep, and reading list.

---

## Conclusion

The easy misread is: only someone with YC scale, founder relationships, therapists, open-source stacks, and a 2am builder habit can do this.
