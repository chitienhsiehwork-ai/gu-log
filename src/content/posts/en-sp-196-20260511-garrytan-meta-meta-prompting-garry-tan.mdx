...
Here is the evidence boundary. This rewrite is based on Garry's May 9, 2026 X Article. Numbers such as 100,000 brain pages, 100+ Skills, 15 cron jobs, more than 100 cron jobs per day, 39 installable Skills, 97.6% recall on LongMemEval, and 87,000+ GitHub stars for GStack should be read as Garry's own claims from the source package, not as independently verified measurements.
...
He added a mandatory fact-check step. In the source article, Opus 4.7 1M is used for precision errors, GPT-5.5 for missing context, and DeepSeek V4-Pro for detecting when the writing feels too generic. He also says later iterations added deeper retrieval through GBrain, so section-level mappings cite actual brain pages instead of floating as plausible-sounding abstractions.
...
The book-mirror workflow, in his telling, calls other Skills like `brain-ops`, `enrich`, `cross-modal-eval`, and `pdf-generation`. Each does one narrow thing, but together they compose into a much richer system. Improve one Skill, and every workflow built on it gets better for free.
...
In under two minutes, Garry says the system assembled Demis's accumulated brain page, highlights from Sebastian Mallaby's biography, Demis's published AGI timeline views, research priorities like continual learning and long-term memory, cross-references to Garry's own public positions on AI, demo scripts for multi-hop reasoning, and conversation hooks based on where their worldviews overlap or diverge.
...
The Skills are where the operational knowledge sits. He lists examples like `meeting-ingestion`, `enrich`, `media-ingest`, and `perplexity-research`. The data layer is where the compounding value accumulates: people, meetings, books, articles, ideas, and the links among them. The code layer handles transcription, OCR, archiving, calendar sync, API integrations, and the rest of the ingestion plumbing.

The models, by contrast, are task-specific interchangeable engines. In the source article, Opus 4.7 1M is framed as strong for precision, GPT-5.5 for recall and exhaustive extraction, DeepSeek V4-Pro for creative work and third perspectives, and Groq plus Llama for speed.
...
Every meeting adds to the brain. Every book enriches the next book. Every Skill sharpens the next workflow. Every updated person page improves the next meeting prep. He says the system is 10x what it was two months ago and will be 10x again two months from now. That should be read as subjective description, not audited measurement, but the direction is clear: the system gets thicker as it is used.

This is also where the article contains one of its own visible tensions. It says he runs more than 100 crons per day, then later describes the second brain as a running system with 100,000 pages, 100+ Skills, and 15 cron jobs. The responsible reading is not to smooth over the discrepancy. It is to mark both as source-level self-report and keep the structural lesson separate from the precise counts.

He also says the whole stack is open source: GStack as the coding Skill framework, GBrain as the knowledge layer, OpenClaw and Hermes Agent as harnesses, and the data repos on GitHub. Again, those are source claims, but they are enough to make the deeper point visible: the strategic moat is not one amazing prompt. It is a living personal context system.
...
First, pick a harness: OpenClaw, Hermes Agent, or build your own. Keep it thin.

Second, start a brain with GBrain. He traces that work back to inspiration from Karpathy's LLM Wiki, later extended into GBrain. The source article also claims 97.6% recall on LongMemEval, no LLM in the retrieval loop while still beating MemPalace, and 39 installable Skills. Those claims should be carried as claims, not upgraded into independent proof.
