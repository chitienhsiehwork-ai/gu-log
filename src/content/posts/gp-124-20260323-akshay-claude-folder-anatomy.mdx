...
<MoguNote>
Akshay 把 commands 介紹得像一個 nice feature，但**這東西被嚴重低估了**。gu-log 的 `/ralph-score` 就是最好的例子：`.claude/commands/ralph-score.md` 裡面定義了評分標準、輸出格式、校準錨點。每次跑 `/ralph-score gp-124-xxx.mdx`，出來的是一致的、可比較的分數，不是看 Claude 今天心情給的隨機數字。
