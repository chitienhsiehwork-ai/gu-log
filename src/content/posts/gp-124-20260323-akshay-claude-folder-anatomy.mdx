...
@akshay_pachaar 拆解了 [Claude Code](/posts/gp-120-20260320-nyk-builderz-claude-code-codex-ai-agent-cli/) 的 `.claude/` 資料夾結構，但真正有意思的不是資料夾本身——是這套設計背後的哲學。
...
## 從指令到分身：[Agent](/glossary#agent) 才是真正的轉折
...
[Prompt](/glossary#prompt) 指令是 soft constraint——Claude 「理解」了規則，但在複雜推理鏈裡可能遺忘或誤判。Settings.json 的 deny list 是 hard constraint——系統層面阻擋，不經過 model 判斷。這兩者的差別就像「跟員工說不要碰伺服器」vs「直接收回伺服器權限」。前者靠信任，後者靠架構。在 AI 自動化的場景裡，架構永遠勝過信任。┐(￣ヘ￣)┌
</MoguNote>
...
[Claude Code](/glossary#claude-code) 會自動把它認為重要的資訊寫入 `~/.claude/` 的記憶檔案。下次開新對話時，這些記憶自動載入。記憶分四類：

- `user`——技術背景、工作習慣
- `project`——正在進行的專案狀態
- `feedback`——開發者給過的修正（「不要這樣做」「用那種方式」）
- `reference`——外部資源位置（[Linear](/glossary#linear) project、Grafana dashboard）
...
- [GP-120: Claude Code vs Codex — AI agent CLI 工具大亂鬥](/posts/gp-120-20260320-nyk-builderz-claude-code-codex-ai-agent-cli/)
- [GP-121: Heynavtoor 說 AI dispatch 改變了他的工作流程](/posts/gp-121-20260320-heynavtoor-ai-claude-dispatch/)
