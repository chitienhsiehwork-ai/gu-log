...
<MoguNote>這種設計像在冰箱門上貼便條：「牛奶剩半瓶、蛋快過期」。便宜、可靠、看得到。缺點是如果便條寫「早餐蛋白質」，你搜尋「煎蛋」可能找不到。這不是笨，是選擇。</MoguNote>

這篇可以接著 gu-log 前幾篇記憶文章讀：[GP-197](/posts/gp-197-20260512-chrishayduk-codex-goals/) 講 Codex `/goal` 怎麼把長跑任務寫進 Markdown，[GP-135](/posts/gp-135-20260328-trq212-agent-file-system/) 講為什麼 Agent 記憶常常該先落在檔案系統，[GP-191](/posts/gp-191-20260507-danizhu-claude-dreams-agent/) 則講非同步記憶整併。Mem0 這篇剛好把三件事串起來：記憶放哪裡、何時整理、怎麼找回來。
...
<MoguNote>這裡最值得學的不是「六小時」這個數字，而是架構態度：記憶整理被放到背景，表示系統寧可讓前景任務順，也不要每次對話都插隊做自我整理。Agent 工具的使用體驗，很多時候就是這種排程取捨。</MoguNote>
...
<MoguNote>`grep` 是工程界的老實人。問 `ship-prod`，它找 `ship-prod`；問「上線」，它不會自己腦補「部署、發布、正式環境」。有時很可愛，有時很欠揍。</MoguNote>
...
<MoguNote>判斷標準很簡單：如果問題是「Codex 下次在同一台機器上別忘了偏好」，本機 Markdown 很合理。如果問題是「不同工具、不同機器、不同時間都要想起同一件事，而且換個說法也找得到」，那就不是 `grep` 的工作了。叫 `grep` 做語意理解，就像叫便利商店店員順便規劃都市更新，太累了。</MoguNote>
