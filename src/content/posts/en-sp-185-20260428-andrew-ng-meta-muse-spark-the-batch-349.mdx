...
The Batch 349 puts two very different AI stories in the same issue.
...
The first is about how small software teams work with models. The second is about how a large model lab releases and packages a model. Read side by side, they show two layers: the people using models, and the companies building them.
...
## Same Issue, Different Source Layers

Andrew Ng is writing about small teams and generalists: engineers closer to product decisions, PMs learning to build, teams kept between 2 and 10 people, and same-room work as the highest-speed setup. That continues a thread he's been pulling since [Batch 340's Hollywood letter](/en/posts/en-sp-59-20260214-batch340-andrewng-hollywood/) through [his agent session take](/posts/cp-199-20260323-andrewyng-agent-andrew-ng-session/).

Meta's piece is model-release coverage: restructured AI labs, a Scale AI stake, more than 1,000 physicians helping with health data, closed Muse Spark, and API preview to selected partners. That describes model labs, product lines, and platform access, not small-team role design.

At the source level, the two pieces sit in different layers:
...
Ng also caveated the scope: his small-team thesis applies to 2-to-10 people, and he'll write about large-team coordination later. Muse Spark has its own boundary: parameter count, architecture, training data, and output length cap are undisclosed, and the API is not broadly open yet.
...
That split preserves the source boundary. The two stories can be compared, but they should not be written as one organizational theory already stated by the source.
...
## Closing: One Issue, Five Scenes

Ng's AI-native team view and Meta's Muse Spark land in the same issue. Neither piece says they're related; the issue also includes Eli Lilly, state AI laws, and Persona Generators.

The five scenes have different keywords:

Ng is role redistribution inside small teams. Meta is closed models, token efficiency, and API preview. Insilico is faster drug design plus clinical limits. State law is compliance patchwork. Persona Generators is synthetic-user coverage. They do not cite one another or announce one shared theory; together, they spread AI's impact across development, model labs, pharma, compliance, and user research.
...
It stands out after the full set of stories because the same issue also puts Meta, Eli Lilly, U.S. state governments, and Google on display, while Ng saves "the golden age" frame for individuals and small teams.
