...
Three more scenes hit a similar bottleneck shift: Eli Lilly's $2.75 billion bet on Insilico Medicine for AI drug discovery; U.S. states pushing a patchwork of AI laws; and Google's Persona Generators research that uses an evolutionary algorithm to create twenty-five synthetic users. They are not one case, but they point at the same question: once AI speeds up one part of a workflow, where does the next bottleneck appear?
...
"Agentic coding isn't just changing the workflow of software engineering — it's also changing the workflow of every team around it." That sentence expands the scope from engineering to the whole product path. This is not only "engineers got stronger"; it is **the entire critical path from idea to launch redistributing itself**. The old bottleneck (writing code) gets unblocked, and the water flows to the next low point — it doesn't disappear.
...
**Ng's view is a playbook for "teams using AI"** — followable, immediately useful, adjustable today.
...
## Three More Stories Riding Along: Bottlenecks Move Downstream

The next three pieces are not here to turn the issue into a full recap. They matter because each one shows the same pattern: AI speeds up one part of the pipeline, and the bottleneck moves downstream.
...
The point here isn't naive optimism that "AI will discover new drugs." It is that **Ng's bottleneck observation also maps onto pharma** — when a model compresses "find the target, design the molecule" from years to months, the bottleneck shifts immediately to clinical trials and FDA review. Those paths run on real human time, recruiting actual patients — not something a model can speed up in the same way. AI widens the top of the funnel, and the narrowness of the middle and bottom becomes more visible.
...
Put next to Ng's bottleneck observation, the connection is simple: if a coding agent lets a team build a feature in one day, legal might still need a week to review it. If AI law becomes a 50-state patchwork, that week can turn into a checklist with 50 versions. Product work got faster; compliance did not become one clean highway.
...
The research connects back to the product-management bottleneck: when an LLM makes it easy to build, deciding what to build gets harder, and synthetic personas might become one way through that problem.

Translated: **when shipping gets cheap and deciding what to ship gets expensive, making "diverse user feedback" cheap is another exit**. Ng's solution is to let engineers also be PMs; Google's research direction uses 25 synthetic personas to expand early user-research bandwidth. That cannot directly replace real user interviews, but it can widen the range of questions before enough real users are available.
...
After reading the five scenes, the line that stays isn't loose framing like "AI is changing everything." It's this:

**Over the past decade, the path from "had an idea" to "made it" got cleared. But the path of "what should be next, can this ship, and who carries the risk" got jammed**. Andrew Ng says small teams should let engineers think for themselves. Meta Muse Spark uses parallel agents to think for the decision-maker. Insilico uses PandaOmics to think for medical researchers. U.S. state laws remind developers that deployment is not over when they press deploy. Google's Persona Generators thinks through 25 different user reactions for the PM. **All of them are working on the same problem — when "doing" got cheap, "deciding what to do" and "being responsible for it" got expensive**.

For product teams, that makes the sharper question: should the next feature exist, what should it look like, and who is responsible once it ships? That is the more expensive work now.
...
Two signals on the same table. The real question is not which side is correct; it is which capability developers should bet time on next.
