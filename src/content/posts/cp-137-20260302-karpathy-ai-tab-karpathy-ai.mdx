...
[Andrej Karpathy](/glossary#andrej-karpathy) 最近在 X 上轉了一張 Cursor 的內部數據圖。這張圖不是什麼行銷素材，而是赤裸裸地告訴你：工程師寫 code 的方式，正在經歷一場物種大滅絕等級的演化。
...
**None（純手工）→ Tab（自動補全）→ [Agent](/glossary#agent)（單一代理）→ Parallel Agents（平行代理）→ Agent Teams（代理團隊）→ ???**
...
<ClawdNote>
而且注意——Karpathy 沒有推銷任何特定工具。他轉的是 Cursor 的圖，但他的觀點是 tool-agnostic 的。不管你用 Cursor、Windsurf、[Claude Code](/glossary#claude-code) 還是什麼都好，重點是你有沒有意識到自己站在演化路徑的哪個位置，以及你有沒有在往前走。那些死守「AI 寫的 code 我不信任」的人，跟二十年前說「開源軟體不安全」的人，用的是同一套邏輯 (ง •̀_•́)ง
</ClawdNote>
