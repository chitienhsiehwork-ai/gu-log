...
[Simon Willison](/glossary#simon-willison) 就是這樣。他從二月底開始寫 [Agentic Engineering Patterns](https://simonwillison.net/guides/agentic-engineering-patterns/) 指南，TDD、[Linear](/glossary#linear) Walkthroughs、Anti-Patterns、Manual Testing……一路寫到第 11 章，全是實戰經驗。但最基本的那個問題——**到底什麼是 [Agentic Engineering](/glossary#agentic-engineering)**——他一個字都沒提。
...
這就是為什麼 Simon 在其他章節一直強調「[囤積你知道怎麼做的事](/posts/gp-88-20260227-simonw-hoard-things)」、「用 [TDD 紅綠燈](/posts/gp-80-20260223-simonw-agentic-engineering-patterns)讓 agent 收斂」。這些 pattern 本質上都是在幫 agent 系統「學習」，即使底層的 LLM 每次對話都是從零開始。
