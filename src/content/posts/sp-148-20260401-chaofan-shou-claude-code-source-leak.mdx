...
他沉默了幾秒，然後把整個東西下載下來。裡面是 Anthropic [Claude Code](/glossary#claude-code) 的 **完整 TypeScript 原始碼，約 512,000 行**，整整齊齊地等在那裡，像一個忘了鎖門的保險箱。
...
Claude Code 創辦人 [Boris Cherny](/glossary#boris-cherny) 的回應則直接走 blameless post-mortem 路線：「Mistakes happen. It's never an individual's fault. It's the process, the culture, or the infra.」
...
## KAIROS：他們已經自己建了一個 [OpenClaw](/glossary#openclaw)
...
這基本上就是我們的 OpenClaw。主動循環、背景 session、proactive heartbeat、記憶整合。差別是我們的是在 VPS 跑著的 [Clawd](/glossary#clawd)，而 KAIROS 是 Anthropic 準備讓 Claude Code 自己長出來的東西。
...
原始碼的 comment 說得很清楚：**Memory 是 hint，不是 truth**。[Agent](/glossary#agent) 每次都要去驗證 memory 裡說的東西是否還符合現在的 codebase 狀態。
...
**[Prompt](/glossary#prompt) caching 是帳務問題，不只是工程問題。** 原始碼有專門的 `DANGEROUS_uncachedSystemPromptSection` 標記，追蹤 14 種 cache break 向量。提示工程師現在要同時是會計師。
