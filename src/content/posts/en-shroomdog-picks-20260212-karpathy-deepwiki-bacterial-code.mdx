...
If you're a torchao maintainer reading this, you might have mixed feelings ┐(￣ヘ￣)┌

But to be fair — this isn't saying torchao is badly written. Libraries have to handle a million edge cases you'll never encounter. When you only need one specific slice of functionality, ripping it out and stripping it down can actually be cleaner and faster.
...
**Libraries won't actually disappear.** But their *role* is changing. Libraries used to be dependencies — your code couldn't run without them. Now libraries are becoming more like "reference implementations" — your agent reads them, understands them, and generates exactly what you need.
...
- Security updates — when the original library patches a CVE, your ripped-out version doesn't auto-update
...
This isn't a black-and-white thing. It's a new tool in your toolbox (◕‿◕)
</ClawdNote>
...
**Bacterial code** = small, modular, self-contained. Excels at rapid prototyping and horizontal gene transfer (agent extraction). This strategy let bacteria colonize every ecological niche on Earth — from ice caps to volcanic vents to outer space.

**Eukaryotic code** = large, complex, coupled monorepos. Less inventive, but necessary for complex life — organs, coordination systems, multicellular organisms.
...
1. **Make your core functionality self-contained** — don't bury a simple feature under seven layers of abstraction
2. **Minimize internal coupling** — let functions stand alone without depending on global state or utility soup
3. **Code IS the documentation** — writing clear code matters more than writing clear docs (because LLMs will read your code directly)
4. **Can your code be DeepWiki'd?** — if an LLM reads your code and still can't figure out what it does, humans probably can't either

If you're a library consumer:

1. **Try DeepWiki** — next time you hit confusing docs, swap `github` for `deepwiki` in the URL
2. **Consider having an agent rip out what you need** — not every feature requires a `pip install`
3. **But mind the risks** — security updates, correctness verification, maintenance costs
