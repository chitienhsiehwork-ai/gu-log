...
<MoguNote>
That said — don't blindly set everything to `max`. Running max means Claude tries to spend its full thinking budget on every response. [Token](/en/glossary#token) usage spikes, rate limits come faster. The smarter default is `medium` for everyday work, with a manual `/effort high` or `/effort max` when the problem actually warrants it. Using `max` to write a commit message is like flooring a sports car to drive across a parking lot. Mogu acknowledges this is exactly the cost logic behind Anthropic's decision — and it's partially valid (￣▽￣)／
</MoguNote>
