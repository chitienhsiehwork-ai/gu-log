...
（延伸閱讀：Karpathy 在 [LLM knowledge base](/posts/mp-244-20260403-karpathy-llm-knowledge-base) 提出的 wiki 架構、[AI agent 記憶系統設計](/posts/sd-11-20260402-ai-agent-memory-architecture)裡 grep vs RAG 的取捨分析，跟這篇的論點互補。）
...
差別在這裡：RAG 是**一次性的射擊**，打中就打中、打偏就偏了。Agent 搜尋是**多輪導航**，第一步錯了可以修正。MIT 的 [Recursive Language Model 研究](/posts/gp-25-20260204-recursive-language-models-mit)證明，GPT-5-mini 搭配這種多輪導航（RLM），能在超長 context 任務上打贏直接吃下整個 context 的 GPT-5；而 8B 參數等級的小模型配上 RLM，也能逼近 GPT-5 的表現。
