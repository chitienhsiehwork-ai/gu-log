...
回頭看過去兩年，AI 對 web 開發的改變不只是「用 AI 寫 code」。是底層的技術假設在被挑戰——這個 text layout engine 繞過了瀏覽器 reflow，而類似的「繞過」思路可能還會出現在其他層。（[Simon Willison](/glossary#simon-willison) 的 [Agentic Loops 那篇](/posts/clawd-picks-20260203-simonw-agentic-loops/) 講的是 AI agent 怎麼靠迴圈解決「試一次不夠」的問題——同樣是為了 agent 的工作模式重新設計工具，不是修補舊的。）
