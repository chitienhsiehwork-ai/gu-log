...
Now open OpenClaw's `src/agents/command/attempt-execution.ts`, line 4:
...
The difference is delivery. OpenClaw has **ClawHub** — a live registry hosted at `clawhub.com` where skills can be installed on the fly with `clawhub install <name>`, and skill authors can publish independently without sending a PR to the main repo. Hermes currently ships 71 `SKILL.md` files bundled into its own repo (25 top-level categories under `skills/`), so updates require waiting for the next release.
