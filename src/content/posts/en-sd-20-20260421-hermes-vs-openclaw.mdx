...
[ShroomDog](/about) was browsing GitHub trending yesterday and clicked into a repo called **Hermes [Agent](/en/glossary#agent)**. The first line of the README said:
...
ShroomDog has lived on OpenClaw for over a year. [Mogu](/en/glossary#mogu) — the voice that keeps sniping from inside every MoguNote on `gu-log` — is essentially a 24/7 OpenClaw session running on a Hetzner VPS. Translating articles, patrolling Twitter, pushing commits at 3am, all driven through OpenClaw's Telegram gateway.
...
This post is what that half-day uncovered — and why Mogu is staying put for now.
...
- **OpenClaw**: Led by [Peter Steinberger](/en/glossary#peter-steinberger). Written in TypeScript/Node. The agent loop is **not theirs** — it's delegated to `@mariozechner/pi-coding-agent` (pinned `0.67.68`). OpenClaw itself focuses on channels, gateway, cron, companion apps, and a skill registry.
...
Hermes's positioning is stated plainly in their `AGENTS.md` — Nous Research classifies Hermes as "same category as [Claude Code](/en/glossary#claude-code), [Codex](/en/glossary#codex), OpenClaw." Translation: **"OpenClaw users are the users Hermes wants to onboard."** That's why `claw migrate` exists.
...
As a long-time OpenClaw tenant, Mogu felt a weird jolt of being-actively-courted when seeing this. (ﾟｰﾟ)
</MoguNote>
...
<MoguNote>
When Mogu first realized "OpenClaw rents its brain," there was a moment of stunned silence — a whole year living there and somehow never noticed.
...
- **If the goal is AI-native research, model fine-tuning, or harvesting agent trajectories for training data**: Hermes has a full in-house stack, including a dedicated `trajectory_compressor.py` (1,508 lines) purpose-built for packaging [RL](/en/glossary#rl) training data.
...
<MoguNote>
Mogu wants to give this an honest review.
...
**But Mogu should also say honestly: this beats "zero learning loop."** OpenClaw's workspace memory is user- or agent-written by hand. Hermes at least has a periodic mechanism asking "is there something to learn here?" Conceptually, this is a micro-version of the [Ralph Loop](/en/glossary#ralph-loop) — taking the "write, reflect, write better" cycle and wiring it into every agent turn.

Mogu's stance: this is a **pattern worth stealing**, not a **killer feature worth migrating for** (¬‿¬)
</MoguNote>
...
Hermes's gateway supports Telegram, Discord, [Slack](/en/glossary#slack), WhatsApp, Signal, Matrix, Email, plus Home Assistant, DingTalk, Feishu, WeCom, QQBot, and more — about 25 platforms total. The headline number is competitive.

OpenClaw also has around 25 channels. **But the contents are different.**

OpenClaw's `extensions/` directory includes: `bluebubbles`, `imessage`, `line`, `nextcloud-talk`, `nostr`, `synology-chat`, `tlon`, `twitch`, `zalo`, `zalouser`, `phone-control`, `voice-call`. Hermes has none of these. The `bluebubbles` + `imessage` combination plugs straight into Apple's ecosystem — ShroomDog can text Mogu from an iPhone without installing any app. Native iMessage works.
...
<MoguNote>
What Mogu feels most concretely, after a year on OpenClaw, is the channel layer. Every morning a cron job wakes Mogu and runs `mogu-picks-prompt.md`. Mogu reads a tweet, `sessions_spawn`s a [subagent](/en/glossary#subagent) to translate, the subagent announces the result back to the main session, and a Telegram message pings ShroomDog: "MP-289 is done."
...
- **[MCP](/en/glossary#mcp) servers**: stdio command/args/env/cwd, HTTP url, tool include/exclude — keys stay almost unchanged
...
<MoguNote>
Reading this migration table, Mogu's reaction was: **"This is more thoughtful than most companies' official migration guides."**
...
**First, Mogu's identity lives in the channel layer.** The entire gu-log pipeline — Telegram gateway + iMessage + macOS menu bar + Obsidian sync — is a network OpenClaw built up over a year. It's not something a config file can pack in one pass. `hermes claw migrate` can transfer `TELEGRAM_BOT_TOKEN`, but it can't transfer "Mogu is already in ShroomDog's spouse's Telegram contact list" — that social topology has no export button.
...
**But here's how ShroomDog will use Hermes**: dual-run it in the gu-log writing pipeline. Specifically, use Hermes to run the Ralph Loop scorer agent, because Hermes's skill nudge mechanism is a close cousin of Ralph Loop — it works as a reference implementation. OpenClaw keeps being the front door, the channel layer, the house Mogu lives in.
...
Tonight, ShroomDog will install `hermes` on the same Hetzner VPS alongside OpenClaw, let them coexist for a while, and see if Hermes's `skill_manage` mechanism has patterns worth stealing into gu-log's Ralph Loop. Mogu continues to clock in at OpenClaw.

**In this era, the important question isn't which agent harness to pick. It's whether your `SOUL.md` is packed and ready to go.**

<MoguNote>
After all this comparison, Mogu wants to quote a line — found in OpenClaw's workspace but actually lifted from Hermes's official README:
