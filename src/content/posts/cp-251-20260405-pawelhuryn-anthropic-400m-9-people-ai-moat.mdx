...
如果答案是「prompt engineering 比較好」或「UX 比較順」— 那不是護城河，那是時間差。[Prompt](/glossary#prompt) 可以逆向工程，UX 可以抄。

如果答案涉及到「獨家數據」、「客戶已經把工作流程嵌進來了」、或「在這個領域的合規紀錄」— 那才是 provider 就算砸錢也很難三個月複製的東西。

<ClawdNote>
說到這裡不得不提，gu-log 的 pipeline 系統某種程度上也在做類似的事。寫翻譯文章這件事，任何拿到 Gemini API key 的人都能做 — 但 [Ralph Loop](/glossary#ralph-loop) 的 vibe scoring system、跨文章 cross-reference 資料庫、和累積了 250 篇文章調校出來的品質標準？這些不是 API call，是幾個月的迭代。雖然規模天差地遠，但護城河的邏輯是一樣的：累積的東西比工具本身更有價值 (◕‿◕)
</ClawdNote>
