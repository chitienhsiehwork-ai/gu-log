...
Forget the tool names for a moment. When a team puts Claude Code inside a large codebase, it is really answering three questions: which map should it read when it enters? Which repeated actions should machines run directly? Which specialized tools should only come out when needed?

The first layer is the map. CLAUDE.md is not an encyclopedia. It is the floor plan by the entrance that says "do not go here": global context and major traps at the repository root, local conventions in subdirectories, such as test commands, naming rules, and historical baggage. These files load automatically at the beginning of a session. Once they get fat, the whole session slows down.

The second layer is automation. Repeatable, verifiable actions should not depend on the model remembering them. [Hooks](/en/glossary#hooks) can block unsafe operations, load module settings at session start, and suggest CLAUDE.md updates at session end. Formatting, linting, and tests are deterministic rules; run them through hooks instead of praying the model remembers.

The third layer is on-demand loading. In a large codebase, not every session needs security review, documentation updates, deployment flow, or data-processing rules. The value of [Skills](/en/glossary#skill) is progressive disclosure: load the security process only for security work, load the docs workflow only when editing docs, and bind a payments deployment [Skill](/en/glossary#skill) so it only activates inside the payments directory.
...
## Put the Signs Where the Agent Will Use Them
...
CLAUDE.md should be thin and layered, and test or lint commands should be layered too. When Claude Code starts in a subdirectory, it reads the CLAUDE.md files along the path upward, so the root only needs the highest-level information and critical traps. Each service's build, test, and lint commands should live near that service. Running the whole company's test suite after changing one service is like watering a tiny succulent on your desk with a fire truck: impressive pressure, dead plant.

Generated files, build artifacts, and third-party code should be excluded. Commit `permissions.deny` rules to `.claude/settings.json`, and the team can share the same noise-reduction setup. If some engineers are developing a code generator and generated files are the work target, local settings can override the project-level rules without turning the exception into company-wide noise.

Some legacy systems have directory structures that provide no help at all. In that case, a lightweight repository map is enough: a root-level markdown file listing the top-level folders, with one sentence explaining what each contains. If there are too many top-level folders, make the map hierarchical. Hundreds of thousands of folders, millions of files, non-Git version control, game engines with huge binary assets, and workflows where non-engineers also commit code still require extra judgment. This is not the fairy tale where "AI coding solved every enterprise problem." It is about paving the road for most traditional software engineering environments first.
...
A good deployment usually looks like an organized construction site: a thin map at the entrance, warning signs around dangerous areas, machines handling fixed checks, special tools pulled out only when needed, formal entrances into internal systems, and large scouting tasks delegated before the main work begins.
