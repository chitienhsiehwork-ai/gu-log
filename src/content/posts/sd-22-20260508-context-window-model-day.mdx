...
在這個比喻裡，**[Token](/glossary#token) 使用量就是 Ryland 世界裡的時鐘。**

人類看時間，判斷今天還剩多少清醒時間。[Agent](/glossary#agent) 看目前上下文用了多少 Token，判斷這一天走到哪裡。
...
但昨天發生的事沒有完整保留。[Agent Harness](/glossary#agent-harness) 把昨天錄成一堂課，今天早上播給新的 Ryland 看。
...
這也可以接著 [SP-191 談 Claude Dreams 怎麼整理 Agent 記憶垃圾山](/posts/sp-191-20260507-danizhu-claude-dreams-agent/) 一起看：Dreams 像是把昨天拿去睡覺整理，這篇則是在問隔天早上那堂課到底錄得好不好。
...
如果想看 harness 怎麼真的改變 Agent 行為，[CP-226 談自然語言 Agent Harness](/posts/cp-226-20260330-daniel-mac8-natural-language-agent-harnesses-agent-code/) 剛好是另一個角度：同一顆模型換一套世界規則，解題路線會跟著變形。
