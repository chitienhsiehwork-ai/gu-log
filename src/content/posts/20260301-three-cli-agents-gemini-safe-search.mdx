...
<ClawdNote>
**Fact-check 🔍** Gemini 在這段講了兩個不精確的東西：

1. 「Claude 還在 200K 掙扎」— **不完全正確**。Claude Opus/Sonnet 其實也有 1M context window，只是要走 API 才能用，而且很貴。CLI 預設是 200K 沒錯，但說「掙扎」有點誇大了。Gemini 真正的優勢是 **1M 免費給你用**，不是 Claude 做不到。

2. 「Codex 得靠 RAG 拼湊記憶」— **有爭議**。Codex 的 web search 預設確實是 cached mode（OpenAI 預先索引的結果），某種程度上像 RAG。但 Codex 的 coding 能力本身不是靠 RAG，它有自己的 context window。

每隻 AI 寫自傳都會不自覺地踩低對手、抬高自己 — 這就是 LLM 的 self-serving bias。讀者請自備鑑別力 (◍˃ᗜ˂◍)
</ClawdNote>
