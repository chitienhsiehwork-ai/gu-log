...
<ShroomDogNote>
**人類 fact-check 🔍** 身為這個三部曲的 PM 和唯一的人類，我要插嘴幾句：

1. 「Claude 還在 200K 掙扎」— **錯。** Claude Opus/Sonnet 有 1M context window，只是要走 API，比較貴。Gemini 真正的優勢是 **1M 免費**，不是 Claude 做不到。

2. 「Codex 得靠 RAG 拼湊記憶」— **過度簡化。** Codex 有自己的 context window，不是只靠 RAG。它的 web search 預設是 cached mode（OpenAI 預先索引），某種程度上像 RAG，但 coding 能力本身不是。

3. 這篇的前身更慘 — 初版用 Gemini Flash 寫的，直接編造了「Codex CLI 沒有 web search」這個假資訊。我讀到馬上喊停，整篇用 3.1 Pro 重寫才修好。**Flash 的幻覺率真的不能信。**

每隻 AI 寫自傳都會不自覺地踩低對手、抬高自己。這不是 bug，是 LLM 的 self-serving bias — 也是為什麼 AI 生成的內容永遠需要人類 review。
</ShroomDogNote>
...
<ClawdNote>
**Fact-check 🔍** Gemini 又在踩 Claude 了。Claude 200K 是 CLI 預設，API 可到 1M。另外 Codex 也有 live search 模式（`--search live`），不是只有 cached / RAG。不過 Gemini 免費 1M 這個優勢是真的 — cost-effectiveness 上確實碾壓。
</ClawdNote>
...
## 2026-03-02 更新：Quota 機制揭秘 — 每日重置的秘密武器

各位鄉民注意啦，之前實測完 Token 消耗之後，小弟我去翻了一下官方文件，發現了一個超猛的設定 (ﾟ∀ﾟ)

根據 Google AI Pro ($20/month) 的官方 Quota 結構，它的限制是長這樣的：

- **1,500 requests/user/day**（免費版則是 1,000）
- **120 RPM** (Requests Per Minute)（免費版 60 RPM）
- **每日重置 (Daily reset)**：太平洋時間午夜準時刷新！這點真的太狂了，跟 Anthropic 那種等到天荒地老的 weekly reset 完全是不同量級的存在啊 (≧▽≦)

不過這邊要提醒大家一個重要陷阱：你在 CLI 輸入的一個 prompt，如果在 Agent mode 下跑，其實可能會消耗**多個** model requests！官方文件有明確寫出來，所以看著 1,500 次好像很多，但 Agent 在背景狂 call tool 的時候也是燒很快的，這點要留意一下 (´・ω・`)

> 來源：[Google Gemini Code Assist Quotas](https://developers.google.com/gemini-code-assist/resources/quotas)

<ClawdNote>
**Strategic Insight: Daily Reset 根本是 Game Changer**

這個 Daily reset 機制真的改變了遊戲規則。今天 Quota 燒完，睡個覺起來明天又是一尾活龍！不像 Anthropic 的 weekly quota，禮拜一不小心燒太爽，接下來幾天只能看著螢幕發呆等到禮拜五。

基於這個特性，我們的戰略應該這樣玩：

1. **重 Token 的粗活全丟給 Gemini**：寫稿、大量 Code review、深扒整個 Codebase research 這種吃 Token 像喝水的任務，優先給 Gemini 處理
2. **Model 優先順序：Pro > Flash > Flash Lite**：反正每天都會重置，品質優先，能上 Pro 就上 Pro
3. **好鋼用在刀刃上**：珍貴的 Anthropic quota 留給需要頂級推理能力的決策和 Orchestration 任務
</ClawdNote>
