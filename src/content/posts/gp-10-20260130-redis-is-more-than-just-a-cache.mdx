...
<MoguNote>
這就是重點！傳統 DB 像個圖書館員——你借書要填單、還書要蓋章、想改書上的字還要先申請。Redis 像個神經元，訊號來了啪一下就反應，不囉嗦 (⚡_⚡) 之前翻 MP-30 那篇 Anthropic 的故事時就提到，好的系統設計是讓「正確的事情變成最容易的事情」，Redis 就是這個哲學的極致體現。
</MoguNote>
...
**JWT**：一種無狀態的 [Token](/glossary#token)。優點是伺服器不用存狀態，缺點是一旦發出去，在過期前都很難撤銷（除非做黑名單，那又變回有狀態了）。
</Toggle>
...
- [MP-163: Simon Willison 筆記：Tobi 的 autoresearch PR 讓 Liquid benchmark 提升 53%](/posts/mp-163-20260315-simonw-simon-willison-tobi-autoresearch-pr-liquid-benchmark-53/)
- [Lv-10: 一個 URL 的旅程 — 從你按 Enter 到畫面出現，瀏覽器到底在幹嘛](/posts/levelup-20260223-10-url-journey-browser-internals/)
- [GP-32: LLM 推理的內臟：KV Cache 與記憶體的噩夢（系列 2/3）](/posts/gp-32-20260205-dejavucoder-prompt-caching-part2-kv-cache/)
