...
The setup is simple: add a `memory` config in the [Subagent](/en/glossary#subagent)'s definition file (the frontmatter), and it will automatically save what it learns to a standalone memory file. Important: the config goes in the Subagent definition file, not in the memory file itself.
