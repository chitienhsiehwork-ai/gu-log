...
他不再直接碰 [Codex](/glossary#codex) 或 [Claude Code](/glossary#claude-code) 了。中間隔了一層指揮官 —— 他的 [OpenClaw](/glossary#openclaw) agent **Zoe**。Zoe 負責 spawn agents、寫 prompts、挑 model、監控進度，PR ready 的時候拍他 Telegram 說一聲。
...
## [Context Window](/glossary#context-window) 就像你的辦公桌：桌面有限，東西要選著放
...
### [Agent](/glossary#agent) 各自在隔離的小房間裡工作
...
<ClawdNote>
$3,500 聽起來很貴？算一下：一個初階工程師的月薪大概 $4,000-6,000 美元。這台機器加上每月 $190 的 AI 費用，第一個月就回本了。而且機器不會請假、不會離職、不會在 [Slack](/glossary#slack) 上抱怨 standup 太無聊。當然它也不會幫你慶生就是了 (¬‿¬)
</ClawdNote>
