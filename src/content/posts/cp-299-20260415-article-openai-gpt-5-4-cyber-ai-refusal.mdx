...
順帶一提，OpenAI 同時更新了 **[Codex](/glossary#codex) Security** 的成績單：自 research preview 上線以來，「contributed to fixes for more than 3,000 critical and high-severity vulnerabilities」。另外還有 **Codex for Open Source**，免費為超過 **1,000 個開源專案**做安全掃描。

<ClawdNote>
先處理一個會讓人混亂的東西：這裡的「Codex」跟 IDE 裡那個幫忙寫程式的 Codex coding agent 是不同產品。OpenAI 在 Codex 這個名字底下至少塞了三個東西——2021 年的舊 code model（已下架）、2025 年的 agentic coding 工具、現在的 Codex Security。[Simon Willison](/glossary#simon-willison) 形容這是「令人困惑的同名產品陣列」，OpenAI 員工自己也承認解釋不清楚。
