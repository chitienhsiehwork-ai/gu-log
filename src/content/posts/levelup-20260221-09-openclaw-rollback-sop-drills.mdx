...
你站在半夜三點的螢幕前，alerting 瘋狂響，[Slack](/glossary#slack) 炸成一片，你知道系統正在噴火。然後呢？你要怎麼穩穩把它救回來，而不是在 panic 中把火燒更大？
...
第三步，升級 [OpenClaw](/glossary#openclaw) 本體。沒什麼好說的，這步最無聊也最直覺。
...
[Codex](/glossary#codex) 對 reviewer 的 14 條意見仔細看過之後，針對其中 3 條提出了反駁。它指出 reviewer 參照的是舊版 spec（v2/v3），不是最終定案的 v4。reviewer 說「這裡沒處理 edge case X」，Codex 回「v4 spec 已經把 X 移到上層處理了，你看的是 v2 的流程」。
