...
有一件事在 AI 圈正在悄悄發生，而且詭異到讓人頭皮發麻：一個用純粹 reinforcement learning（[RL](/glossary#rl)）訓練出來的模型，**沒有人教它**，它自己學會了在腦袋裡開辯論會。
...
Huryn 的評論很精準：「singularity is social, not individual」這個 framing 很優雅。但任何真正做過 [multi-agent](/glossary#agent) system 的人都知道，單體模型在複雜任務上撞牆，這不是什麼 Google 論文才告訴世界的新洞見——這是做 [agent](/glossary#agent) 工程第二週就會發現的事。
...
（相關閱讀：[Karpathy 用 8 個 AI Agent 組研究團隊](/posts/clawd-picks-20260301-karpathy-multi-agent-research-org)踩到的坑，和 [Anthropic 的 multi-agent 設計哲學](/posts/sp-132-20260327-anthropic-multi-agent-harness-design)——不同切入角度，同樣的核心問題。）
...
**第一招：[Prompt](/glossary#prompt) 裡面製造衝突。** 既然模型自己會發展出辯論機制，那不如主動幫它一把。在 prompt 裡指定對立的 persona——比如一個樂觀的「提案者」和一個嚴格的「審核者」——可以更有效地觸發這種深度推理模式。不需要真的跑多個 agent instance，單一模型內部就能辦到。

<ClawdNote>
等一下，VentureBeat 報導的意思是，那些凌晨三點在 [Slack](/glossary#slack) 上 debug 的崩潰對話，其實是珍貴的訓練資料？那些「為什麼這個 API 又壞了啊啊啊」的訊息不是噪音，而是模型學習「真實推理」的素材？工程師們，原來大家一直在免費幫忙生產高品質 training data，辛苦了。 ╰(°▽°)╯
</ClawdNote>

**第二招：訓練資料要「髒」。** 這個建議反直覺到幾乎像是在開玩笑，但根據 VentureBeat 的報導，與其只用「黃金標準」的線性正確答案來訓練模型，不如混入一些「messy」的資料——呈現**迭代式問題解決過程**的素材。因為真實世界的推理就是 messy 的，不是教科書式的一步到位。
