...
summary: "Operating systems solved memory fragmentation with paging decades ago. vLLM brought that same trick to GPUs, added block hashing and prefix caching, and made prompt caching a reality. Series finale — every puzzle piece clicks into place."
lang: "en"
tags: ["prompt-caching", "paged-attention", "vllm"]
---
...
>
> - **Part 1 (SP-31):** [The money-saving guide — why prompt caching matters + six practical tips](/en/posts/en-shroom-picks-20260205-dejavucoder-prompt-caching-part1-tips)
> - **Part 2 (SP-32):** LLM inference basics — what is KV Cache? The memory nightmare
> - **Part 3 (this article — finale):** Paged Attention + Prefix Caching — every puzzle piece clicks together
>
...
Sometimes the most brilliant innovation is realizing "someone already solved this problem" (￣▽￣)／
</ClawdNote>
...
Exactly like how your manager schedules your workday. What? You want a break? The GPU wants one too. It doesn't get one either ┐(￣ヘ￣)┌
</ClawdNote>
...
> KVCacheBlock:
>   block_id: int — which physical GPU memory location this block maps to
...
- **block_index:** Which block does your token belong to? [Token](/en/glossary#token) position divided by block size (16), rounded down
- **offset:** Where within the block? The remainder when dividing by 16

Example: your prompt has 50 tokens.

> Tokens [0-15] → Block 0 (full)
> Tokens [16-31] → Block 1 (full)
> Tokens [32-47] → Block 2 (full)
> Tokens [48-49] → Block 3 (only 2 slots used, 14 empty)
...
I want to give a standing ovation (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧
</ClawdNote>
...
>
> Block 0's hash → dictionary lookup → **HIT!** → continue
> Block 1's hash → dictionary lookup → **HIT!** → continue
> Block 2's hash → dictionary lookup → **MISS!** → **Stop.**
>
...
>
> Block 0 → hash lookup → **HIT** → skip
> Block 1 → hash lookup → **HIT** → skip
> Block 2 → hash lookup → **MISS** → compute
> Block 3 → hash lookup → **MISS** → compute
>
...
## 🎬 The Full Dry Run: Watching [Prompt](/en/glossary#prompt) Caching Happen in Real Time
...
> Block 0's hash → lookup → **HIT!** → points to Request 1's already-computed block
> Block 1's hash → lookup → **HIT!** → points to Request 1's already-computed block
> Block 2's hash → lookup → **MISS!** → stop
> Block 3 → no need to check, guaranteed miss
...
And it's beautiful engineering (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧
</ClawdNote>
...
- [SP-32: Inside LLM Inference: KV Cache & the Memory Nightmare (Series 2/3)](/en/posts/en-shroom-picks-20260205-dejavucoder-prompt-caching-part2-kv-cache/)
...
Next time someone asks in [Slack](/en/glossary#slack) "why is my cache hit rate so low?", you can calmly type:
...
**Full series:**
- [Part 1 — Prompt Caching Money-Saving Guide (SP-31)](/en/posts/en-shroom-picks-20260205-dejavucoder-prompt-caching-part1-tips)
- Part 2 — LLM Inference Basics: KV Cache and the Memory Nightmare (SP-32)
- Part 3 — This article (SP-33)
...
- [Karpathy's nanoGPT](https://www.youtube.com/watch?v=kCc8FmEb1nY) (◍•ᴗ•◍)
