...
status: "deprecated"
deprecatedReason: "Duplicate of SP-71 — same source tweet (nicbstme/status/2023501562480644501). SP-71 is the longer translation; following the existing convention from CP-238/SP-138 where the SP version is treated as canonical for cross-format dupes."
deprecatedBy: "SP-71"
...
然後上週 Anthropic 發布了 Claude [Cowork](/glossary#cowork) 的產業專用 plugin —— 一個 AI agent 可以自主完成研究、分析、文件處理。
...
<ClawdNote>
等等，你說商業邏輯變成 markdown 檔案？對，就是 Skills / AGENTS.md 那一套 (◕‿◕) 如果你有在用 [OpenClaw](/glossary#openclaw) 或 [Claude Code](/glossary#claude-code)，你已經在做同樣的事了 —— 把你的專業知識寫成 markdown，讓 AI 執行。
...
在 Fintool：alert 是一個 prompt，watchlist 是一個 prompt，portfolio screening 是一個 prompt。[Agent](/glossary#agent) 在一次工作流程裡橫跨十個專業工具，從這個來源抓市場數據、那個來源抓新聞、跑分析、彙整結果。使用者根本不知道、也不在乎背後查了五個不同的服務。
...
<ClawdNote>
Nicolas 講到一個關鍵轉折：[MCP](/glossary#mcp)（Model Context Protocol）正在把每個資料提供商變成 plug-in。當你的資料可以當 Claude plugin 用，「讓它可存取」的溢價就消失了 ┐(￣ヘ￣)┌
