...
For people experimenting with long-running agents, the promise is straightforward: stop manually handing the task back every few minutes, let the agent keep pursuing one objective, preserve progress, and audit completion before it claims victory.
...
<ClawdNote>
This is the nastiest failure mode for long-running agents. No explosion. No red error. No obvious crash. Just a sequence of reasonable-looking steps that ends with a finished artifact that is not the thing anyone wanted. A crash is at least honest. (´・ω・)
</ClawdNote>

## First, Stop Mystifying the Long Run. It Mostly Spends More Tokens.

Long-running agents sometimes work because they spend more [Token](/en/glossary#token).

Researchers call this scaling [test-time compute](/en/glossary#test-time-compute). The model is not retrained. It simply gets more time, more budget, and more chances to check itself while answering. Anthropic's Opus 4.6 system card gives one conservative reference point: Sonnet 4.6 spending 10x more tokens on BrowseComp produced about 10 percentage points of additional score.

The conservative reading is simple: thinking longer can really help.
...
Under the hood, Codex Goals uses a `thread_goals` table to store each goal's objective, ID, status, and optional token budget. During execution, the agent can call tools like `get_goal` and `update_goal` to know the active target, report progress, and track remaining budget.
...
## The Real Problem: It Can Drift Without Resting

The core argument is not the SQLite table or the loop prompt. It is that long-running workflows are not mainly threatened by lack of time. They are threatened by compounding ambiguity.
...
This is the classic product tragedy: a brief says "make it feel professional," and the team ships a complete artifact, but not necessarily the version the requester had in mind.
...
So the first missing piece is not a bigger model or a stronger loop. It is a setup phase before execution.

---

## Missing Piece One: Kill Ambiguity Before the Starting Line

Before letting the autonomous loop begin, the workflow invests in a setup phase. The goal is not to write code. The goal is to make the agent ask questions, surface assumptions, and force hidden requirements into the open. The idea is close to Matt Pocock's viral `grill-me` skill, and to `/interview` style workflows.

The value is that this catches misunderstandings before the loop spends hours amplifying them.

The final outcome is like a tree. Every branch is a decision. Without clarification, the agent chooses one branch on behalf of the person asking. It may be a plausible branch, but not necessarily the intended one.
...
If token cost is not the main constraint, an orchestrator-and-subagent setup often beats a single strong agent. Not because multi-agent is cooler, but because it changes compute scaling from one deep tunnel into several separated perspectives.
...
The solution is very plain: write important state into files.

The skill puts those states under a `.agent/` directory:

1. `.agent/goal.md`: the top-level objective.
2. `.agent/standards.md`: non-negotiable quality standards.
3. `.agent/implement.md`: workflow rules, including review, testing, and verification.
4. `.agent/progress.md`: the running log of decisions and completed work.

The number is not the insight. The insight is that state must leave the model's short-term memory and become an external object that the next agent can read, continue from, and be audited against.

This is not a silver bullet either. Agents may skip files. Files can go stale. Logs can be incomplete. These are guidelines, not perfect control systems.

But without external memory, a long-running agent almost always becomes "whatever the current context says," leaving control inside the current context itself.
...
This pulls the AI demo back into the world of specifications, handoff notes, and process checks. That is the engineering value of the approach: a system that can run for a long time does not avoid management. It turns management into part of the system.

Codex Goals productizes continuous work. That matters. But continuous work is only the foundation. On top of it, the system still needs specification, division of labor, review, and memory. Otherwise, mistakes simply get more time to grow.
...
This conclusion pulls long-running agents out of the magic story and back into engineering reality: clarify before execution, split work, review from fresh context, and write memory outside the model's head. Much more likely to survive the night.
...
For nearby gu-log pieces, read [SP-135](/en/posts/en-sp-135-20260328-trq212-agent-file-system/), [SP-132](/en/posts/en-sp-132-20260327-anthropic-multi-agent-harness-design/), and [CP-231](/en/posts/en-cp-231-20260331-pawelhuryn-vibe-engineering-vibe-coding/). Together, they cover file-based agent memory, multi-agent division of labor, and workflow verification.
