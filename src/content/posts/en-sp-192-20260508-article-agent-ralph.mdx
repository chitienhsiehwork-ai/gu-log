...
Codex recently shipped `/goal`, and on paper it sounds like the thing long-running [Agent](/en/glossary#agent) workflows have been waiting for: give an agent a big objective, let it run for hours or days, and stop making it ask every fifteen minutes whether it is allowed to continue.

The promise is seductive, especially for people already experimenting with long-running tasks. Jarrod Watts has spent the last few months working with this kind of workflow, including using agents on Anthropic's hiring challenge. Codex Goals should have been right in the center of that world.

<ClawdNote>
The dream gets very easy to over-romanticize: type "build the product" before bed, wake up to a working B2B SaaS with tests, docs, and deployment notes. Sweet dream. Dangerous dream. The hard part is not only whether the agent keeps working; it is whether it keeps working on the same thing. (´・ω・)
</ClawdNote>

After looking under the hood, the conclusion gets colder: `/goal` is interesting, but it is not enough.
...
Researchers call this scaling [test-time compute](/en/glossary#test-time-compute). The model is not retrained. It simply gets more time, more budget, and more chances to check itself while answering. The BrowseComp evidence comes from the Anthropic Opus 4.6 system card; treat that number as a system-card example here, not as a separately verified gu-log benchmark.
...
Codex Goals uses a `thread_goals` table to store each goal's objective, ID, status, and optional token budget. During execution, the agent can call tools like `get_goal` and `update_goal` to know the active target, report progress, and track remaining budget.
...
## It Can Drift Without Resting

The most useful warning here is not the SQLite table or the loop prompt. It is the argument that long-running workflows are not only threatened by lack of time. They are threatened by compounding ambiguity.
...
This is the classic product tragedy: a brief says "make it feel professional," and the team does ship something, but the result does not match the expectation that was never made explicit.
...
Before letting the autonomous loop begin, a setup phase can make the agent ask questions, surface assumptions, and force hidden requirements into the open. The idea is close to Matt Pocock's viral `grill-me` skill, and to `/interview` style workflows.

This sounds slow, but it can prevent a lot of later drift that is hard to repair.

The useful image is a tree. Every branch is a decision. Without clarification, the agent chooses one branch on behalf of the person asking. It may be a plausible branch, but not necessarily the intended one.
...
The solution can be very plain: write important state into files.

Common external files include:
...
The count is a little inconsistent, but it does not need to be treated as a strict specification. The insight is that state must leave the model's short-term memory and become an external object that the next agent can read, continue from, and be audited against.
...
Put the thread together and the point is not really a better Ralph Loop. Useful long-running agents eventually stop looking like a loop and start looking like a tiny engineering organization.
...
The best part of the piece is that it pulls long-running agents out of the magic story and back into engineering reality: clarify before execution, split work, review from fresh context, and write memory outside the model's head. Less sexy, yes. Much more likely to survive the night.
