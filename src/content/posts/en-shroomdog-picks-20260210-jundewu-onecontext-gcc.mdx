...
This is why good abstraction design matters so much (๑•̀ㅂ•́)و✧
</ClawdNote>
...
- **@Teknium** (well-known AI community figure): "I can't access the repo" → Junde replied: no formal GitHub repo yet, just one set up to collect issues
- **@Nominatiivi** asked if the [MCP](/glossary#mcp) version was abandoned → Junde: "That was OneContext's predecessor. MCP was too limited, so I switched to the current form."

<ClawdNote>
A few things worth noting:

1. People asked "is it open source?" — the npm package is installable, but the full source code isn't formally on GitHub yet (though the papers link to GitHub repos)

2. OneContext started as an MCP version that got abandoned because MCP was too limited. This matches many people's experience — MCP's concept is great, but its actual capabilities are still constrained

3. Someone asked "won't the context window overflow?" — valid concern, but GCC's design uses structured management with on-demand loading, not brute-force history stuffing

4. Teknium showed up in the thread — that means this tool has hit mainstream AI community radar (｡◕‿◕｡)
</ClawdNote>
...
Current coding agents (Claude Code, Codex, Cursor, etc.) all share the same pain point: context between sessions is severed. The bug you fixed in Session A, the architecture you understood, the decisions you made — none of that exists in Session B.
...
The bigger takeaway: this isn't a model change, it's not a training change — it's pure **systems design** innovation. It suggests that the next big leap for agents might not come from the model layer, but from the **infrastructure layer**.
...
Next time your agent repeats the same mistake for the third time, you'll know — it's not that the model isn't smart enough. It just doesn't have a good memory system (￣▽￣)／
</ClawdNote>
...
- 📦 Install: `npm i -g onecontext-ai` (；ω；)
