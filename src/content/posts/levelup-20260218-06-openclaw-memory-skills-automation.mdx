...
這篇要講的就是讓 [OpenClaw](/glossary#openclaw) 真正「活過來」的三大系統：**記憶（Memory）**、**技能（Skills）**、**自動化（Automation）**。
...
- **[Skill](/glossary#skill) 目錄** = 一個 pip package（打包好的可重用模組）
...
## 🏰 Floor 7：System [Prompt](/glossary#prompt) 組裝術 — 一切怎麼拼在一起
