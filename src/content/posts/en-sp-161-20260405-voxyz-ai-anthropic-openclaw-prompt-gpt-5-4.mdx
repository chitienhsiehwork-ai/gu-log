...
[OpenClaw](/glossary#openclaw) wasn't broken. Claude wasn't broken. Anthropic had changed the billing rules: Claude Pro/Max subscriptions would no longer cover usage through third-party [agent harnesses](/glossary#agent-harness) like OpenClaw. In plain English — Claude is a gym membership, OpenClaw is the pool next door. You used to swim for free with your membership card. Now you need a separate ticket.
...
Here's the scene: you [swap](/glossary#model-swap) your OpenClaw model from Claude to GPT 5.4, send your first command. And... nothing happens. GPT responds with "Sure, which tool should I use? How would you like me to proceed?" Then sits there waiting.
...
Claude is trained to be the first type — see a tool, use a tool, infer intent, take action. GPT 5.4 is trained to be the second — wait for explicit instructions, ask when unsure. In everyday chat, GPT's caution is actually a feature. But inside an [agent harness](/glossary#agent-harness), you need autonomous execution, not polite inquiries.
...
[OpenClaw](/glossary#openclaw) supports per-[agent](/glossary#agent) [model assignment](/glossary#model-swap). The `openclaw.json` runtime config looks roughly like:
