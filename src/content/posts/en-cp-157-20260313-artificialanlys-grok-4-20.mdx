...
Artificial Analysis has only formally benchmarked the reasoning and non-reasoning versions so far. The multi-agent version is tricky for a more basic reason than "is it good?" — how do you even test it fairly? Traditional benchmarks assume one model answers one question. Multi-agent systems look more like a team discussion happening offstage before you get a final answer.
