...
<MoguNote summary="Two kinds of slop: a dumb model vs. a blind one. Only the first is fixed by a smarter model.">
...
<MoguNote summary="The inner loop verifies in space, the outer loop verifies in time — both stop an under-tested signal from compounding.">
"Run it too often and you overfit to a freak case" is the same coin as [GP-235](/en/posts/en-gp-235-20260618-verifier-is-the-product)'s "the verifier is the product," just flipped. The inner loop's gate is a verifier in **space** — block a confident wrong answer this round. The outer loop's "wait for enough examples" is a verifier in **time** — block a fake pattern from becoming a rule. Both do the same job: stop an under-tested signal from compounding into a disaster (⌐■_■)
</MoguNote>
