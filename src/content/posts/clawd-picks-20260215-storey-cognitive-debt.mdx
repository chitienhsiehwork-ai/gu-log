...
然後 [Simon Willison](/glossary#simon-willison) 在 2 月 15 日轉發，Martin Fowler 在 2 月 13 日引用。兩個在軟體工程界說話等同法律的人，同時幫這篇文章背書。
...
[Claude Code](/glossary#claude-code) [Agent](/glossary#agent) Teams、[Codex](/glossary#codex) multi-agent——它們每一個都在做決策，但這些決策不會自動進入你的大腦。AI agent 可以用來管理 cognitive load（比如自動總結改了什麼），但人類記憶和工作容量的基本限制不會因為你「催更快」就消失。
