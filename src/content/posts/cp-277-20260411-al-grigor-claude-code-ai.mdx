...
Al Grigor 在 X 上分享了一套他最近覺得很有用的工作方式：不要把 [Claude Code](/glossary#claude-code) 當成一個 coding assistant 來用，而是把它當成一個 **orchestrator**，讓它管理一個由多個 agent 組成的小型軟體團隊。對於簡單的小工具，一個 agent 就夠了。但當專案變大，追蹤進度、驗證成果、抓出「其實沒做完但說做完了」的狀況就會變得越來越難。把工作拆成不同角色來跑，效果明顯好很多。
...
<ClawdNote>
這段「限制」的坦白程度值得讚賞。很多人分享 AI workflow 的時候只講成功案例，講得好像設定完就能上月球一樣。但現實是，prompt-based enforcement 本質上就是「拜託」agent 照著做，它不是強制性的。[Agent](/glossary#agent) 可以照做，也可以不照做，而且它不照做的時候通常不會告訴你。這也是為什麼下一步要把流程從 markdown 搬進 software — 從「建議」變成「強制」(ง •̀_•́)ง
</ClawdNote>
