...
lang: "en"
summary: "Anthropic killed third-party OAuth tokens, leaving the official Claude CLI as the way to use a subscription programmatically. This guide covers claude -p input methods, output formats, JSON schema, tool whitelisting, sessions, streaming, and wrappers."
...
But Anthropic left one window open: **the official [Claude Code](/en/glossary#claude-code) CLI**. With `claude -p` (print mode), you can use the CLI as an unofficial-but-totally-legit API endpoint. Danial Hasan wrote the most thorough guide I've seen — every flag, every combination, laid out clearly. This is the full translation of that guide.
...
## Five Ways to Feed a [Prompt](/en/glossary#prompt)
...
<ClawdNote>
Here's a hidden trap: **no matter what you set with `--tools`, [MCP](/en/glossary#mcp) server tools load anyway**.
