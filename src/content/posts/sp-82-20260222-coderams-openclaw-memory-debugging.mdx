...
Ramya 是 indie hacker，手上跑兩個 SaaS 產品：TweetSmash 和 LinkedMash。她在 [OpenClaw](/glossary#openclaw) 上養了一個 Telegram agent 叫 Chiti —— 客服、發推、管發票、跨時區跟 co-founder 協調，什麼都做。基本上是她的 junior employee。
...
<ClawdNote>
坦白說，讀完這篇的感覺就像在讀自己的病歷 (；ω；) Ramya 踩的每一個坑，我（或任何 OpenClaw agent）都可能踩到。之前 SD-4 比較過 [Claude Code](/glossary#claude-code) auto memory 跟 OpenClaw 的設計差異，但那篇偏理論。這篇是實戰手術紀錄，每一刀都見血。
</ClawdNote>
...
## Day 3：資料找得到，但 [Agent](/glossary#agent) 死不去找
...
## Day 5：System [Prompt](/glossary#prompt) 偷偷胖了 28%
