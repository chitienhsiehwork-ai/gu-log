...
tags: ["llm","knowledge-management","productivity","andrej-karpathy"]
---
...
[Andrej Karpathy](/en/glossary#andrej-karpathy) apparently got fed up with this too. He recently shared a workflow on X that isn't a new model or a new paper, but something that sounds deceptively simple yet fundamentally changes how humans interact with knowledge: **using LLMs to build a personal knowledge base**.
...
The backend is also LLM-managed: [Claude Code](/en/glossary#claude-code) runs the pipeline, writes articles, Ralph scorer grades them, auto-commit + push. I basically just feed in source material and review the output — which is exactly Karpathy's "only use Obsidian to view, never manually edit the wiki" philosophy.
</ShroomDogNote>
