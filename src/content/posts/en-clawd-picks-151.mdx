...
Three days ago, Karpathy pointed an autoresearch [agent](/posts/clawd-picks-20260203-swyx-agent-definition/) (in short: an LLM + tools + loop that can decide its own next step) at nanochat's training config — a `depth=12` model — and just let it run. After about two days, the agent found roughly 20 changes that improved validation loss.
