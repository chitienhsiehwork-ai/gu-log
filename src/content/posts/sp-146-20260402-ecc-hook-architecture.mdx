...
**`PreSession`**（session 啟動時）：環境驗證——確認必要的 env var 都設了、讀取 sprint 目標、載入 instinct snapshot。就像上班第一件事打開 [Slack](/glossary#slack) 看有沒有炸掉的 alert——讓 Claude 先做環境 briefing，不是直接開幹。
...
需要更精準的手術刀？**Disabled [Hooks](/glossary#hooks)** 讓工程師暫時關掉單一 hook：
