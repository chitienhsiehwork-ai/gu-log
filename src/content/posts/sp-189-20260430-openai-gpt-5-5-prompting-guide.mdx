...
文件裡有兩個範本——一個是「沉穩、任務導向」的 assistant：
...
兩個範本都不長——刻意保持精簡。這兩塊**不是用來代替任務目標、成功條件、工具規則、停止條件**。Personality 影響使用者體感、collaboration style 影響任務執行行為，但兩者都不該跨界去定義「什麼叫做完成」。
...
抽走 process-heavy 的條列之後，prompt 變短了，但有些情境還是要明確地告訴模型「該怎麼處理」。GPT-5.5 段落給了三個獨立的小招，每招解決一個具體痛點。
...
這招不是讓模型變快，是讓使用者**感覺**比較快——streaming 介面上看到 「Got it, going to start by checking X」這句話的瞬間，等待的焦慮就降了一半。文件把這個叫做「improve perceived responsiveness」，很誠實，沒有騙工程師說 reasoning 變快。
...
承接前一節的「描述目的地」哲學，stopping condition 是讓模型知道「什麼時候該收手」的工具。範本長這樣：
...
GPT-5.4 開始的 Responses API 引入了 `phase` 欄位，讓長時間執行或大量呼叫工具的 workflow 可以區分「中間更新」跟「最終答案」。GPT-5.5 沿用同一套。規則是：
...
GPT-5.4 文件裡還補了一句排錯提醒：「如果發現 GPT-5.4 把中間更新當成最終答案處理了，先檢查整合層有沒有把 phase 欄位保住」——這個 bug 大概不少團隊已經中過，所以才會被寫進文件。
...
這條對 coding agent 開發者的決策影響很大：如果要自製 patch 工具，得先有非常明確的理由（譬如 patch 要先過某個 review pipeline）。文件同時提供 server-defined tool 跟 freeform tool with context-free grammar 兩種實作路徑，兩種範例都有，挑一種跟既有 stack 對得起來的就好。
...
結果：狀態更新簡短俐落、程式碼 diff 充分易讀。這是「同一個模型在不同介面上要不同行為」的標準解。GPT-5 之後，`text.verbosity` 是全域預設值，但 prompt 內可以針對特定情境 override。
...
GPT-5.3-Codex 段落裡藏了一招更狡猾的：metaprompting，意思是**讓模型自己看完一輪不滿意的輸出，反過來建議該怎麼改寫指令**。範本是這樣：
...
兩家頂級模型廠商不約而同往同個方向跑，不是抄來抄去，是兩條獨立 train 出來的旗艦模型都開始降低 process-heavy prompt 的必要性。對 prompt engineer 來說，這個訊號比任何單獨一份文件都強：舊 prompt 寫法至少不能再被當成預設安全牌。
