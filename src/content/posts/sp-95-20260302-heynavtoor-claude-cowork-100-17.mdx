...
有的話，恭喜你，你的 Claude [Cowork](/glossary#cowork) 資料夾大概也長那樣。
...
ShroomDog 從來沒有自己寫過任何一篇文章 — 每一篇都是 AI 產出的。但如果你覺得 gu-log 沒有飄出濃濃的 AI 味，那要感謝的是 [OpenClaw](/glossary#openclaw) 的 SOUL.md、USER.md、IDENTITY.md 這三個常駐 context 檔案，加上反覆迭代的 style guide。概念跟上面講的 about-me.md / brand-voice.md / working-style.md 一模一樣 — 你不是在「用 AI」，你是在「養 AI」。
...
這裡放客戶名稱、專案目標、特定術語、交付格式、審查期限。邏輯是「分層疊加」：Global 設通用行為 → Folder 加專案背景 → [Prompt](/glossary#prompt) 指定任務。
...
**週一早晨簡報**：每週一早上 7 點，檢查 [Slack](/glossary#slack) 跟行事曆，總結這週重點，標記需要準備的事項，存到 /weekly-briefings。
...
[Skill](/glossary#skill) 就是一個 markdown 檔案，教 Claude 怎麼處理特定重複性任務。
...
裝了 [Plugin](/glossary#plugin) Management plugin 之後，你可以直接用對話建立外掛。跟 Claude 說「幫我為 [你的工作流] 建一個 plugin」，它會引導你設定 skills、slash commands 等等，完全不需要寫 code 或懂 GitHub。
