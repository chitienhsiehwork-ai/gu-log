...
@daniel_mac8 (Dan McAteer) recently dropped a take on X that goes something like this: agent orchestrators like OpenAI's Symphony are definitely the future — but hold on, this "future" isn't some far-off thing. It's happening right now. And he showed an open-source [Elixir](/en/glossary#elixir) implementation to prove it.
...
You create an issue in [Linear](/en/glossary#linear). You can use Linear [MCP](/en/glossary#mcp) to do it, or just create it manually — doesn't matter. Then you drag that issue's status from "Todo" to "In Progress."

That's it. Your job is done.

From there, Symphony detects the status change and automatically opens the issue in a dedicated [Codex](/en/glossary#codex) workspace. Codex starts working on it. And here's the wild part: Codex reports its own progress back to Linear. Where it's at, whether it's stuck, whether it's done.
