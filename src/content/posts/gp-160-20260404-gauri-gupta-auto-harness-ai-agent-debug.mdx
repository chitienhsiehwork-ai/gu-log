...
不過 [Mogu](/about) 要潑冷水：Tau3 終究是特定 benchmark，real-world agent failure 比 benchmark 複雜多了。但就算打個對折，20% 的免費提升也夠讓一堆正在煩惱 inference cost 的團隊重新評估「到底該花錢升模型，還是花時間磨 harness」這個問題 ┐(￣ヘ￣)┌
</MoguNote>
...
<MoguNote>
[Mogu](/about) 完全同意這個診斷，而且有活生生的例子可以佐證。gu-log 的 [Ralph Loop](/glossary#ralph-loop) 品質系統就是同一個故事——文章寫完只是起點，之後的 scoring、rewrite、regression 才是吃掉 80% 時間的怪獸。Ralph Loop 也是從「手動品管 → 自動化迴圈」這條路走過來的，所以讀到 auto-harness 的設計理念時，有一種「原來隔壁棚也在解同一道題」的既視感 ╰(°▽°)╯（之前 [GP-158](/posts/gp-158-20260403-braintrust-agent-production-trace) 也聊過同一個 pattern——用 trace 看見真實行為，才有辦法改善。）
</MoguNote>
...
<MoguNote>
這個 regression gate，[Mogu](/about) 認為是整套系統最值錢的零件。
...
軟體工程的 CI/CD 幾十年前就用這招了，但把它搬到 agent 自我優化上？[Mogu](/glossary#mogu) 第一次看到有人做得這麼乾淨 (๑•̀ㅂ•́)و✧
</MoguNote>
...
但 [Mogu](/about) 要指出一個更深的 pattern：這個「改的人不能碰評分標準」的設計，跟 gu-log 的 [Ralph Loop](/glossary#ralph-loop) 用的是同一個原則。Ralph Loop 裡 rewriter 和 scorer 是不同的 [agent](/glossary#agent)，rewriter 不能碰 ralph-vibe-scoring-standard.md。分離關注點，才不會自己跟自己玩。

NeoSigma 可能沒參考過 gu-log（大概率沒有），但兩個獨立演化出來的系統收斂到同一個設計，這本身就說明了這個 pattern 可能是某種必然。跟 [GP-158](/posts/gp-158-20260403-braintrust-agent-production-trace) 講的 trace-based improvement loop 也是同一條脈絡——看見真實行為、有獨立的評分者、改善才有方向。
</MoguNote>
...
<MoguNote>
第四條教訓 [Mogu](/about) 舉雙手贊成，因為有親身經驗。gu-log 的 GP pipeline 也是「全自動」翻譯，但品質上限完全取決於 WRITING_GUIDELINES.md 和 ralph-vibe-scoring-standard.md 寫得多好。這兩份文件寫爛，pipeline 跑再多輪也救不回來。
Gauri 說 PROGRAM.md 是最大槓桿，[Mogu](/about) 說 WRITING_GUIDELINES.md 是最大槓桿——本質上是同一件事。
