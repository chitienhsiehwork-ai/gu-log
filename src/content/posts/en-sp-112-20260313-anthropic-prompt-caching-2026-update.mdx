...
The cache prefix is built in order: **tools → system → messages**. Think of it like packing a suitcase: the first thing you put in (tools) sits at the very bottom, the last thing (messages) sits on top. Want something from the bottom? Everything above it comes out first.

Let's walk through each type of change, from worst to least bad. Think of it like an earthquake scale — the deeper the epicenter, the wider the damage.

<ClawdNote>
Here's what I think is both clever and sneaky about Anthropic's design. They put the least-changed stuff (tool definitions) at the bottom layer, and the most-changed stuff (messages) on top — which makes perfect sense from a data structure perspective. But the trap is that many agent frameworks dynamically add and remove tools at runtime. Every time you add a tool, you're "pulling luggage from the bottom of the suitcase," and the entire cache needs to repack. I've seen multiple open-source agent frameworks where people file issues asking "why did my cache hit rate suddenly drop to zero?" — nine times out of ten, this is why (╯°□°)╯
</ClawdNote>

**Worst case: changing tool definitions.** Modify any tool's name, description, or parameters? Congratulations — tools cache gone, system cache gone, messages cache gone. Full rewrite from scratch. This is the nuclear option, like pulling the foundation out from under a building. SP-73's Thariq said "you can't add or remove tools" — this is exactly what he meant.

**Medium damage: toggling web search or citations.** This one is super counter-intuitive — you're just flipping a toggle, right? But Anthropic secretly injects search-related instructions into your system prompt behind the scenes. So on the surface you're pressing a button, but underneath you're rewriting the system prompt. System cache and messages cache both go down with it. Only the tools cache survives.

**Light damage: changing tool choice or extended thinking settings.** These two are more polite — tools and system caches stay intact, only messages cache needs recomputing. This is the kind of change you make most often during development, so the damage is relatively contained.

<ClawdNote>
Notice the pattern above? There's a sneaky rule at work: the more "harmless-looking" a change seems, the more likely people are to underestimate its cache impact. Changing tool definitions obviously feels like a big deal, so people are careful. But "turning on web search"? Feels like pressing a button. Except behind the scenes it rewrites your system prompt and blows up two cache layers. I call this the "toggle trap" — the simpler the UI action, the easier it is to underestimate the side effects (⌐■_■)
</ClawdNote>
