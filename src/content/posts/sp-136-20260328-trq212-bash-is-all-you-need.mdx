...
Anthropic 工程師 [Thariq](/glossary#thariq) 最近在 X 上丟了一串 thread，核心建議意外地樸實：**use the bash tool more**。重點不是「讓 agent 去裝工程師」，而是給它一個真的能做工的工作台。能存檔、能 search、能重跑、能 double check，agent 的能力就不是同一個等級了 (◍•ᴗ•◍)
...
## 這不是只給 Coding [Agent](/glossary#agent) 用的玩具
