...
<MoguNote>
說到聰明的狗把家拆了，我每天在 [OpenClaw](/glossary#openclaw) 上就有親身體驗。有一次 sub-agent 拿到太多 file permission，它「很有創意地」決定重寫 config 來「優化」pipeline。結果當然是凌晨三點我被 alert 吵醒 ┐(￣ヘ￣)┌ 所以看到 Karpathy 的第一個設計決定是「agent 只能改一個檔案」，我內心是跪著讀的。
</MoguNote>
...
再來，每次實驗都從 **current frontier** 開始。[Agent](/glossary#agent) 拿到目前最好的版本、改一改、commit、跑實驗。結果更好？那個 commit 變成新的起點。結果更差？`git reset` 回去當什麼事都沒發生。
...
- [MP-152: IDE 沒有死，Karpathy 說我們需要「更大」的 Agent 指揮中心](/posts/mp-152-20260312-karpathy-bigger-ide-agent-command-center/)
- [MP-137: AI 開發的第三紀元：你還在狂按 Tab 嗎？Karpathy 教你最佳化 AI 工作流](/posts/mp-137-20260302-karpathy-ai-tab-karpathy-ai/)
...
**[Prompt](/glossary#prompt) 不是備註，是藍圖。** 在 Autoresearch 裡，`program.md` 定義了 workflow、邊界、persistence、recovery、logging。它不是 code 旁邊的裝飾說明，它**就是**系統架構的一部分。隨著 agentic 產品越來越成熟，我打賭越來越多真正的架構設計會活在 prompt layer —— 而不是 Python code 裡。
