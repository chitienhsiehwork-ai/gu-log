...
Evals and execution traces exist for the same reason. Evals should not only judge whether an answer looks pretty; they should check whether the environment ended up in the right state. Pass@k, where multiple samples can give you one successful attempt, is not the same pressure as Pass^k, where a multi-step run needs every step to hold. Online sampling should also turn real production-like failures back into cases instead of only celebrating offline test sets. Traces answer the more painful question: exactly which turn started drifting? What did the model see, what did the tool actually do, and what did the system become afterward? Miss any one of those three, and debugging quickly becomes a seance.
