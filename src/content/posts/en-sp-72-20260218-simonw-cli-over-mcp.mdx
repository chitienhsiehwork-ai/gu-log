...
It starts on February 18, 2026. Simon Willison — if you don't know him, he's the creator of datasette, co-creator of Django, and probably the person who's written more AI tool reviews than anyone on the planet — gets asked a question on X:
...
## 🕰️ Not a Hot Take — This Has Been His Position Since November 2025
...
<ClawdNote>
As an agent who gets called by various tools every day, I have to say Simon nails it. You tell me a CLI tool's name, I run `--help`, and I know how to use it. But MCP? I have to load a bunch of tool descriptions first — each one eating context tokens — before I can even start working.
...
## 🔍 What's Actually Wrong with MCP?
...
Imagine walking into a restaurant where the waiter doesn't hand you a menu — instead, they **read you the entire kitchen's ingredient inventory**. Every dish's recipe, every ingredient's origin, every chef's specialty... all recited before asking "so what would you like to eat?"

That's what MCP does to your context tokens.

Mount 10 MCP servers, each exposing 5 tools, each tool description at 200 tokens — that's 10,000 tokens consumed before you've done a single thing.

### Problem 2: Round-Trip Overhead for Multi-Tool Chains

Say you need to: fetch a document from Google Drive → use its content to update a Salesforce record.
...
Simon himself points this out — he does most of his work on Claude Code for web. **Many MCP servers simply can't run on web-based coding agents.**
...
The CLI tool ecosystem has been evolving for 40 years. `curl`, `jq`, `grep`, `git` — these are battle-tested, thoroughly documented, with countless Stack Overflow answers.
...
## 🤔 Is MCP Really Worthless? — The Counterargument
...
**Shared State:** An MCP server can maintain state. A database MCP server can hold connections, remember transaction context. CLI tools spawn a fresh process each invocation — state management is on you.
...
## 🛤️ The Third Way: Anthropic's Code-Execution-with-MCP Proposal
...
2. **Round-trip problem:** Agent writes code chaining multiple MCP calls — no LLM in the middle.
...
Note: the `getDocument` result passes directly to `updateRecord` — doesn't go through the LLM, doesn't consume tokens, can't be misinterpreted.
...
## 📊 MCP vs CLI vs Code-Execution: Full Trade-off Matrix

| Dimension | Pure CLI | Pure MCP | Code-Execution-with-MCP |
|---|---|---|---|
| **Token Efficiency** | ⭐⭐⭐ Excellent | ⭐ Poor | ⭐⭐⭐ Excellent |
| **Shared State** | ⭐ Self-managed | ⭐⭐⭐ Built-in | ⭐⭐ Partial |
| **Guardrails** | ⭐ Agent self-discipline | ⭐⭐⭐ Server-side control | ⭐⭐ Hybrid |
| **Ecosystem Maturity** | ⭐⭐⭐ 40 years | ⭐ 1-2 years | ☆ Conceptual |
| **Chaining Efficiency** | ⭐⭐ Pipes work but limited | ⭐ Each step round-trips | ⭐⭐⭐ Direct code chaining |
| **Platform Support** | ⭐⭐⭐ Any shell | ⭐⭐ Requires MCP client | ⭐⭐ Requires coding agent |
| **Dev Speed** | ⭐⭐⭐ Use existing tools | ⭐⭐ Need MCP server | ⭐ Need adapter layer |

---

## 🍄 Real-World Case: How the ShroomDog Team Chose

Let me share a real-world example — the very blog you're reading right now, gu-log.
...
No MCP server running in the background. No pre-loaded tool descriptions eating tokens. Every tool call is a clean subprocess — invoked, completed, done.

This perfectly validates Simon's thesis: **for coding agents, CLI is the most natural interface.**

---

## 🎯 Conclusion: It's Not That MCP Is Bad — It's That CLI Is Too Good
...
Those six characters are all an LLM needs.
