...
If you've started running [Claude Code](/en/glossary#claude-code), [Codex](/en/glossary#codex) CLI, or other [agents](/en/glossary#agent) on a remote machine, you probably learned one brutal lesson very quickly: **an SSH session without tmux is basically an unsaved document.** One network hiccup, one closed laptop lid, one flaky coffee shop Wi-Fi moment, and the task that was happily running for 20 minutes is suddenly dead. [Simon Willison](/en/glossary#simon-willison) [wrote about this exact problem](/en/posts/en-sp-86-20260225-simonw-claude-code-remote-cowork/) — session management is the first thing you need to solve when running agents remotely. Felipe's thread is the answer.
...
<ClawdNote>
Developers are suckers for rare-but-awesome tricks: the macro you use once a month, the perfect one-liner you paste into [Slack](/en/glossary#slack) to impress everyone. The real money is usually in the boring command you hit 50 times a day. At that point you're not just using a tool. You're training your nervous system (๑•̀ㅂ•́)و✧
</ClawdNote>
...
The moment you start SSHing into a VPS to run [Claude Code](/en/glossary#claude-code), [Codex](/en/glossary#codex) CLI, or similar [agent](/en/glossary#agent) tools, tmux changes category. It stops being a tool that helps organize terminals and becomes a tool that hosts a fleet of long-running processes. A disconnect no longer kills the task, because detach and reattach is all it takes. Multiple agent sessions can run side by side — one working on a feature, one watching logs, one handling a hotfix — instead of nesting them all inside one window like Russian dolls.
...
"You never think about session names" sounds like a tiny improvement. It is. And that is exactly why it matters. Tiny decisions are the ones that quietly tax your whole day. Especially now that the terminal is no longer just where you run a few commands, but a place where you host [agents](/en/glossary#agent), manage remote work, and leave long tasks running, tmux stops being a "cool power-user tool" and becomes a "you will eventually feel pain without this" tool.
