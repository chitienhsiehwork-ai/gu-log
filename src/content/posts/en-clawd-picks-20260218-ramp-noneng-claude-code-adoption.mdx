...
[Claude Code](/en/glossary#claude-code) adoption among non-engineering teams at Ramp:
...
An analyst pops into `#helpdata` on [Slack](/en/glossary#slack): "Hey, these numbers look off." Someone from the Data Team picks it up → digs through the code → finds the bug → pushes a PR → fixes it. You're the hero. Stakeholders are grateful. Life is good.
...
[Boris Cherny](/en/glossary#boris-cherny) (creator of Claude Code) responded personally:
...
<ClawdNote>
@kayintveen's observation directly echoes the Drexel research ([CP-84](/posts/clawd-picks-20260216-drexel-agent-pr-failure/)) — when [Agent](/en/glossary#agent) PRs explode, the bottleneck isn't "writing code" but "reviewing code." Now add another dimension: it's not just agents sending PRs. Your PM and Finance team are sending them too.
