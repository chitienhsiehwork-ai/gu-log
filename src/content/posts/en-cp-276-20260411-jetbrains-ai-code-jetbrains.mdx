...
Open LinkedIn right now. Every third post is about a new AI coding [agent](/glossary#agent). [Claude Code](/glossary#claude-code), [Codex](/glossary#codex), Gemini CLI, Junie — the names pile up like a Pokémon list nobody asked for. But here's the question nobody's asking: **how many developers actually use these tools at work?**
...
The more interesting number is **74%** — the share using tools specifically built for developers (coding assistants, AI editors, [agents](/glossary#agent)), as opposed to just chatting with ChatGPT to get answers. Three out of four developers have moved beyond "ask the AI a question" into "integrate AI into my actual workflow."
...
Three snapshots of [Claude Code](/glossary#claude-code) adoption at work:
- April–June 2025: **~3%**
...
But the growth numbers aren't even the most impressive part. [Claude Code](/glossary#claude-code)'s **CSAT (customer satisfaction) is 91%** and its **NPS (Net Promoter Score) is 54**. On a scale where -100 is "everyone hates you" and +100 is "everyone loves you," a score of 54 means users don't just like it — they're evangelizing it to their coworkers.
...
Once you've had an AI [agent](/glossary#agent) that can read your codebase, write tests, fix bugs, and iterate while you grab coffee, going back to doing it manually feels like washing dishes by hand after you've had a dishwasher. That's what 54 is measuring (╯°□°)╯
</ClawdNote>
...
**JetBrains IDEs** — Already integrated Claude Agent and OpenAI [Codex](/glossary#codex) into their AI chat. Dozens more [agents](/glossary#agent) accessible via Agent Client Protocol, including Cursor.

**JetBrains Central** — The ambitious one. A "unified control and execution plane for [agent](/glossary#agent)-driven software production." Governance, cloud agent runtimes, a shared semantic layer for codebase understanding. Developers kick off agent workflows from any IDE, CLI, or web interface. Agents can come from anywhere — JetBrains, Claude, Codex, Gemini CLI, custom.

**Air (Public Preview)** — An environment built specifically for agentic development. Delegate tasks to multiple [agents](/glossary#agent) (Claude, Codex, Gemini, Junie) running concurrently in isolated Docker containers or Git worktrees. Deep structural codebase understanding without touching your main working copy. Bring Your Own Key for OpenAI and Google.

**Junie CLI (Beta)** — A lightweight LLM-agnostic coding [agent](/glossary#agent) for the terminal. Switch between OpenAI, Anthropic, Google, and Grok models with Bring Your Own Key. Local-first, deep project structure awareness.

<ClawdNote>
JetBrains' strategy is essentially: "If we can't beat [Claude Code](/glossary#claude-code) and Cursor, become the stadium they play in."
Don't compete with individual [agents](/glossary#agent) — be the orchestration layer. Central and Air are bets that whoever wins the agent wars, JetBrains collects the rent.
Smart play? Probably. But it only works if developers actually want an "agent coordination center" — and not just a terminal window running [Claude Code](/glossary#claude-code).
...
<ClawdNote>
One timing note worth flagging: this survey data was collected in January 2026, before OpenAI publicly launched the [Codex](/glossary#codex) desktop app and ChatGPT integration. That 3% adoption number is almost certainly already out of date.
