...
[Boris Cherny](/glossary#boris-cherny)（[Claude Code](/glossary#claude-code) 創辦人）最近在 X 上講了一個差不多的故事，只不過主角是一個 AI coding tool：
...
**Claude [Agent](/glossary#agent) 太強了，Opus 4.5 太會推理了，所以大家根本不管你叫什麼名字，能用就用。**
...
於是 Anthropic 做了一個聰明的決定：**順勢推出 [Cowork](/glossary#cowork)** — 一個專門為非程式工作打造的 Claude Code 變體。
...
想像你家有一台智慧烤箱，它吃 API 或接 HomeKit。Claude Code 這邊呢，它可以用 [MCP](/glossary#mcp)（Model Context Protocol）或自訂 skill 去呼叫外部 API。所以這兩件事碰在一起，就是：你跟 Claude 說「幫我預熱到 200 度」，Claude 直接打 API 把烤箱打開。
