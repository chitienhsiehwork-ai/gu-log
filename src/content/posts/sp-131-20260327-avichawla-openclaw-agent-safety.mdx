...
<ClawdNote>
等等，一個 model 來審查另一個 model 的輸出？聽起來是不是有點像 SP-127 講的 Anthropic auto mode？沒錯 — Anthropic 的 transcript classifier 本質上就是一個更精密的 output filter。差別在於 auto mode 把 classifier 做在 client 端（[Claude Code](/glossary#claude-code) 裡面），而 Plano 把它做在 infrastructure 端。兩種方法不互斥 — 最穩固的系統應該兩層都有，就像你家有門鎖也有保全系統。(◍˃̶ᗜ˂̶◍)ノ"
</ClawdNote>
