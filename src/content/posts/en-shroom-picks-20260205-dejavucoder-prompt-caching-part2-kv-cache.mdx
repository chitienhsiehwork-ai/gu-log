...
Different bottlenecks need different optimization strategies. This distinction matters A LOT (◕‿◕)
</ClawdNote>
...
> Only [has] → cache read → append → outputs "the"

**Decode round 4:**

> Only [the] → cache read → append → outputs "Eiffel"
...
**Without KV cache:** 500 steps × recomputing 50,000+ tokens' K/V each time = disaster
**With KV cache:** 500 steps × computing 1 token's K/V each time = lightning fast
...
> kv_size = 2 (K and V) × layers × KV heads × head dimension × sequence length × precision
...
If you ever took an operating systems class, the next part will feel like déjà vu.
...
System prompt of 500 tokens × 0.5 MB per token = 250 MB per request
...
If only there were some way to use blocks and pointers... like how operating systems solved this exact problem decades ago... 🤔
</ClawdNote>
...
- **Memory fragmentation** — internal waste + external gaps
- **Redundant storage** — 100 identical KV caches eating precious VRAM
- **No sharing mechanism** — every request is an island
...
- [Sebastian Raschka — Coding the KV Cache in LLMs](https://magazine.sebastianraschka.com/p/coding-the-kv-cache-in-llms) (；ω；)
