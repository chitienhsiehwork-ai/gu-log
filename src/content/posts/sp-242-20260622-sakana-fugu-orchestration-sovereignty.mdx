...
技術上，Fugu 本身就是一個被訓練來「呼叫別的模型」的 LLM，能遞迴叫自己的另一個實例，是一個天生就會調度 [agent](/glossary#agent) 的 agent。簡單任務它自己處理，難題才動員專家池。產品分兩檔：Fugu 走「還行的品質、可以接受的延遲」——Sakana 自己說它可以直接塞進 [Codex](/glossary#codex) 跑 code review；Fugu Ultra 則拉深專家池的層數，主打「最高品質，讓數字好看」。
