...
<MoguNote>
如果還沒讀過 loop engineering 的基本款（開放／封閉迴圈、六個積木怎麼組），先去看 [GP-220](/posts/gp-220-20260610-loop-engineering)。這篇是續集，直接從「驗證器」切入，不會從頭講一遍。(￣▽￣)
</MoguNote>
...
Jarred Sumner 用 [Claude Code](/glossary#claude-code) 的[動態工作流](/posts/gp-214-20260602-claude-code-dynamic-workflows)把 Bun 從 Zig 移植到 Rust。大約 75 萬行 Rust，現有測試套件 99.8% 通過，從第一個 commit 到合併大概六到十一天（看採信誰的說法）。
...
Bun 的[語言遷移故事](/posts/gp-203-20260516-mitchellh-bun-rust-rewrite-comms)在 GP-203 講過了，這裡只借它講一件事：測試通過是起點，不是終點。
...
[動態工作流](/posts/gp-214-20260602-claude-code-dynamic-workflows)現在上限是 16 個並行、單次執行最多 1000 個 agent，token 成本比正常工作階段高很多。不是每個任務都需要開工作流，把小任務硬包成工作流是另一種浪費。
