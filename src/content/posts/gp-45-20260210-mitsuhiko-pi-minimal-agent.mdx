...
那個專案是 [OpenClaw](https://openclaw.ai/)。而 [OpenClaw](/glossary#openclaw) 的引擎，就是 **[Pi](https://github.com/badlogic/pi-mono/)**。
...
現在 coding agent 多到不知道怎麼選——[Claude Code](/glossary#claude-code)、Cursor、AMP、[Codex](/glossary#codex)，隨便挑一個都能體驗 agentic programming。我們之前在 GP-2 比過 Claude Code 跟 Codex，那時候就覺得這市場已經夠擠了。
...
Claude Code 有什麼？Read、Write、Edit、Bash、Glob、Grep、Browser、[Agent](/glossary#agent)、TodoWrite、WebFetch……加上一堆 [MCP](/glossary#mcp) tool，我隨便數都超過十五個。Cursor 更誇張，光是 UI 上看得到的面板就比 Pi 整個 codebase 大。
...
有趣的是，MP-12 裡面 Boris 公開他用 Claude Code 的做法是開五個平行 session 來做不同的事情。Pi 用樹狀結構解決同一個問題，但方向完全相反——Boris 是用「多條平行線」，Pi 是用「一棵會分岔的樹」。殊途同歸，都是在對抗 context 汙染這個 coding agent 的原罪。
...
- [GP-99: Agent Observability：別再盲目微調，用 OpenRouter + LangFuse 看清 AI 的思考軌跡](/posts/gp-99-20260304-nearlydaniel-openrouter-langfuse-agent-observability/)
