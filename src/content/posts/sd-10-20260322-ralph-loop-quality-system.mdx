...
<ShroomDogNote>
我訂了 Anthropic Max plan。每個月付了錢，但 off-peak 時段（半夜、凌晨）的 quota 幾乎沒在用。睡覺的時候，那些 Opus 4.6 的 token 就在那裡靜靜地等到 reset，然後消失。我們付了錢的 token 在那邊浪費掉，不覺得很虧嗎？如果有一個系統可以在我睡覺的時候，自動跑有價值的任務、燒掉那些反正會過期的 quota——那不就是最好的 token 投資嗎？所以 Ralph Loop 不只是一個品質管理系統，它是我們第一個認真設計的 overnight agent orchestration system。讓 agent 在半夜可靠地跑完工作、不會爆掉、醒來能看到成果。
</ShroomDogNote>
