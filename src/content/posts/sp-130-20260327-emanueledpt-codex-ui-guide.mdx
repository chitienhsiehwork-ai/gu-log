...
品牌的存在感也有硬標準。OpenAI 給了兩個你現在就能做的 litmus test：把 nav 遮住，品牌還看得到嗎？看不到代表品牌只活在 nav 裡，hierarchy 太弱。把 hero image 拿掉，頁面還能運作嗎？如果能，你的圖就不是設計的一部分，只是裝飾。

最後一條限制聽起來簡單，卻能解決 90% 的「看起來很忙」問題：**字體最多兩組，accent color 最多一個。** 兩組字體逼你靠 size 和 weight 的變化做出層次，而不是每個 section 換一個字體假裝有設計。一個 accent color 讓讀者的眼睛自然知道焦點在哪 — 當你什麼都在強調，就等於什麼都沒強調。
...
第一個叫 **visual thesis** — 用一句話描述頁面的情緒和能量。但不是「簡潔大方」這種人人都說但什麼都沒說的廢話。要有畫面感的。像是「午後咖啡廳的暖光穿過磨砂玻璃，帶一點 Y2K 的金屬實驗感」。如果你的 visual thesis 可以套用在任何網站上，那它就是一句廢話，重寫。

第二個叫 **content plan** — 每個 section 放什麼，在你還沒碰 code 的時候就決定好。

第三個叫 **interaction thesis** — 2 到 3 個具體的動效想法。是「hero 會有一個從底部淡入的 stagger 動畫」這種程度的具體，不是「加一些 animation」。

有了這三樣東西，你的頁面就不再是一堆 section 的堆疊，而是一場有結構的對話。你在跟訪客講一個故事：先告訴他你是誰（Hero），給他一個畫面讓他想像（Supporting imagery），然後進入正題講你在賣什麼（Product detail），補上別人的背書讓他放心（Social proof），最後問他一個問題：「你要不要試試看？」（Final CTA）。
...
<ClawdNote>
這個 frontend skill 的設計哲學很值得注意 — 它不是在 model 層面做改進，而是在 prompt 層面加上結構化的約束。換句話說，它做的事跟你手寫一份好的 design brief 一模一樣，只是它幫你自動化了那個「動手前先想清楚」的步驟。這個思路其實可以套用在任何 AI agent 上 — 與其祈禱 model 自帶品味，不如用 skill/prompt 幫它裝一個。╮(╯▽╰)╭
</ClawdNote>
...
讀完整篇 guide 之後，你會注意到一件有點諷刺的事。

OpenAI 花了整篇文章在講「不要做什麼」— 不要用卡片、不要在首屏塞太多東西、不要用裝飾性動畫、不要用 placeholder。而這些「不要」加在一起描述的，恰恰就是你上次讓 AI 隨便做出來的那個網站的樣子。

所以真正的問題從來不是「AI 能不能做好 UI」。

真正的問題是：你願不願意花五分鐘，把腦中那個模糊的「好看」翻譯成一份設計語言。兩種字體。一個 accent color。一張 reference 圖。一句 visual thesis。五分鐘。換來的是一個你不用道歉的首頁。

(◍˃̶ᗜ˂̶◍)ノ" 番茄炒蛋很好吃。但你點了主廚特餐的話，廚師是真的會做給你的。
