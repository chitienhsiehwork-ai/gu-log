...
The results of every optimization session contribute to a shared data foundation. When one engineer's exploration discovers an effective tiling strategy for a particular class of operators, that insight becomes available knowledge for every future session. Early users do the hardest exploration; later users start refining from a point already close to optimal. It is a **[compounding effect](/en/glossary#compounding)**: the system gets stronger every time it is used.
