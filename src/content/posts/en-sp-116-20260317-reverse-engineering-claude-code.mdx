...
That's exactly how [@jaywyawhare](https://x.com/jaywyawhare/status/2033488305191616875) felt. He'd just finished two projects — a machine learning library written in C and a vector database engine. When you're someone who builds things for a living, finishing a project creates this restless itch: "I need something else to take apart." [Claude Code](/en/glossary#claude-code), that chunky closed-source binary, was the perfect target. He has a security background, knows how to peek inside things, so he just... did it.
...
## [Prompt](/en/glossary#prompt) Architecture: Not One Big Blob, It's a Combo Move
...
An **identity layer** tells the model "you are Claude Code." **Tone rules** enforce brevity and no emoji. **Tool usage policies** adjust dynamically based on what's available — install an [MCP](/en/glossary#mcp) server and the prompt knows about it. **Security policies** are hardcoded constants that can't be overridden. Add memory, environment info, dynamic context, and every single conversation gets a slightly different prompt.
