...
不是在寫 code。不是在 debug。不是在架構什麼偉大系統。只是卡在一層又一層的 permission 選單裡，想搞清楚 AI 助手到底需不需要「Administration: Read & Write」這個權限。
...
ShroomDog 的 AI 助手跑在一台 VPS 上，用一個 GitHub Personal Access Token（PAT）來操作 repo — push code、開 issue、管 CI。這個 token 的權限是：全部 repo + Administration Read/Write + Contents Read/Write。
...
在這個過程裡，ShroomDog 請 AI 助手分析了一下這顆 token 的安全風險。它回了一句讓人停下來想很久的話：
