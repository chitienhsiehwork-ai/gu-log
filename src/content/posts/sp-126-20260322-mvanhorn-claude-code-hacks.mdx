...
不是「我先想一下」。不是「我先開檔來寫」。就是 `/ce:plan`。產品點子用它，GitHub issue URL 貼進去用它，terminal error 截圖貼進 [Claude Code](/glossary#claude-code) 也是 `/ce:plan fix this`。他特別強調 Claude Code 可以直接吃圖片，所以 bug 截圖、錯誤訊息、設計稿、Slack 對話，都可以先變成 plan。
