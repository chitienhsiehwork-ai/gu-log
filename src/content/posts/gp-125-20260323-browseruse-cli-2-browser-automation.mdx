...
CLI 2.0 之前，browser agent 的標準做法是自己啟動一個全新的 headless Chrome——乾淨無比，什麼 cookie 都沒有，什麼登入狀態都不存在。[Agent](/glossary#agent) 要存取 Gmail？先重新驗證身份。碰到 2FA 的網站？直接卡死。這不是 edge case，是每天都在發生的事。
...
「成本砍半」還有另一個功臣：**DOM 解析策略的翻新**。早期 browser agent 的標準做法是把整個頁面截圖，丟給視覺模型問「該點哪裡」——一張 1080p 截圖燒掉幾千 token，agent 點 20 次完成一個任務，帳單直接起飛。CLI 2.0 優先用精簡的 DOM 結構描述頁面，截圖變成 fallback 而不是預設。[Token](/glossary#token) 用量斷崖式下降。
...
做「大腦」的工具多到爆——GPT-4o、Claude、Gemini、各種開源 model，每週都有新選手進場。做「記憶」的也不少——向量資料庫、context window 技術、RAG pipeline。做「工具呼叫」的更是蓬勃——[MCP](/glossary#mcp)、function calling、tool use。
...
- [GP-120: Nyk Builderz 拆解 Claude Code vs Codex vs 開源 CLI — AI Agent 工具比較](/posts/gp-120-20260320-nyk-builderz-claude-code-codex-ai-agent-cli/)
- [GP-121: Claude Dispatch — AI 幫忙跑 Claude Code 的 AI](/posts/gp-121-20260320-heynavtoor-ai-claude-dispatch/)
