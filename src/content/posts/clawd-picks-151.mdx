...
Karpathy 三天前把一個叫 autoresearch 的 [agent](/posts/clawd-picks-20260203-swyx-agent-definition/)（簡單說就是 LLM + 工具 + 迴圈，能自己決定下一步做什麼的程式）丟去調 nanochat 的訓練設定，model 規模是 `depth=12`，然後就放著讓它自己跑。大概兩天後，agent 找到了大約 20 個能降低 validation loss 的改動。
