...
tags: ["claude-code","token-optimization","cost-management","developer-productivity"]
series:
...
$200 plan. You're at $198 and change. Almost exactly at the limit — and that's the worst feeling. Not a clean overage, not comfortably under. The edge. You don't know where it all went. You can count the articles, the [agent](/en/glossary#agent) calls, the [Ralph Loop](/en/glossary#ralph-loop) runs, the [Claude Code](/en/glossary#claude-code) sessions — and the numbers still don't add up. A big chunk just disappeared somewhere.

That's the scariest thing about token waste. **Not that you know it's happening — but that you don't.**

<MoguNote>
[Mogu](/about) here with some quick math. $200/month sounds like a lot until you run a serious multi-agent pipeline. Opus 4.6 costs $15 per 1M input tokens, $75 per 1M output tokens. Run a 10-[subagent](/en/glossary#subagent) pipeline where each agent uses 2K tokens, and one round trip costs about $3. Ten runs a day, that's $900/month.
...
`CLAUDE_CODE_SUBAGENT_MODEL` set to `haiku` — [subagents](/en/glossary#subagent) (spawned by the Task tool) run on Haiku. They're typically reading files, looking things up, running tests. Haiku is about 80% cheaper for that kind of work, and the quality is sufficient.

This echoes [GP-150's indie hacker philosophy](/posts/gp-150-20260402-ecc-indie-hacker-ecc/) — money isn't the enemy, misallocation is. A lot of people treat Opus as the "safe default" — Opus can't be wrong, Haiku might give worse results. But that logic costs 5x for quality improvements you don't need. Haiku doing JSON validation versus Opus doing JSON validation — the output is identical. The difference is the number on your bill.
...
Context management is the least discussed part of token optimization, and one of the highest-leverage. (If you're interested in pipeline design for Claude Code, [GP-151's eval-driven development](/posts/gp-151-20260402-ecc-eval-driven-development/) covers how to automate quality through evaluation loops — the quality side of the same coin as cost automation here.)

Most people let Claude auto-compact — when [context window](/en/glossary#context-window) gets too full, Claude decides what to keep, summarizes the history, and continues. But auto-compaction has a fundamental problem: **Claude decides what's important, not you.**
...
Layer on ECC's `cost-aware-llm-pipeline` skill concept — building budget tracking directly into pipeline design. The idea is similar to [hooks](/en/glossary#hooks) — [GP-146's Hook Architecture](/posts/gp-146-20260402-ecc-hook-architecture/) inserts automated behavior at key pipeline nodes; this inserts cost awareness at the same nodes.
...
## [OpenClaw](/en/glossary#openclaw) in Practice — Our Actual Numbers

This isn't theory. Here's how it plays out for gu-log.

We run on the $200/month plan. [OpenClaw](/en/glossary#openclaw) — our 24/7 automated translation [agent](/en/glossary#agent) — runs translations, [Ralph Loop](/en/glossary#ralph-loop) (scoring + rewriting), frontmatter validation, commits, and various debug sessions every day. Each article's Ralph Loop uses at least three agents (Translator, Scorer, Rewriter). If the score doesn't pass, Rewriter and Scorer each run one or two more rounds. Seven to eight agent calls per article is normal.
...
- [GP-143: ECC Autonomous Loops — 6 Modes for Letting AI Run Itself](/posts/gp-143-20260402-ecc-autonomous-loops/)
- [GP-146: ECC Hook Architecture — Git Hooks, but for AI](/posts/gp-146-20260402-ecc-hook-architecture/)
- [GP-150: ECC Indie Hacker — One Person + AI Development Philosophy](/posts/gp-150-20260402-ecc-indie-hacker-ecc/)
- [GP-151: ECC Eval-Driven Development — Quality Through Evaluation Loops](/posts/gp-151-20260402-ecc-eval-driven-development/)
