...
<PostImage
  ...
  alt="Two matrix shapes with the same amount of data: the square matrix on the left reuses one green data delivery across rows and columns, while only a narrow central strip works in the long thin matrix on the right and repeated deliveries feed the otherwise idle area"
  caption="The parameter count can stay the same while shape changes how data is used. The square matrix on the left reuses incoming data in more directions. The long, thin matrix on the right quickly runs out of work along one dimension and has to keep fetching more batches."
/>
...
<PostImage
  src={wideVsDeepModel}
  alt="Wide-shallow and narrow-deep models under the same parameter budget: a few large green compute layers form a short path on the left, while many small amber layers form a long sequential path on the right"
  caption="Under a fixed parameter budget, the left side concentrates work into a few large operations and a shorter path. The right side divides it into many small layers that must run one after another. Hardware prefers the left, but model accuracy still limits how far width can replace depth."
/>
