...
更精準地說，這是在增加 [test-time compute](/glossary#test-time-compute)：模型本身不變，但在回答時給它更多時間、更多預算、更多機會反覆檢查。Jarrod 引用 Anthropic Opus 4.6 系統卡裡的 BrowseComp 圖表，重點大概是：Sonnet 4.6 多花十倍 Token 時，分數大約多了十個百分點。
...
自主迴圈開始前，流程會先投入一段設定階段。這個階段的目標不是寫程式，而是讓 Agent 問問題、抓假設、逼出隱藏需求。概念接近 Matt Pocock 那個很紅的 `grill-me` 技能，也像 Jarrod 自己提到的 `/interview` 做法。

這一步聽起來很慢，其實是在省命。

Jarrod 把最終成果想成一棵樹。每個分支都是一次決策。沒有事前釐清時，Agent 會替需求方走其中一條路；那條路可能看起來合理，但不一定是原本腦中想要的版本。
...
Jarrod 的 skill 把這些狀態放進 `.agent/` 目錄底下：
...
這聽起來一點都不酷，甚至會把 AI 展示拉回規格、交接紀錄和流程檢查的世界。但這正是這套做法的工程價值：真正能跑很久的系統，不是因為它完全不需要管理，而是因為它把管理變成流程的一部分。
...
Jarrod 的結論把長跑型 Agent 從魔法敘事拉回工程現場：先問清楚，再分工執行，用新脈絡審查，把記憶寫到模型腦袋外面。聽起來沒那麼性感，但這才像能活過一整晚的系統。
