...
Jarrod 對長跑型 Agent 的第一個拆解很樸素：它有時候有效，不是因為模型突然悟道，而是因為它花了更多 [Token](/glossary#token)。
...
這聽起來一點都不酷，甚至有點像把 AI 展示拖回 Jira 和 README 的世界。但這正是這套做法的工程價值：真正能跑很久的系統，不是因為它完全不需要管理，而是因為它把管理變成流程的一部分。
