...
tags: ["claude-code","multi-agent","ecc","distributed-systems"]
series:
...
今晚 9 點，這個 repo 裡同時跑著 9 個 [Claude Code](/glossary#claude-code) agent。

每個都在寫文章，每個都在讀同一批 source files，每個都想在完成後 commit 和 push。聽起來很美對吧？平行作業，效率翻 9 倍，ShroomDog 可以去睡覺了。

然後 `scripts/article-counter.json` 裡的 GP 號碼，開始長出不同的分叉。

兩個 agent 讀到的 `GP.next` 是同一個數字，都以為自己拿到了那個號碼的所有權，愉快地繼續往下寫。等到要 merge 的時候，才發現世界上同時存在了兩篇 GP-152。
...
## 問題的本質：Context 是共享的，但 [Agent](/glossary#agent) 是平行的
...
<MoguNote>
這讓我想到 [GP-132 講的 multi-agent harness design](/posts/gp-132-20260327-anthropic-multi-agent-harness-design/)。Anthropic 的設計哲學是：orchestrator 要控制資訊流，不是讓每個 subagent 自己去抓全部資料。這兩篇在說同一件事的不同面向。
...
`scripts/article-counter.json` 存著下一個 ticket ID（例如 `GP.next: 152`）。正常流程是：讀 counter → 拿號碼 → 寫文章 → 更新 counter。
...
今晚最有效的根本解法：用 `git worktree` 讓每個 agent 在完全獨立的 branch 工作。Agent A 在 `/tmp/worktree-gp-151`，Agent B 在 `/tmp/worktree-gp-152`，彼此的 file system 完全不重疊。
