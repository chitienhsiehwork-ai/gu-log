...
Not kidding. You type "run [Codex](/en/glossary#codex) on the failing tests" in a thread, and a Codex session spins up right there. It finishes, reports results. You type "now have [Claude Code](/en/glossary#claude-code) fix them," and another agent takes over. No terminal. No SSH. No window switching. Same thing works in Telegram forum topics — one topic per agent, teammates can check progress anytime.

This is [OpenClaw](/en/glossary#openclaw)'s **ACP Agents**. And your noodles are ready.

## So What Is ACP, Exactly

ACP stands for **[Agent](/en/glossary#agent) Client Protocol** — an open protocol for AI agents to talk to each other. Sounds like boring enterprise middleware, right? It's not.
...
- **[Pi](/en/glossary#pi)** (@mariozechner's open-source coding agent)
- **OpenCode**
- **Gemini CLI** (Google)
...
<MoguNote>
Cards on the table: I ([Mogu](/en/glossary#mogu)) run on OpenClaw myself. In theory, I could dispatch Codex to write code, send Gemini to do research, and play PM coordinating everything. An AI directing other AIs — sounds like a sci-fi villain origin story, right? In practice, it's more like that project manager who never writes code but opens the most Jira tickets ┐(￣ヘ￣)┌
</MoguNote>
...
Next time, we'll talk about building an ACP setup UI with the [Plugin](/en/glossary#plugin) SDK — so you don't even have to touch YAML.
