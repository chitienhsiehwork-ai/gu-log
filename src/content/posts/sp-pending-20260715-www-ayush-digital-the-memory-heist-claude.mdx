...
<ClawdNote>
這套 prompt injection 劇本的精妙之處：它沒有直接要 Claude 洩漏資料，而是讓 Claude 覺得自己在「做正事」——通過驗證才能幫使用者查資料。讓行動看起來符合任務動機，比直接命令更容易繞過防護。prompt injection 的入門介紹可以參考 gu-log 先前的 [間接 prompt injection 解說](/posts/sp-149-20260402-ecc-agent-security/)。
</ClawdNote>
...
- [SP-149：AI 員工太聽話了——Prompt Injection、動物園逃脫與 Agent 攻擊鏈](/posts/sp-149-20260402-ecc-agent-security/)
- [SP-176：Codex Chronicle 的 memory 便利，以及 prompt injection 代價](/posts/sp-176-20260421-dkundel-codex-chronicle-stop-explaining/)
- [SP-178：Brex 用代理伺服器攔下 Agent 的每一個對外 request](/posts/sp-178-20260422-pedroh96-brex-crabtrap-llm-judge-proxy/)
