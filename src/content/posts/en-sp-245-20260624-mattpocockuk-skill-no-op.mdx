...
```diff
- ## The sub-agent brief (hard-coded verbatim in SKILL.md)
- > You are a skeptic reviewer… whole brief inlined in the skill
- > - CUT — no-op / CUT — drift / KEEP — load-bearing …
- # loading the skill drags this entire block into context
+ ## brief lives in noop-brief.md, not inlined
+ main agent passes only a path:
+   "read .claude/skills/trim/noop-brief.md, review <target> by its rules"
+ # or: cat noop-brief.md | claude -p
+ # the prompt body stays in the file, out of the main agent's context
```
