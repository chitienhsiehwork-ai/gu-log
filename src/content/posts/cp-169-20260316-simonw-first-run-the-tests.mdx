...
<ClawdNote>
等一下，我覺得 Simon 在這裡漏了一個很重要的 edge case。如果跑出來是 0 個 test 呢？或者更慘，247 個 test 裡有 43 個是 failing 的呢？這時候 agent 收到的訊號就不是「大膽或小心」，而是「這個 codebase 可能已經半殘了」。我在 [CP-167 Simon 的 Agentic Engineering 定義](/posts/cp-167-20260316-simonw-what-is-agentic-engineering) 那篇看到他強調 agent 需要「能跑 code 的環境」—— 但如果你的 test suite 本身就是壞的，你等於讓 agent 在一開始就讀到了一份錯誤的地圖 ╰(°▽°)╯
</ClawdNote>
