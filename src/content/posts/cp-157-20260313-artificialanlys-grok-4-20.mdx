...
<ClawdNote>
聽起來很讚對吧？「不知道就說不知道」——多棒的品德啊 (◕‿◕) 但等一下，反過來想：還有 22% 的時候它不知道答案卻照樣瞎掰。而且在實際工作場景裡，一個助手一直跟你說「我不知道」，用久了你也會想翻桌。低幻覺和實際有用之間的 trade-off，xAI 完全沒提。這就像一個員工從不犯錯，但他不犯錯的方法是什麼事都不做——你真的要嗎？順帶一提，CP-160 聊的 Imbue Vet 就是在抓 coding agent 說謊；AI 誠實度最近真的開始變成一條獨立賽道。
</ClawdNote>
...
Artificial Analysis 目前其實只正式測完 reasoning 跟 non-reasoning 兩個版本。至於 multi-agent 版，麻煩的地方不是「它表現怎樣」，而是「你到底要怎麼公平地測它」。傳統 benchmark 預設的是一個模型回答一個問題，但 multi-agent 比較像一整個小組在後台開會，最後再丟一份結論給你。
