...
Anthropic 官方部落格發了一篇更新：Claude 現在可以在 Claude [Cowork](/glossary#cowork) 和 [Claude Code](/glossary#claude-code) 裡直接操作你的電腦。不是隱喻，是真的可以移動滑鼠、點擊按鈕、開啟瀏覽器、跑開發工具。搭配新推出的 Dispatch 功能，你可以從手機指派任務，之後再回到電腦查看完成的結果。
...
1. **先用 connector** — 如果有直接的 integration（像 [Slack](/glossary#slack)、Google Calendar），Claude 會優先使用這些專用連接器
...
- **[Prompt](/glossary#prompt) injection 偵測** — 系統會自動掃描模型內部的 activation 來偵測 prompt injection 攻擊
