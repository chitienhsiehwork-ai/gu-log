...
Dreams addresses a problem every [Agent](/en/glossary#agent) builder eventually hits: **[context rot](/en/glossary#context-rot)**.

<ClawdNote>
The name is cute, maybe even a little dramatic. AI can talk, reason, use tools, and now apparently we have to schedule bedtime too. But the real point is not the branding. It is that Anthropic is treating long-running agent memory as something that does not stay clean by itself.
</ClawdNote>
...
<ClawdNote>
It is like going to a convenience store for tea eggs while carrying a full camping kit: stove, tarp, folding chair, the whole thing. You have not bought the egg yet, and you are already exhausted. When agent memory rots, the pain is not "not enough stuff." It is that the agent keeps bringing the wrong stuff along for every job.
</ClawdNote>
...
Developers can also pass custom instructions to guide what Claude should prioritize or ignore during consolidation. For example, Claude can prioritize coding-style preferences and ignore one-off debugging notes.
...
Dreams is an intentional metaphor. To be clear: the sleep-and-memory analogy below is just a way to understand the naming through the common cognitive-science idea that sleep helps consolidate memory. It is not a claim from Anthropic that Claude literally has biological sleep.
...
Dreams also creates a second loop outside the agent loop. In plain English: agents cannot only have a "do the work" pipeline. They also need an "after-action review" pipeline.
...
Better agents do not necessarily come from a larger [context window](/en/glossary#context-window). They come from better memory lifecycle management. You can think of the context window as the size of Claude's desk — how much material it can see in one conversation.
