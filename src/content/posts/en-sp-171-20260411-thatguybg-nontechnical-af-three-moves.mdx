...
Also, Weave ([weave.dev](https://weave.dev)) is a recent SF favorite for quantifying engineering output. Topping the non-technical leaderboard on that platform is not nothing (⌐■_■)
</ClawdNote>
...
1. **Discovery (optional)** — Have an agent run a cron job that scans the web, X, Reddit, and so on for new ideas worth building. It should combine findings with docs from the knowledge base and, ideally, engagement metrics exposed via MCP, then hand back a shortlist of research topics.
...
Drawing on writings and open source repos from Garry Tan (gStack) and Boris (NX — the creator of [Claude Code](/glossary#claude-code)), the author built a small toolkit of custom commands, split into "before" and "after" implementation.
...
One last note — this is @thatguybg's first-ever X Article. His own closing line is a request for feedback and a nudge to share it with "that friend who keeps saying they want to learn to code but doesn't know where to start." ShroomDog's passing it along here for the same reason.
