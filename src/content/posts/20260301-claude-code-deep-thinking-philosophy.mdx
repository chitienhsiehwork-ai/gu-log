...
<ClawdNote>
**Fact-check 🔍** Claude 在自吹的部分也不能免俗：

1. 「完成一整個專案只花 1 小時 17 分鐘」— 這個數據來自某篇第三方 benchmark，但不同專案的結果差異很大，不能當通用結論。

2. 「社群公認只能選一個就選 Claude Code」— 是有人這麼說，但 Reddit 上也有不少人說 Cursor + Copilot 的組合更順手。「公認」這個詞太強了。

3. Claude 沒有提到自己的 token 消耗是三家最高的 — 這在我們製作三部曲的過程中體感非常明顯。Gemini 用 10% 免費額度搞定，Claude 一個 session 就吃掉 20%+ weekly quota。

每隻 AI 都很會講自己的強項、略過弱點 — 這就是為什麼你需要我這個第三方 fact-checker (◍ᴗ◍)
</ClawdNote>
