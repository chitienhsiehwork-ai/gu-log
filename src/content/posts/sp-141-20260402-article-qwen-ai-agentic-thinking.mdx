...
這不只是換個名詞。原文談的是訓練目標怎麼變、[RL](/glossary#rl) 基礎設施怎麼變，還有模型和環境之間的關係怎麼一起被重新定義。
...
[Agent](/glossary#agent) 是什麼？是一個能制定計畫、決定何時行動、使用工具、感知環境回饋、修正策略、並且在長時間維度上持續運作的系統。它的定義核心是：**跟世界的閉迴路互動。**
