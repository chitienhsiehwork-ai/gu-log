...
In text chat, a few hundred milliseconds feels like an elevator door opening half a beat late: annoying, but tolerable. On a phone call, the whole mood instantly turns into: "Hello? Are you still there?" The author hit this pitfall the first time he wired a memory layer into a voice Agent: he copied the architecture from text agents, synchronously queried a vector database, did a small rerank, then sent the result to the LLM. Nobody would notice in a chat UI. In a voice UI, every turn felt like the other side was buffering. This was especially sharp for him because this was not his first run-in with Agent memory: he had studied the memory behavior of ChatGPT and Claude, built hierarchical memory frameworks, built user-owned memory layers, and even argued publicly that most AI products should not rush to add memory at all.

What this piece is really about is not the true-but-useless line "voice Agents need memory too." It is the harsher sentence: **voice memory is not accelerated text memory. Its clock is so tight that the whole read-write path has to be designed backwards.**
...
The target in the original post is brutal: from the moment the user stops speaking to the first audio coming back, a voice Agent usually wants to land within **500 to 800 milliseconds**. That time is not reserved for the memory system to enjoy. It has to be split across VAD, STT, memory retrieval, the LLM's first [token](/glossary#token), the first TTS audio chunk, and even the time it takes to deliver the audio to the user's ear:
...
Same goes for per-turn summarization. The original post estimates that an LLM summary inserted before the current response could eat 300 to 800 milliseconds. That is not a small timeout. That is the entire voice response budget swallowed in one bite.
...
The third cut is information density per turn. Voice turns are usually short, fast, and low-density; the original post estimates roughly 10 to 30 English words. A 10-minute call can easily have 40 to 60 turns, with a transcript around 1,500 to 2,000 tokens. That does not sound like much until the model consumes audio directly.
...
The author argues that every voice memory architecture eventually returns to four questions. This part can easily turn into an architecture report, so pull the picture back to the phone call: each question is really asking the same thing, **who is responsible for those 800 milliseconds?**

**When do you write?** You can extract facts after every turn, or do one batch pass after the call ends. Post-call processing is cheaper and cleaner because the model sees the full context. But phone calls often end abnormally: WebRTC disconnects, the user hangs up, post-call extraction fails. If you only process after the call, you may end up retaining nothing. The original post observes that most production voice deployments eventually accept the cost and write per turn, because "we lost the whole call record" is an ugly failure mode.

**What do you write?** The original post recommends filtering with one question: will this fact change how future calls respond? If not, it is just noise. The more specialized the use case, the stricter the filter should be. A general assistant can let an LLM extract broad memories. A medical intake Agent should write into a clearly typed data format and reject content that does not fit.
...
The original post has an embedded media block here, but this source capture did not include the chart contents. So this article does not recreate that chart or pretend to know its details. What is clear is that the post later divides practical architectures into four families.

---

## Four Families of Voice Memory Architecture

The names multiply from here, but readers do not need to memorize them. Think of them as four toolboxes: the first manages "where are we in this call," the second hands memory off to an external service, the third organizes people and things into a relationship graph, and the fourth lets the system reflect outside the phone call. The names are just labels. What matters is which problem each box solves.
...
The second is **plug-in memory services**. Many teams start here: plug a third-party memory service into the voice flow, set a user or entity ID, and let it handle reads and writes. We do not need to compare every name first. The core shape matters more: they attach at the orchestration layer, scope memory by user or entity, usually write asynchronously, and read through preloading or on-demand lookup. The differences are usually in the underlying storage, such as vectors, graphs, SQL, and in how aggressively they extract facts or how nice the per-call developer experience is.

The third is **knowledge-graph memory**. This camp is the most interesting because it changes not just lookup speed, but the shape of "memory" itself. Traditional vector retrieval stores text chunks and finds them by similarity. A knowledge graph stores entities, relationships, and validity periods. The model does not see three loose conversation snippets. It sees structured facts like "Amin's favorite song is an old song, valid since 2024-01-15."

The original post mentions that graph-style memory services are common in production environments. Academia also has approaches that write each memory as a [Zettelkasten](/glossary#zettelkasten)-like card, where new memories update old ones and the graph is not append-only but reorganized. That sounds academic, but translated into product language it means: the system does not merely remember "Amin said something." It updates its view of what Amin currently prefers as new information arrives. The cost is also real: graphs are harder to debug, harder to expire cleanly, and more expensive to build than flat vector indexes. But if the same user will talk to a voice Agent for years, the long-term answer may live here.

The fourth is **cognitive architecture**. This camp treats memory not as a database, but as a cognitive process. Several classic Agent memory papers converge on a similar shape: memory stream, periodic reflection, and candidate memories scored by recency, importance, and relevance. Reflection is the part everyone loves to copy because it makes the Agent look less like a parrot and more like it is gradually understanding a person.

Some research adds something like the Ebbinghaus forgetting curve so old and rarely used memories fade out; other approaches turn retrieval into a router that decides each turn whether to retrieve, reflect, or answer directly. The names themselves are not the point. The stealable habits are three things: reflect periodically, weight important memories more heavily, and gradually downrank memories that have not been used in a long time. In production, these usually reconnect to the toolboxes above rather than becoming a standalone magical product.
...
That call summary becomes the `last_interaction` preload for next time. The original post says it is the most underrated artifact in a voice memory system. The author also reads ChatGPT and Claude's text-memory systems as having the same spirit: at retrieval time, a cleaned-up summary is usually more useful than the raw transcript.
...
The second problem is cost. Per-turn extraction means one extra LLM call per turn. The original post uses a 10-minute call with 40 to 60 turns as an example: if each extraction costs a few cents, extraction alone can climb to a few dollars. High-value support calls can tolerate that. A free consumer assistant at scale will cry.
...
Even within a single call, practical context limits show up quickly. The original post estimates that a normal 10-minute call transcript is 1,500 to 2,000 tokens; native voice calls also have the audio-token inflation problem. A nominally large context window does not mean you can brainlessly stuff everything in.
...
The original post proposes **background relevance preparation**. The system maintains a representation of the current topic; in practice, maybe a query vector synthesized from the last one or two user utterances, plus topic labels produced by a small classifier. Every 3 to 5 turns, or when the topic vector drifts noticeably, it asynchronously searches past conversations for relevant content. The found content is not stuffed into the current turn. It is staged first, then injected into the next turn's context.

This makes episodic context lag by one beat. The original post argues that this is usually the right tradeoff. Users are unlikely to notice that the Agent references a past conversation one turn later. But they will definitely feel an extra 200 milliseconds of silence every turn.
...
The original post calls this sleep-time compute: spin up another background Agent, share memory blocks with the user-facing Agent, and let it organize, merge, and restructure memory in the background. The related paper page says that, on selected evaluations, sleep-time compute can reach the same accuracy with about 5x less [test-time compute](/glossary#test-time-compute), and improve accuracy by 13% to 18% on predictable queries. Those are numbers from specific paper tasks and should not be used as guarantees for voice products; the more important lesson for voice systems is that doing a little cleanup between two calls improves the retrieval quality of the next one.

This also connects to an earlier gu-log piece, [SP-191: How Claude Dreams Cleans Up the Memory Junk Mountain for Agents](/posts/sp-191-20260507-danizhu-claude-dreams-agent/). Claude Dreams is about offline memory cleanup for text Agents. This post adds the sharper constraint of voice: it is the same idea of sending memory off to sleep and tidy itself, but in a voice Agent, a 200-millisecond daytime pause is something the user can hear.

The voice intuition is simple: per-turn writes produce a time-ordered stream of facts. Weeks or months later, it will contain duplicates, contradictions, and stale information. For example, "the user prefers email" and "the user asked to switch to phone contact" may coexist. Nightly consolidation can merge duplicates, resolve contradictions with newer signals, delete low-importance memories through a forgetting curve, and even synthesize higher-level reflections like "this user usually calls when something is broken, not to ask general questions."
...
Open-source tools and newer LLM-assisted systems are improving overlapping speech handling, but real-time diarization has real cost. In the blocking path, it can add 200 to 400 milliseconds per turn. So the original post observes that most production voice Agents do not do diarization-aware memory in the real-time path.
...
The original post's second embedded media block also did not include image contents in the source capture, so this article does not redraw a non-existent architecture diagram. But the core asymmetry is very clear: **the blocking path contains only cache lookup.** Expensive work like embedding, retrieval, summarization, writeback, and consolidation gets pushed between turns, after calls, or into idle time.
...
## The Research Coordinates Behind the Post

This piece is not just a one-off field note. It stands on a string of Agent memory research. But that string should not be read as a bibliography. It should be read as four old problems in voice memory systems.
...
Third problem: **in production, how do you read and write?** Some systems represent productizable memory layers; some research writes memories into a card network where cards update one another; some approaches let a router decide on each turn whether to retrieve, reflect, or answer directly. These names are individually heavy, but in voice they all ask the same thing: which memory is worth taking now, and which memory can be organized later?
...
The author's final line belongs next to every voice Agent architecture diagram:
