...
[Boris Cherny](/glossary#boris-cherny) 在 X 上整理了 15 個他最喜歡、也最常用的 [Claude Code](/glossary#claude-code) 隱藏或較少人注意到的功能。不是那種「官方文件寫了但沒人看」的東西，是他自己每天都在用、覺得大家都還沒注意到的招數。
...
- `/loop 30m /slack-feedback` — 每 30 分鐘自動根據 [Slack](/glossary#slack) 回饋開 PR
...
## [Hooks](/glossary#hooks) — 在 [Agent](/glossary#agent) 生命週期裡埋自訂邏輯
...
## [Cowork](/glossary#cowork) Dispatch — 不寫 code 的時候也能用

Boris 說他每天都用 Dispatch。它是 Claude Desktop app 的安全遠端控制，能存取你的 [MCP](/glossary#mcp)、瀏覽器跟電腦。用來幹嘛？回 Slack、看 email、管理檔案 — 不寫 code 的時候也能讓 Claude 幫忙。
