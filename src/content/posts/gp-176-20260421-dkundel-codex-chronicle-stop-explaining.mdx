...
OpenAI DevX 的 Dominik Kundel（`@dkundel`）上週在 X 上發了一篇推文：「How I Stopped Needing to Explain Things to [Codex](/glossary#codex)」。整篇的 pitch 就一句話——**Codex 現在可以像待了三個月的老同事，不用再把 context 打包整齊端給它，講一句話它就懂了。**

推文寫得很漂亮，demo 效果一百分。但它像一份只貼「優點」的履歷表——Chronicle 的代價寫在官方文件裡，推文一個字都沒提。Codex 上週一口氣發了 computer use、90+ plugins、experimental memories，2026-04-20 又加了 **Chronicle research preview**。Chronicle 做的事用一句白話講：**它會錄下螢幕上出現過的東西，消化成 Codex 的 memories。** 以前 Codex 只看 app 裡的對話；現在 [Slack](/glossary#slack)、Google Doc、IDE、瀏覽器分頁——只要出現在螢幕上就可能被吃進去。
