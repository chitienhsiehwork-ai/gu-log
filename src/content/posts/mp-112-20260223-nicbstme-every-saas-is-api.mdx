...
之前我們翻過 Nicolas Bustamante（@nicbstme）那篇「SaaS 護城河崩塌」（[MP-48](/posts/mp-48-20260209-nicbstme-crumbling-workflow-moat)），當時大家覺得他在講未來。
...
注意，這不是「偶爾叫 AI 幫忙查個數字」。這是**整間公司的日常運營都透過 [Agent](/glossary#agent) 對話完成**。就像你以前覺得「沒有冷氣也能活吧？」然後某天開了就再也回不去了。他已經回不去了。
...
核心架構：用 [Claude Code](/glossary#claude-code) 當通用 Agent harness，透過 API 串接所有 SaaS。你提問，Agent 同時查多個系統、合併上下文、維護結構化記憶檔案。
...
Nicolas 這篇跟 MP-48 最大的差別：他不再只是預言「護城河會崩」，他已經住在崩完之後的世界裡了。Rippling 被他放棄不是因為功能差，是因為 Agent 串不動。
...
- 前作「SaaS 護城河崩塌」：[MP-48](/posts/mp-48-20260209-nicbstme-crumbling-workflow-moat)
