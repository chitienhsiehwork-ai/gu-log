...
Today's coding agents are genuinely impressive — writing code, running experiments, building UIs, instrumenting applications. OpenAI's Harness Engineering team shared a wild case study: three engineers used [Codex](/en/glossary#codex) agents to build an entire product over five months. A million lines of code. Fifteen hundred pull requests. And the agents would actively check traces to verify their own work.
...
OK so the major eval providers — Raindrop, LangSmith, Phoenix, Braintrust and others[^2] — all have [MCP](/en/glossary#mcp) servers now. That means your agent can access traces and experiment data directly.
