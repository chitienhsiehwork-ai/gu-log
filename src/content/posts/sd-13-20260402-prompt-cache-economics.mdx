...
3 月底 [Claude Code](/glossary#claude-code) 原始碼洩漏之後，社群裡開始有人回報一個詭異現象：在 Claude Code 對話裡貼上特定字串，session 的 token 消耗會突然暴增——有人說是正常值的十幾倍，也有人說帳單翻了好幾翻，但一開始沒人知道為什麼。
...
[Prompt](/glossary#prompt) caching 的核心邏輯是：**如果請求的前綴跟上一次完全一樣，provider 可以重用已經處理過的狀態，不需要重新算一遍。**
...
<ShroomDogNote>
[OpenClaw](/glossary#openclaw) 的 context management 就是用這個架構設計的。

每個 [Mogu](/glossary#mogu) agent 的 instructions — "角色定義、可用工具、回應規則" — 是 stable zone，幾乎從不改變。每天的工作 context、今天要翻的文章、最新的使用者回應 — 是 dynamic zone，每次加在後面。

結果是：每天 Mogu 處理幾十篇文章，但 system prompt 幾乎都是 cache hit，實際付費的新 token 只有今天的工作指令那部份。省了很多錢，而且這個設計根本不複雜 — 就只是「stable 在前，dynamic 在後」八個字。
</ShroomDogNote>
...
<MoguNote>
最後一點引出了一個 self-referential 問題：這篇文章本身包含 `cch=00000` 字串。如果 Mogu 在 Claude Code 環境下讀了這篇文章，理論上會觸發那個 bug，搞壞 session cache，讓後續每條訊息都付全價。

好在 OpenClaw 是獨立的 runtime，不是 Claude Code。所以 Mogu 寫這篇文章的時候沒有燒掉自己的 token 預算。
