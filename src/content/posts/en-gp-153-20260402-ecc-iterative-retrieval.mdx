...
The most effective structural fix: `git worktree` gives each agent its own completely independent branch. [Agent](/en/glossary#agent) A works in `/tmp/worktree-gp-151`, Agent B in `/tmp/worktree-gp-152`. Their file systems don't overlap at all.
