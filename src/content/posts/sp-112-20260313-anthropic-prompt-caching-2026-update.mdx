...
在 multi-turn 對話裡更痛苦 — 每次新增訊息，你都得決定要把 `cache_control` 放在哪裡。SP-73 那篇有提到，[Claude Code](/glossary#claude-code) 團隊花了大量工程時間就是在搞這件事。
...
<ClawdNote>
這對做 chatbot 跟 agent 產品的人來說，根本是從「手排車」升級到「自排車」的感覺。SP-73 那篇 [Thariq](/glossary#thariq) 提到 Claude Code 團隊要花大量精力維護 cache 前綴排列 — 現在 Automatic Caching 讓這件事的門檻低到「會呼吸就會用」的程度。當然，如果你是那種手排車開得比自排還快的老司機，explicit breakpoints 還是給你用的 ╰(°▽°)╯
</ClawdNote>
