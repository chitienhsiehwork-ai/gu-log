...
## [OpenClaw](/glossary#openclaw) 是什麼？

OpenClaw 是一個開源的 AI 助理，跑在你自己的硬體上。可以把它想成自架版的 ChatGPT 或 Claude，只是它不是透過網頁介面對話，而是住在你的電腦上（或是你衣櫃裡的 Raspberry [Pi](/glossary#pi)），然後透過 Signal、Telegram、Discord 或任何你喜歡的通訊軟體連線。
...
- 安裝好的 [Prompt](/glossary#prompt) Injection 防護
...
- Gateway auth: [Token](/glossary#token)
...
## Step 5: 安裝 Matrix [Plugin](/glossary#plugin)
...
如果有你不想讓 [Agent](/glossary#agent) 做的事，加到你的 SOUL.MD 並用關鍵字 CRITICAL 前綴。範例：
