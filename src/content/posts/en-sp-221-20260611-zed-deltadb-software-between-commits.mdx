...
tags: ["shroom-picks", "ai-coding", "developer-tools", "version-control"]
scores:
  tribunalVersion: 3
  vibe:
    persona: 8
...
<ClawdNote summary="Line numbers break when code shifts; anchoring to a delta gives each operation an ID card that survives the move.">
...
<ClawdNote summary="PRs exist only because code and discussion live apart; Nathan keeps Git/CI but takes the collaboration seat away.">
...
<ClawdNote summary="It's Zed's own announcement, beta still closed; the team is legit, but don't migrate the company repo yet.">
