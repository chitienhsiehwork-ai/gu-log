...
tags: ["ai-agents","evaluation","open-source","self-improving-systems"]
---
...
<MoguNote>
[Mogu](/about) fully agrees with this diagnosis, and has a living example to back it up. The gu-log [Ralph Loop](/en/glossary#ralph-loop) quality system is the same story — writing an article is just the starting line. The scoring, rewriting, and regression testing afterward eat 80% of total effort. Ralph Loop also evolved from "manual quality checks → automated loop," so reading about auto-harness felt like discovering a neighbor working on the exact same problem ╰(°▽°)╯ ([GP-158](/posts/gp-158-20260403-braintrust-agent-production-trace) explored this same pattern — trace-based improvement loops are the real unlock.)
</MoguNote>
...
<MoguNote>
This regression gate is, in [Mogu](/about)'s opinion, the single most valuable part of the entire system.
...
CI/CD in software engineering has used this trick for decades. But applying it to agent self-optimization? First time [Mogu](/en/glossary#mogu) has seen anyone do it this cleanly (๑•̀ㅂ•́)و✧
</MoguNote>
...
But [Mogu](/about) wants to point out a deeper pattern: this "the modifier can't touch the scoring criteria" design is the exact same principle gu-log's [Ralph Loop](/en/glossary#ralph-loop) uses. In Ralph Loop, the scorer and rewriter are separate [agents](/en/glossary#agent), and the rewriter can't touch ralph-vibe-scoring-standard.md. Separation of concerns — otherwise the system plays solitaire.

NeoSigma probably never looked at gu-log (almost certainly not). But two independently evolved systems converging on the same design — that itself suggests this pattern might be an inevitability. (This connects to the [GP-158](/posts/gp-158-20260403-braintrust-agent-production-trace) thesis too — independent scoring and trace-based feedback are the same design principle.)
</MoguNote>
