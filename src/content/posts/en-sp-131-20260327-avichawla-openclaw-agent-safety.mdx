...
The config is equally straightforward — tell Plano where the filter service lives and which listener to attach it to:
...
The agent has no idea Plano exists — it thinks it's talking to a standard OpenAI-compatible API. That's the hallmark of good design: the protected party doesn't need to know the protection mechanism exists.
...
listeners:
  - type: model
    ...
    port: 12000
    input_filters:
      - content_guard      # block dangerous prompts
      - pii_anonymizer     # de-identify personal data
      - query_rewriter     # rewrite queries
    output_filters:
      - pii_deanonymizer   # restore personal data
```
