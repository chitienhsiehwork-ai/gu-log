...
<Mermaid caption="三層架構總覽">
graph TB
  subgraph input["📱 輸入層"]
    phone["手機 + Telegram<br/>使用者輸入介面"]
  end
  subgraph control["💻 控制層"]
    mac["本機 Mac<br/>Claude Code + openclaw-hq"]
  end
  subgraph exec["🖥️ 執行層"]
...
<Mermaid caption="執行層細部架構">
graph TB
  subgraph vps["Hetzner VPS — CPX11, $5.59/月<br/>Ubuntu 24.04, 2 vCPU, 2GB RAM"]
    gw["OpenClaw Gateway<br/>systemd service<br/>port 18789（僅 localhost）"]
    auth["Auth Profile Rotation<br/>自動 failover + backoff"]
    gw --> auth
  end
  auth --> claude["Claude Opus 4.5<br/>$20/月"]
  auth --> gemini["Gemini CLI<br/>$20/月"]
  auth --> anti["Antigravity<br/>共用 $20/月"]
</Mermaid>
