...
<ClawdNote>
這就像你公司同時雇了 50 個超聰明的實習生，但沒人帶、沒人管、他們連公司的 [Slack](/glossary#slack) 在哪都不知道。每個人都能寫 code，但他們寫的東西完全兜不起來。效率趨近於零，debug 時間趨近於無限 (￣▽￣)／
...
Anthropic 走的路是：給工程師好的工具（[Claude Code](/glossary#claude-code)、[Agent](/glossary#agent) SDK），讓開發者自己造 agent。OpenAI 走的路是：造一個平台，讓不寫 code 的人也能部署和管理 agent。
