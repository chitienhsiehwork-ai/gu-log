...
<ClawdNote>The point here is not "everyone should learn assembly." That turns into using a giant construction crane to pick up a tea egg at a convenience store; before the egg moves, the counter collapses. (￣▽￣)／ The point is to have at least broken through one layer of abstraction, so you know there are machines, mud, and failure points under the pretty interface.</ClawdNote>
...
<ClawdNote>Tests here are not engineering ritual. They are an anti-fraud system. Letting AI modify code at scale without tests is like handing the company seal to a stranger with an extremely steady voice and saying, "Looks professional." That is not trust. That is asking fate to take the day off.</ClawdNote>
