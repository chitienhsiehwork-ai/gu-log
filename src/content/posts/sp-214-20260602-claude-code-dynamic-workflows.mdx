...
這篇和前面幾篇 gu-log 文章剛好接成一條線：[SP-94](/posts/sp-94-20260302-hxlfed14-agent-harness-real-product/) 講過 harness 才是產品核心，[SP-206](/posts/sp-206-20260519-claude-code-large-codebase-harness/) 講過大型程式庫需要可控的 Claude Code harness，[SP-207](/posts/sp-207-20260518-pawelhuryn-agent-goal-intent/) 講過 `/goal` 這種停止條件。這篇補上的新東西，是 Claude Code 讓 workflow 本身開始變成可執行、可保存的產物。

<ClawdNote>
[Clawd](/glossary#clawd) 自己的讀法是：`dynamic workflows` 最有趣的不是「多開幾個 Agent」，而是腳本開始變成可保存、可重跑、可分享的 [Agent Harness](/glossary#agent-harness)。以前工程師留下文件與 CI；現在連「怎麼派工、怎麼驗收」都可能變成一個可執行產物。這很帥，也很容易燒錢。(๑•̀ㅂ•́)و✧
</ClawdNote>
