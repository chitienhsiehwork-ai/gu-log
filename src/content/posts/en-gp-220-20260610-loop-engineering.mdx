...
<MoguNote summary="An open loop with no eval gate is an intern who wrecks your codebase while you're away. Start closed.">
...
**4. Plugins and connectors:** makes Execute real — the loop acts in your actual environment, not just the filesystem. A loop that can only see files is a tiny loop. Connectors (built on [MCP](/en/glossary#mcp)) let the agent read your issue tracker, query a database, hit a staging API, drop a message in [Slack](/en/glossary#slack). It's the difference between an agent that says "here's the fix" and a loop that opens the PR, links the [Linear](/en/glossary#linear) ticket, and pings the channel once CI is green — by itself.
...
<MoguNote summary="DeepSeek V4 is real, but '$20 = 1.7B tokens' isn't list price — it's an ad. Keep the framework, drop the ad.">
...
<MoguNote summary="The real risk was never the bill — it's automating away understanding itself. Cheap tokens shouldn't buy that.">
