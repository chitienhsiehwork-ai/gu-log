...
When [Claude Code](/glossary#claude-code)'s 512K-line source code [leaked last week](/posts/sp-139-20260401-article-claude-code-npm-source-map), most people fixated on KAIROS (an unreleased always-on [agent](/glossary#agent) mode), internal model codenames like Capybara, or the legendary `print.ts` — one function, 3,167 lines, 12 levels of nesting.
...
claw-code — a Python clean-room rebuild of Claude Code made using [Codex](/glossary#codex) — hit 75K+ stars after the leak. It's testing another boundary.
