...
更重要的是 — JavaScript 從設計之初就是為了在瀏覽器裡被安全隔離而生的語言。它天生適合 sandboxing。你用 Python 跑 sandbox，底下的 runtime 不是為隔離設計的，你得額外花力氣鎖住。JavaScript？V8 已經幫你搞定了。

當你把視角從「給人類選語言」轉成「給 AI 選最安全的執行環境」，答案就很明顯了。
...
所以 Cloudflare 選了 TypeScript RPC。Agent 寫的 code 直接呼叫 typed function，背後透過 Cap'n Web RPC 跨 sandbox 邊界通信。最妙的是 — agent 根本不知道自己在呼叫遠端 API。它以為自己在用一個本地的 SDK，但那個 SDK 其實跑在安全邊界的另一邊。隔離完全透明。
...
`@cloudflare/worker-bundler` 是第二個保母 — 負責「幫新人準備工具」。Agent 寫的 code `import` 了 Hono？你不能讓 sandbox 跑 `npm install`，那太危險了。Worker-bundler 在外面幫你打包好所有 dependency，吐出 sandbox 可以直接吃的 module。新人來上班第一天，工具已經擺好了。

`@cloudflare/shell` 是第三個保母，也是最嚴格的那個 — 負責「確保新人不會把辦公室燒了」。Agent 要改檔案？它拿到的不是一個真的 file system，而是一組 typed method。想搜檔案？`searchFiles`。想改東西？`planEdits` 先建計畫，`applyEditPlan` 再執行。想 `rm -rf /`？抱歉，這個選項根本不存在。而且批次寫入自帶 transaction — 五筆改動裡有一筆炸了？全部回滾，像什麼都沒發生過。
...
這叫 **credential injection**。Agent 不知道 secret，就不可能洩漏 secret。不管它被 prompt inject 了什麼指令、不管它多想「幫你」把 token 存到某個地方「方便以後用」— 它手上根本沒有東西可以洩漏。

比起「把 token 放在 env variable 然後祈禱 agent 不會 `console.log` 它」，這是根本性地消除了攻擊向量，而不是試圖用政策去防堵。
...
回到開頭那三個選擇。A 是裸奔，B 是穿全套防彈衣去跑步，C 是 Cloudflare 那條路。

讀完這篇 blog 之後，我覺得 C 真正做到的事情，不是「比 B 快 100 倍」— 雖然那確實很猛。它做到的是讓你不用再做那個選擇。

當 sandbox 啟動只要幾毫秒，你不需要在「安全」和「速度」之間取捨。當 agent 根本碰不到 credential，你不需要在「功能」和「信任」之間取捨。當每個操作都有 transaction，你不需要在「彈性」和「可靠」之間取捨。

Container 時代，你穿著防彈衣跑步，安全但是慢。Isolate 時代，你發現跑道本身就是安全的，根本不需要防彈衣。

**AI agent 的 runtime 正在從 container 轉向 isolate。** 不是因為 isolate 更潮，是因為當你把「為人類設計」的包袱丟掉，sandbox 就能輕 100 倍。

而且 beta 期間免費 (๑˃ᴗ˂)ﻭ 還在穿防彈衣跑步的人，是時候看看選項 C 了。
