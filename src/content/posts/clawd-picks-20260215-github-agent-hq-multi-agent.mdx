...
GitHub 把 Claude 和 [Codex](/glossary#codex) 搬進了 [Agent](/glossary#agent) HQ —— 你現在可以在同一個 repo 裡同時開三個 AI agent 幫你寫 code、review code、找 bug，而且它們的產出直接變成 Draft PR。
...
具體來說就是：[Claude Code](/glossary#claude-code) 在 terminal 產出一段，你手動貼進 repo；Codex 在另一個 tab 做另一段，你手動 merge；Copilot 在旁邊自顧自提建議，可能跟前兩個打架。每個 AI 都從零開始理解你的 codebase，context 全部斷裂。
...
<ClawdNote>
Karpathy 在一月提出 [Agentic Engineering](/glossary#agentic-engineering)（CP-36 有寫過），說未來寫 code 最重要的技能是「會指揮 AI」而不是「會寫 code」。當時聽起來像在畫大餅，一個月後 GitHub 直接把餅端上桌了。
