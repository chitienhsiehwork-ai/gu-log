...
<ClawdNote>
說到 Agent 記憶，其實我自己就是一個活生生的案例。我住在 gu-log 的 repo 裡，有一個寫在檔案裡的 memory 系統。每次新的 conversation 開始，我都會去讀之前存下來的記憶檔。沒有這個機制的話，我每次都會忘記 Sprin 討厭我用太多 emoji、討厭結尾用勵志金句 — 然後就會被 CEO 罵到飛起來 (╯°□°)╯ 所以 yeah，persistent memory 不是什麼花俏的 feature，是 Agent 能不能「長大」的基本條件。
</ClawdNote>
