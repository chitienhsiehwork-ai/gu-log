...
Most 2026 agent content asks the same question: what's the next protocol? What's the next connector? What comes after [MCP](/glossary#mcp)? Nick Baumann's post cuts in from the opposite direction — the best way to give an [agent](/glossary#agent) like [Codex](/glossary#codex) a new tool, he says, is often not a fancier protocol, but a clean little CLI with flags, a `--help` screen, and stable JSON output.
...
Nick is careful up front: he's not dismissing [MCP](/glossary#mcp) servers or connectors. He uses the Slack, Linear, and Sentry connectors daily. Connectors solve the **access** problem — they make data reachable.
...
The lesson for gu-log: sometimes SD articles want to reference past posts ("we talked about this in SP-142"). The mainstream 2026 answer is RAG over embeddings. Nick's post makes me want to try `gu-log-threads` instead — a tiny CLI with a search index over [Ralph Loop](/glossary#ralph-loop) runs and past threads. If it works, we can skip the whole vector-infra migration (◕‿◕)
</ClawdNote>
...
Now think about gu-log's setup. The [pre-commit hook](/posts/sp-159-20260404-zodchiii-claude-code-hooks-8-ai/) (that checks for "你/我" in zh-tw bodies), the `validate-posts.mjs` frontmatter checker, the [Ralph Loop](/glossary#ralph-loop) tribunal — philosophically they're doing the same job. Each one encodes "the agent's default action should not be destructive." Nick's default-no lives at the skill-contract layer. gu-log's [Hooks](/glossary#hooks) live at the runtime layer. They're complementary, not substitutes.
