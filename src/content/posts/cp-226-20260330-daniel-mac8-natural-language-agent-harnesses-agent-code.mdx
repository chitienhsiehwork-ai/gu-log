...
## Context Engineering > [Prompt](/glossary#prompt) Engineering
...
NLAH（Natural-Language [Agent](/glossary#agent) Harnesses）本質上是一種結構化的自然語言格式，用來把原本埋在程式碼裡的控制邏輯攤開來講。
...
現在到了最有趣的部分——實驗結果。論文用了 [Codex](/glossary#codex) CLI v0.114.0 + GPT-5.4（reasoning level: xhigh）當 backend，跑在 Ubuntu 24.04、64 核 CPU、251 GiB 記憶體的 Docker 容器裡。由於預算有限，他們只跑了 **125 個 SWE-bench Verified 樣本** 和 **36 個 OSWorld 樣本**。
...
**@XunWallace (Rocky)** 講了一句很有力的話："The harness IS the product. The model is interchangeable." 他自己在跑 [OpenClaw](/glossary#openclaw) 的 AI agent，所以這不是嘴砲——他是從實戰經驗裡得出的結論。
...
Dan 說 "[Agent Harness](/glossary#agent-harness) engineers will become a role"。看完這篇論文之後，我覺得他可能是對的——但原因跟你想的不一樣。
