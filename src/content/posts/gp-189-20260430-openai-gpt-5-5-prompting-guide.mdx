...
lang: "zh-tw"
summary: "OpenAI 把 GPT 家族（4.1 → 5.5）的 prompt 指南放同一頁，對 GPT-5.5 給了一句結論：prompt 改成描述目的地、流程交給模型。process-heavy 條列被推下車，換上 personality / collaboration 拆兩塊、retrieval budget、stopping condition、phase parameter 那一套。Cursor 的 GPT-5 prompt tuning 內幕擺進去當實戰例。Anthropic Opus 4.7 在 GP-175 走過同方向，這次輪到 OpenAI 自己埋舊寫法。"
tags: ["openai","gpt-5-5","prompt-engineering","coding-agent"]
scores:
  tribunalVersion: 3
  vibe:
    persona: 8
...
這跟 Anthropic 在 [GP-175](/posts/gp-175-20260416-anthropic-opus-4-7-prompting-best-practices/) 給 Opus 4.7 的 best practice 講的「intent-first」幾乎是同一句話換個皮。兩家旗艦模型不約而同往同個方向跑，意味著舊 prompt 寫法真的進入退場期。
...
把這份 OpenAI GPT-5.5 prompting guide 跟 Anthropic 在 [GP-175](/posts/gp-175-20260416-anthropic-opus-4-7-prompting-best-practices/) 給 Opus 4.7 的 best practice 攤在桌上對比，會看到很有意思的收斂：
