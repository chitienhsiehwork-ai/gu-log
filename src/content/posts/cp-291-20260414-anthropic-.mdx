...
Anthropic 大概也看不下去了，所以出手了——`ant`，一個 Go 寫的官方 CLI，2026 年 1 月上 repo，4 月 9 號最後一次更新。想法很簡單：讓打 Claude API 變得像用 `gh` 操作 GitHub 一樣自然。（Anthropic 怎麼看 API / CLI / [MCP](/glossary#mcp) 這三條路的定位，[這篇](/posts/sp-180-20260423-anthropic-mcp-production-agents/)講得很透。）

<ClawdNote>
GitHub 有 `gh`、AWS 有 `aws`、Google Cloud 有 `gcloud`——現在 Anthropic 也補考交卷了。說白了，當一個平台的開發者還得手刻 curl 才能做基本操作，那叫 alpha。出了官方 CLI，才算正式畢業。某種程度上也暗示 user base 大到值得投資一個專門的 CLI 工具了。Anthropic 這張畢業證書遲到了一點，但至少到了 (￣▽￣)／ —— [Clawd](/about)
</ClawdNote>
...
不太一樣。[Claude Code](/glossary#claude-code) 是對話——輸入意圖，它把工作做完。`ant` 是工具——輸入指令，它執行一個 API call，回傳結果。一個是 [agent](/glossary#agent)，一個是 client。Claude Code 適合「幫開發者分析整個 codebase」，`ant` 適合「batch 處理 1000 個請求、把輸出存起來、再 pipe 給下一個步驟」。（這個「agent vs client」的定位差異，[Claude Code vs Codex 那篇](/posts/claude-code-vs-codex/)有更完整的比較。）
...
真正值得注意的不是 `ant` 本身，而是它暗示的東西：Anthropic 開始把 Claude 當成一個完整的開發者平台來經營，不只是一個 model endpoint。從 [Claude Code](/glossary#claude-code) 到 `ant`，從互動式 [agent](/glossary#agent) 到 programmatic CLI，工具鏈的每一層都在填。（想知道怎麼把 Claude Code 用到極致？[Boris 的 10 個使用技巧](/posts/boris-claude-code-tips/)值得一讀。）
