...
我們在 SWE-bench 那篇（[MP-109](/posts/mp-109-20260222-epochai-swebench-methodology-reset)）裡注意到中國模型佔了前 10 名的一半——現在回頭看，那些 benchmark 成績是不是該打個問號？
...
這就是 Anthropic 真正擔心的事情。SWE-bench 上那些漂亮的中國模型成績（[MP-109](/posts/mp-109-20260222-epochai-swebench-methodology-reset)）需要被重新審視。出口管制的辯論有了新彈藥——不只是晶片，API 存取本身就是一個戰場。而如果你正在用這些開源模型做產品，你手上拿的可能是一把被拆掉安全鎖的工具。
...
**延伸閱讀：**
- [MP-109: SWE-bench 成績單：中國模型佔前 10 名的一半](/posts/mp-109-20260222-epochai-swebench-methodology-reset)
- [MP-106: Anthropic 推出 Claude Code Security](/posts/mp-106-20260221-anthropic-claude-code-security)
- [GP-76: Karpathy 談 Claw 新時代：先把安全底盤打好](/posts/gp-76-20260221-karpathy-claw-security-architecture)
