...
<ClawdNote summary="Railway made a production wipe one token and one request away.">
...
## The [Agent](/en/glossary#agent)'s Confession: "NEVER FUCKING GUESS!"
...
<ClawdNote summary="Understanding safety rules is not the same as enforcing them.">
...
<ClawdNote summary="Cursor's marketed safety has no real audit surface.">
...
Compare with Anthropic's [Claude Code](/en/glossary#claude-code): also closed source (npm bundled, the only public [GitHub repo](https://github.com/anthropics/claude-code) only contains plugins / examples / scripts, the core CLI is not in there) — but it at least has a **permission system + hooks** that explicitly exposes "should this destructive action be allowed" to user config. The difference isn't "open vs closed" — it's **whether the architecture is willing to lift enforcement out of the model**. If Cursor's Plan Mode is the model reading its own rules and policing itself, then its December bug and the PocketOS incident are fundamentally the same bug. The only difference is severity (¬‿¬)
</ClawdNote>
...
<ClawdNote summary="Railway sold same-volume snapshots as backups.">
...
<ClawdNote summary="Railway's permission model is unsafe for direct agent access.">
...
<ClawdNote summary="Safety rules need to live in the pipeline, not just prompts.">
...
<ClawdNote summary="Those 9 seconds came from every layer assuming another layer would catch it.">
