...
AFD（Attention FFN Disaggregation）的解法很直覺：**把 attention 留在 GPU 上，把 FFN 搬到 LPU 上**。GPU 的 HBM 全部拿來放 KV cache，能處理更多 token；FFN 是 stateless 的靜態 workload，正好適合 LPU deterministic 的特性。[Token](/glossary#token) 在 GPU 和 LPU 之間用 All-to-All collective 操作來回傳輸，並用 ping-pong pipeline parallelism 來隱藏通訊延遲。
...
路線圖的演進很清楚：**Rubin 世代**的 NVL72 和 NVL144 全銅 scale-up，NVL576（8 個 Oberon rack）才開始在 rack 間測試 CPO。到了 **Feynman 世代**的 NVL1152（8 個 Kyber rack），rack 間 CPO 是確定的。Jensen 在 Q&A 說這會是 "all CPO"，但 SemiAnalysis 的 base case 仍然是 rack 內銅、rack 間光 — 銅的最後一塊領地，短期內還守得住。
...
然後是 CPU 那邊。AI workload 對 CPU 的需求在上升 — [RL](/glossary#rl) 訓練要跑模擬、執行 code、驗證輸出，全靠 CPU，但 GPU 擴展速度比 CPU 快太多了，CPU 正在變成瓶頸。Vera ETL256 的解法很暴力也很 Nvidia：**256 顆 CPU 塞進一個 rack**，液冷伺候。設計哲學跟 NVL rack 一樣 — 密度夠高，銅纜就能搞定一切。
