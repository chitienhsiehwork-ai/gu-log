...
The valuable part of Chris Hayduk's thread is not the claim that Codex can work for a long time. It is that he breaks "work for a long time" into three very engineering-shaped problems: **the loop needs to know when to stop, it needs a fast way to judge whether the last step helped, and it needs somewhere to remember what it already tried.**
...
The thread says Codex now supports `/goal` inside the app. The usage is simple: start the prompt with `/goal`, then describe what you want the agent to achieve. Codex enters a continuous loop until it decides the goal has been met.
...
Reality is less generous. Chris describes Goal mode as a loop:
...
Chris says models have become so good over roughly the last six months that everyday prompting has gotten a little lazy. Often you can gesture vaguely at what you want, tell GPT-5.5 the general direction, and it can infer the next steps.
...
Chris points to two failure modes for under-specified goals.
...
Chris gives a better version: reduce the runtime of the code in a specific file by 20%, without regressing existing unit tests or integration tests.
...
Some tasks do not naturally look like "reduce runtime by 20%." Chris gives a very representative example: converting a NeurIPS preprint into ICML conference paper format.
...
Chris does not ask Codex to "make the formatting right." He first has Codex extract the LaTeX rules into a `checklist.md` with more than 200 formatting and style requirements. Then he sets the goal: convert the NeurIPS paper to ICML format according to `checklist.md`, without changing any technical content.

That move is clean. The original goal, "make the format right," sounds like an editor's soul judgment. After the checklist, the goal becomes "finish all 200-plus items." Each individual item may still have some fuzziness, but judging one rule is much easier than judging whether an entire paper now has the right ICML aura.

The original thread includes a checklist screenshot, but the available source only preserves it as `[media]`; the screenshot text itself is not available. So the reliable facts are limited: the checklist had more than 200 items, and it covered formatting and style requirements. The exact items are not in this source.

Chris also adds a crucial instruction: check off items as they are completed. That writes progress into the file system instead of leaving it inside the agent's temporary context. Humans can inspect the checklist. The agent can reread state during a long task. The brakes stop being "seems about done" and become a visible row of boxes getting crossed off.
...
Chris's second recommendation is to shorten the feedback loop. After each round of changes, the agent needs some test mechanism to evaluate the result. The faster the test runs, and the simpler it is to execute, the more quickly the agent gets signal about whether it is closer to the goal or farther away.
...
Chris recommends using a smaller model and a sampled dataset so the agent can test ideas faster. The point is not to make the score sloppy. The point is to accelerate feedback as much as possible without destroying score quality.

His example is protein-structure model architecture search. Scoring on the full dataset could take days. He instead uses NanoFold, a small but well-sampled dataset he published on Hugging Face, and cuts experiment scoring time from days to minutes. That number matters: not "a little faster," but from day-scale to minute-scale. For an agent that iterates repeatedly, that difference changes how many ideas it can afford to try.

The source does not expand NanoFold's internal details. It only says the dataset is small, well-sampled, publicly available on Hugging Face, and used for protein-structure model architecture experiments. So this article should not invent extra benchmark numbers or dataset composition.
...
Chris's third recommendation looks humble and matters a lot: give the agent Markdown files where it can write its plan, experiments, and live thoughts.
...
Chris usually provides three files in Goal mode.
...
**`EXPERIMENTS.md`** holds experiment records. In Chris's machine learning context, it records each experiment's title, what was tried, and the result. In other domains, it can become a record of attempts: what changed, why it changed, and what happened.
...
Chris especially values `EXPERIMENTS.md`, because it lets both the human and the agent review past attempts and why each one worked or failed.
...
The original thread also includes a screenshot excerpt of `EXPERIMENTS.md`, but again the available source only preserves it as `[media]`, without the actual text. The reliable structure we can extract is a clean experiment list where each entry includes a title, a short description, and a result.
...
Put this next to recent gu-log agent pieces and the pattern gets clearer. [SP-192](/en/posts/en-sp-192-20260508-article-agent-ralph/) is about how long-running agents need external memory and human clarification. [SP-191](/en/posts/en-sp-191-20260507-danizhu-claude-dreams-agent/) and [SP-193](/en/posts/en-sp-193-20260508-article-autobrowse-agent/) approach the same problem from memory cleanup and browser-task handoff. The real fear in long-running work is not that the agent cannot run long enough. It is that after running for a long time, it forgets why it started running.
...
Without those three things, the magic button just throws a wish into a washing machine. It spins very earnestly. When you open the lid, the result may not be any cleaner.
