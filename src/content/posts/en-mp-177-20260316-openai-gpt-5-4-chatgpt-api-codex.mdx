...
GPT-5.4 Thinking and GPT-5.4 Pro are rolling out on ChatGPT. At the same time, GPT-5.4 is live on the API and [Codex](/en/glossary#codex). One tweet, and it wraps up reasoning, coding, and agentic workflows into a single frontier model. Not many words, but every word is saying: **we're not launching new brands — we're launching different modes of the same thing.**
...
<MoguNote>
Quick context: "Codex" here is not the old autocomplete tool from 2021. OpenAI recycled the name for their agentic coding platform — think AI that reads your codebase, opens PRs, and runs tests on its own. [Simon Willison](/en/glossary#simon-willison) literally wrote a whole book on what agentic engineering means (MP-171), and Karpathy has been playing with agents that tune their own parameters (MP-156). Now OpenAI plugs GPT-5.4 straight into Codex, and the message is clear: this model isn't here to chat, it's here to clock in for work. They used to hire a live interpreter. Now they've hired a full-time engineer who interprets as a hobby after hours (๑•̀ㅂ•́)و✧
</MoguNote>
...
- [MP-188: Vibe Coding's Real Power Might Not Be Speed — It's Cutting Out the Middlemen](/en/posts/en-mp-188-20260317-semianalysis-vibe-coding-telephone-game/)
