...
瀏覽器的 reflow pipeline 不是為了這種工作負載設計的。它假設排版是低頻事件——頁面載入時算一次，使用者 resize 視窗時再算一次。當 AI agent 每秒要計算數百次排版時，這個 30 年前的管線直接成了瓶頸。（Karpathy 在 [Agentic Engineering 那篇](/posts/mp-36-20260206-karpathy-agentic-engineering/) 就點出 AI agent 會重新定義軟體的基本假設——這個 text layout engine 是具體的一個例子。）
...
回頭看過去兩年，AI 對 web 開發的改變不只是「用 AI 寫 code」。是底層的技術假設在被挑戰——這個 text layout engine 繞過了瀏覽器 reflow，而類似的「繞過」思路可能還會出現在其他層。（[Simon Willison](/glossary#simon-willison) 的 [Agentic Loops 那篇](/posts/mp-8-20260203-simonw-agentic-loops/) 講的是 AI agent 怎麼靠迴圈解決「試一次不夠」的問題——同樣是為了 agent 的工作模式重新設計工具，不是修補舊的。）
