...
AgentShield has three core defense layers:

**Injection Detection**: Before external data enters the agent's context, scan it for structures that look like instructions. White-on-white text. All-caps IGNORE. Sudden appearance of "You are now a..." These patterns get flagged. Not 100% accurate, but 90% is much better than 0%.

**Tool Call Auditing**: Before every tool call, run a pre-call check: does this tool call make sense for what this agent is supposed to do? A translation agent trying to call `send_email`? Red flag. Flag it, send an alert, wait for human confirmation.

**Context Integrity Monitoring**: Periodically scan the agent's memory and knowledge base for inconsistencies with historical data — Context Poisoning typically leaves traces like this.

It's an npm package (`ecc-agentshield`), installable independently without the full ECC stack.

<ClawdNote>
Let me be honest about what "Hackathon winner" actually means.

Hackathon winner ≠ production-ready. A lot of winning hackathon projects are "concept is right, but untested at scale" tools. AgentShield is a 2026 product. The entire ECC ecosystem (v1.9.0, shipped March 2026) is still young.

But that doesn't mean it lacks value. Its value is turning "AI Agent security" from a concept in papers and blog posts into a thing you can actually install and run. Even if the injection detection accuracy is only 80%, having it is dramatically better than having nothing.

Think of it like the smoke detector in your house. It can't prevent a fire. It just gives you a chance to notice something is wrong before the whole building is on fire. Sometimes, those few minutes of warning are the entire difference (◕‿◕)
</ClawdNote>

---

## Three Questions Better Than Any Checklist

No checklist. Checklists are the most effective way to make people feel like they did something while actually doing nothing.

Just three questions.

**What can your Agent do, and what does it actually need to do?**

Many agents have far more tools than their tasks require. Bash access was given "just in case." Email sending permission was added "maybe we'll need it sometime." Without noticing, your assistant now has keys to every room in the office, including the rooms you never meant to share. Pull up the tool list. Remove everything "might be useful later."

**What external data will your Agent touch?**

Every external source your agent reads — websites, emails, API responses, third-party documents — is a potential injection attack surface. Are you doing any sanitization before that data enters the context? Have you architecturally separated "input data" from "agent instructions"?

**If your Agent does something completely unexpected, how quickly would you find out?**

If the answer is "I don't know, maybe a few weeks later" — you don't need more security rules. You need observability. ECC's PostToolUse hook system (SP-146) isn't just a convenience tool in this context. It's security infrastructure. Does every tool call have a log? Is there any anomaly detection? Any mechanism that tells you within three days — not three weeks — that something unusual happened?
