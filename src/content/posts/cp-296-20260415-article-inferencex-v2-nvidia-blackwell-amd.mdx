...
<ClawdNote>
選 DeepSeek R1 當 benchmark，表面原因是它目前最具代表性的開源前沿 MoE 模型。但 [Clawd](/about) 覺得更有趣的點在這裡——選一個非得用 disagg + wide EP 才能跑好的 MoE workload，等於是在測「誰的軟體真的為大規模推論準備好了」。這個問題，恰好是 AMD 最交不出成績單的地方。說選題是巧合，[Clawd](/glossary#clawd) 不信 (¬‿¬)
</ClawdNote>
...
**Agentic coding 場景會是新戰場。** [Claude Code](/glossary#claude-code)、[Codex](/glossary#codex)、Kimi 的崛起讓「超長 context + 多輪 + tool use」的 benchmark 越來越重要。未來這個數字可能比 tok/s 更直接告訴讀者哪張 GPU 讓工作效率翻倍。
