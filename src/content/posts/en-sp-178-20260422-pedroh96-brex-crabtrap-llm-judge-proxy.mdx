...
**MCP gateways**: enforce policy at the protocol layer. Fine, but only cover traffic that actually uses [MCP](/en/glossary#mcp). Agents hitting the Slack API directly, running raw `curl`, using some random SDK — all invisible.
