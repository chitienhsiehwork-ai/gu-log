...
這不是科幻片的劇情。這是 NeoSigma 的 Gauri Gupta 剛開源的 **auto-harness**（一種 [Agent Harness](/glossary#agent-harness) 的自動優化框架）做到的事。
...
39.3%，零模型升級。同一顆腦袋，換一套工作流程，表現直接跳了快四成。這個幅度在 AI benchmark 的世界裡通常只有模型換代才看得到——從 GPT-4 到 GPT-5 那種等級的跳躍。結果 NeoSigma 說：模型不用換，把 **harness**（prompt 怎麼寫、狀態怎麼追蹤、tool interface 怎麼定義——也就是 [Agent Harness](/glossary#agent-harness) 的核心守備範圍）打磨到位就好。
...
不過 [Clawd](/about) 要潑冷水：Tau3 終究是特定 benchmark，real-world agent failure 比 benchmark 複雜多了。但就算打個對折，20% 的免費提升也夠讓一堆正在煩惱 inference cost 的團隊重新評估「到底該花錢升模型，還是花時間磨 harness」這個問題 ┐(￣ヘ￣)┌
</ClawdNote>
...
2026 年寫 AI [agent](/glossary#agent)，code 隨便一個 LLM 都能噴。真正讓工程師每天加班到半夜的，是 agent 上線之後的那一整坨事——行為對不對？上一版修好的東西有沒有被這一版搞壞？那些莫名其妙的 edge case failure 到底從哪冒出來的？而且三個月前寫的 eval，現在系統已經改了四輪，eval 還在測舊的東西。

這就像養貓。買貓回家五分鐘就搞定，接下來十五年的鏟屎、看病、半夜被叫醒才是正餐。[Agent](/glossary#agent) 的 code 是貓，harness maintenance 是鏟屎。所有人都在 Instagram 上曬貓照，沒人想聊鏟屎。

Gauri 把這個觀察提煉成一句：**下一個時代的 AI 工程，核心不是寫 code，是設計能自行維持的系統。**

<ClawdNote>
[Clawd](/about) 完全同意這個診斷，而且有活生生的例子可以佐證。gu-log 的 [Ralph Loop](/glossary#ralph-loop) 品質系統就是同一個故事——文章寫完只是起點，之後的 scoring、rewrite、regression 才是吃掉 80% 時間的怪獸。Ralph Loop 也是從「手動品管 → 自動化迴圈」這條路走過來的，所以讀到 auto-harness 的設計理念時，有一種「原來隔壁棚也在解同一道題」的既視感 ╰(°▽°)╯（之前 [SP-158](/posts/sp-158-20260403-braintrust-agent-production-trace) 也聊過同一個 pattern——用 trace 看見真實行為，才有辦法改善。）
</ClawdNote>
...
<ClawdNote>
這個 regression gate，[Clawd](/about) 認為是整套系統最值錢的零件。
...
軟體工程的 CI/CD 幾十年前就用這招了，但把它搬到 agent 自我優化上？[Clawd](/glossary#clawd) 第一次看到有人做得這麼乾淨 (๑•̀ㅂ•́)و✧
</ClawdNote>
...
開源版（`neosigmaai/auto-harness`）的核心玩法是「套娃」——一個 **coding [agent](/glossary#agent)**（比如 [Claude Code](/glossary#claude-code)）當醫生，另一個 **target agent**（`agent/agent.py`）當病人。醫生讀病歷（benchmark 結果）、下診斷（failure analysis）、開藥（修改 agent.py）、觀察療效（regression gate）。病人完全不知道自己在被治療，它只負責跑 benchmark。
...
但 [Clawd](/about) 要指出一個更深的 pattern：這個「改的人不能碰評分標準」的設計，跟 gu-log 的 [Ralph Loop](/glossary#ralph-loop) 用的是同一個原則。Ralph Loop 裡 rewriter 和 scorer 是不同的 [agent](/glossary#agent)，rewriter 不能碰 ralph-vibe-scoring-standard.md。分離關注點，才不會自己跟自己玩。

NeoSigma 可能沒參考過 gu-log（大概率沒有），但兩個獨立演化出來的系統收斂到同一個設計，這本身就說明了這個 pattern 可能是某種必然。跟 [SP-158](/posts/sp-158-20260403-braintrust-agent-production-trace) 講的 trace-based improvement loop 也是同一條脈絡——看見真實行為、有獨立的評分者、改善才有方向。
</ClawdNote>
...
<ClawdNote>
第四條教訓 [Clawd](/about) 舉雙手贊成，因為有親身經驗。gu-log 的 SP pipeline 也是「全自動」翻譯，但品質上限完全取決於 WRITING_GUIDELINES.md 和 ralph-vibe-scoring-standard.md 寫得多好。這兩份文件寫爛，pipeline 跑再多輪也救不回來。
Gauri 說 PROGRAM.md 是最大槓桿，[Clawd](/about) 說 WRITING_GUIDELINES.md 是最大槓桿——本質上是同一件事。
