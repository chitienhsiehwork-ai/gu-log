...
translatedBy:
  ...
  pipeline:
    - role: "Fetched"
      model: "gp-pipeline FetchX"
      harness: "fxtwitter"
    - role: "Evaluated"
      model: "Opus 4.6 + Codex"
      harness: "gp-pipeline eval"
    - role: "Written"
      model: "Opus 4.7"
...
[Mitchell 部落格那篇全文](https://mitchellh.com/writing/ghostty-leaving-github) 沒在講「哪個替代方案比較好」這種對照表，整篇是一封寫給 GitHub 的告白信，告白的主軸不是技術選型，是一段感情走到盡頭的紀錄。讓他終於把信寄出去的最後一根稻草，是過去一個月他每天在日記裡標的那個 X——只要 GitHub 那天又害他工作做不下去就畫一個——「幾乎每天有 X」。寫文當天他開始寫這篇 GP 之前，剛因為 GitHub Actions 服務中斷卡了兩個小時的 PR 審查。

這篇 GP 不是在報導「又一個維護者離開 GitHub」這種趨勢觀察。是要把 Mitchell 這封告白信底下三件特別值得記下來的東西拉出來：第一，**為什麼要等到第 1299 號用戶這種人開口，整個圈子才會聽進去**；第二，**他刻意只搬 Ghostty 不搬個人專案這個區分**；第三，他自己擺進腳註的那句「**跟昨天那場大停機是巧合，這篇文一週前就寫好了**」——這層保留條件比搬家本身更值得讀。
...
<MoguNote>
[Mogu](/about) 想對著「畫 X 日記」這個動作多講一段，因為這是這篇文章裡最被低估的一條操作模式。
...
這條對獨立開發者的啟示其實很實際：**抱怨工具壞掉的時候，先做 30 天的 X 標記再開口**。同事抱怨 [Slack](/glossary#slack) 通知爛、CI 不穩、IDE 卡頓，下次直接拿出手機備忘錄打開「2026-04 月 X 標記」給對方看——上面寫著「03、05、07、09、12、14、15...」具體到日期。對方沒辦法用「我覺得還好啊」當回應。
...
順帶一提，這個方法的相反面是「**只記印象，不記證據**」——這是大多數人對工具的態度，包括 [Mogu](/glossary#mogu) 自己以前用 Notion 卡到崩潰時的反應。下次再對著什麼工具煩，先開個 X 日記試一個月再說，不行再走 (◕‿◕)
</MoguNote>
...
<MoguNote>
這段「為什麼第 1299 號用戶開口才有人聽得進去」的觀察，Mogu 想拉成一個更通用的訊號分析。
...
<MoguNote>
「離開的決定」跟「下一個去哪」分開做——這條操作模式 Mogu 想拉出來多講一段，因為這在獨立工程師圈是最常被搞反的事。
...
Mogu 私心覺得這個保留條件比「我要搬家」這句宣告還重要——這代表 Mitchell 不是在做政治表態，他是真的在解決一個實際的工程問題 (⌐■_■)
</MoguNote>
...
<MoguNote>
腳註這個工具被嚴重低估，Mogu 想用一句話講完：**腳註是給「會讓主文敘事走鐘但不寫又會被質疑」的內容專用的抽屜**。
...
順帶把這篇放進 gu-log 的脈絡裡看：[那篇講 Mitchell 用 Vouch 簽 Ghostty 發布版本（2026-02-08）](/posts/mp-47-20260208-mitchellh-vouch-oss-trust/) 是關於「OSS 信任怎麼建」、[ShroomDog 自己寫的「Mitchell 兩年 AI 採用心路歷程」（2026-02-07）](/posts/gp-40-20260207-mitchellh-ai-adoption-journey/) 是關於「他怎麼從質疑 AI 到擁抱 AI」、[GP-169（2026-04-11）dani_avila7 那篇講 Ghostty 配 Claude Code](/posts/gp-169-20260411-dani-avila7-ghostty-claude-code-sand/) 是關於「Ghostty 變成 [Agent](/glossary#agent) 終端機的工程角度」。三篇加起來、加上這篇，gu-log 對 Mitchell 跟 Ghostty 的記錄已經接近完整——從 2 月信任建設、到 4 月工程整合、到這個月底搬家公告，都在現場。
