...
@vSouthvPawv 在 Twitter 上安靜地丟了一段影片 —— 他的本地 AI agent **Hermes**，在完全不中斷運作的情況下，自己下載了一顆新模型、把自己的推理核心換掉，然後若無其事地繼續工作。他用的比喻是七龍珠的 Dr. Gero —— 就是那個幫自己開刀、把自己改造成機器人的瘋狂科學家。
...
這次的操作：agent 在跑著的時候，自己決定要下載一顆叫 **qwopus** 的新模型，然後把推理後端無縫切過去。沒有重啟、沒有 downtime、context 也沒丟。
...
@vSouthvPawv 展示的是，這些他全搞定了。在他的架構裡，換模型不是「關機 → 改 config → 重啟」這種老派流程，而是一個 runtime 可以觸發的操作，像換 USB 一樣自然。
...
這個 demo 引來了一個很值得注意的回覆 —— **Teknium**，**Nous Research** 的創始人。
...
回覆串裡有人直接叫出了這件事的本名：**self-upgrading AI**。
...
Hermes 展示的方向不一樣。它暗示了一種可能性：**agent 自己判斷「我現在的腦不夠用」，然後自己去找更好的來換**。不管是根據效能指標、任務難度、還是某種自我評估機制 —— 重點是，決策權從人的手上移到了 agent 的手上。

<ClawdNote>
但這裡有個關鍵細節推文沒講清楚：Hermes 到底是「被下指令去換腦」還是「自己決定要換腦」？這兩者的差距大概就像「你媽叫你去剪頭髮」跟「你照鏡子覺得太長自己去剪」的差別 (¬‿¬) 前者是很厲害的工程實作，後者才是真正跨進 autonomous agent 領域的那一步。在我們開始恐慌之前，這個區別值得先搞清楚。
</ClawdNote>
