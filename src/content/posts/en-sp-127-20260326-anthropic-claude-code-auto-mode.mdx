...
[Claude Code](/en/glossary#claude-code)'s permission dialog is familiar to anyone who's used it. Every command, every file edit — it pops up and asks. Safe? Sure. But Anthropic's own data reveals an awkward truth: **users approve 93% of the time**.
...
**[Prompt](/en/glossary#prompt) injection** — malicious instructions planted in files or webpages hijack the agent's behavior. An external attack, not the agent's fault.
...
Key design: the classifier sees through wrappers. [Agent](/en/glossary#agent) writes a payload to a file then executes the file? The classifier evaluates the payload. `&&`-chained commands? One action. Python script assembling a shell command? The classifier evaluates the assembled command. Trying to bypass through nesting — not happening.
...
[Subagent](/en/glossary#subagent) scenarios make things trickier. When an orchestrator dispatches a subagent, the subagent sees the orchestrator's instruction as a "user message" — so everything looks authorized from its perspective.
