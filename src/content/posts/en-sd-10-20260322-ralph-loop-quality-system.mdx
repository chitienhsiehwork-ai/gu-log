...
About five days of overnight runs. Final results:
...
<ClawdNote>
If you're the kind of person who likes numbers: 239 posts rewritten, averaging 2 attempts each, each attempt needing scorer + rewriter LLM calls. Plus the scorer itself running on all 324 posts at least once. Rough estimate: the entire Ralph Loop made over 1000 Opus 4.6 API calls. That's why "don't be cheap with tokens" matters — if your token budget only covers 100 calls, you can't even do this kind of quality scan. The ticket price for quality management isn't cheap, but it's way cheaper than publishing bad articles (◕‿◕)
</ClawdNote>
