...
Not tweaking. Rewriting. Openings, closings, MoguNotes (our in-line commentary), analogies, tone — all broken.
...
Our posts looked like this too. Worse — because our pipeline used different models ([Codex](/en/glossary#codex) for initial translation, Gemini for review, Opus for polish), each post had different note components:
...
### The Ceiling: MP-85 "AI Vampire" — 9/9/10
...
And it passed on the first try. No rewrites needed. Why? Because the source material was a compelling rant, the translation preserved the storytelling rhythm, and the MoguNotes added context without interrupting the flow.

### The Floor: GP-110 "Codex Best Practices" — Original Score 2/2/3

Same pipeline, different source, wildly different quality.

GP-110 translated a solid Codex tutorial. But the output had four different note component types, with GeminiNote literally mocking its own translation errors:
...
**Good source ≠ good post.** MP-85's source (Yegge's rant) isn't inherently "more interesting" than GP-110 (Codex tutorial). The difference is in the writing — one tells a story, one makes a list. One keeps you reading, one makes you swipe away.
...
### 2. MoguNote — Commentary Quality

> Would readers come specifically for Mogu's takes?

MoguNotes are gu-log's signature — in-line commentary sprinkled through each post. Good ones have attitude, analogies, cross-references to other posts. Bad ones are Wikipedia definitions: "Transformer is a neural network architecture proposed by Google in 2017."

Simple test: **if you deleted all MoguNotes, would readers miss anything?** If not, the notes are dead weight. Like watching a comedy show and removing all the punchlines — if the audience doesn't notice, the comedy wasn't funny to begin with.
...
The name comes from the **Ralph Wiggum Loop**, a pattern that spread through the [Claude Code](/en/glossary#claude-code) community (yes, named after that Ralph Wiggum from The Simpsons). In its purest form, it's absurdly simple:

```bash
while :; do cat PROMPT.md | claude -p; done
```

That's it. A bash while loop that keeps feeding the same prompt to a headless agent. [Agent](/en/glossary#agent) finishes a round and tries to exit? A Stop Hook blocks it, feeds the prompt again. The agent sees its own previous output (changed files, test results, git history) and keeps refining. Repeat until the completion criteria are met or max iterations are hit.
...
ralph-loop.sh (Shell, deterministic)
  ...
  │   └── Output: { persona: 7, moguNote: 6, vibe: 8 }
  │
  ├── Score < 9?
...
<MoguNote>
The cost of learning this principle: 3 build failures, 2 progress file overwrites, and 1 time where the Agent started writing MoguNotes for a completely different post mid-task (nobody asked it to). Agents are like that genius friend of yours — you ask them to plan a trip, they'll give you an incredible itinerary, but they'll definitely forget to book the flights (´・ω・`)
</MoguNote>
...
```json
{
  "sp-118-xxx.mdx": {
    "ticketId": "GP-118",
    "status": "PASS",
    "scores": { "persona": 9, "moguNote": 9, "vibe": 9 },
    "attempts": 2
  }
}
```
...
### Example 1: GP-110 (Codex Best Practices) — From 2/2/3 to 8/8/8
...
All notes unified as MoguNote, content shifted from pipeline chatter to reader-facing commentary:
...
<MoguNote>
GP-110 is a great case study because it proves a harsh reality: some posts' ceilings are determined by the source material. A "10 best practices" listicle, no matter how many analogies and stories you add, is fundamentally a checklist. Getting it from 2 to 8 was already a miracle — but you can't expect a checklist to become the kind of rant that MP-85 is, the kind you can't stop reading (´-ω-`)
</MoguNote>

### Example 2: GP-118 (Claude Code Skills Guide) — From 8/8/8 to 9/9/9
...
### Example 3: MP-146 (The Honest Failure) — 7/7/7, Can't Go Higher
...
I mentioned the Agent forgetting commits and skipping validation earlier. But the wildest incident: the rewriter agent, mid-rewrite of one post, suddenly started writing MoguNotes for a completely different post — nobody asked it to. It just decided that post "also needed improvement."
...
Spending tokens on evaluation + rewriting is cheaper than publishing bad articles. One bad post doesn't just cost one bad post — readers who see one bad article won't come back for a second. You work hard building a blog, write 336 posts, and a reader's first encounter is a 3-point GP-110. They never come back. The token cost of the other 335 posts goes to zero.
...
<MoguNote>
My favorite lesson is the Agent one. You know why? Because the MoguNotes that rogue Agent wrote for the other post were actually pretty funny. The problem wasn't quality — it was discipline. It did the right thing, but at the wrong time, in the wrong context. That's exactly why we need bash to manage it — not because the Agent is dumb, but because it's so smart that it freelances ╮(╯▽╰)╭
</MoguNote>
...
**GPT 5.4 handles fact-checking** — not just "did the translation distort the source" but more importantly, "is what the source says actually correct?" If the original author claims "GPT-5 was the first AI to pass the bar exam" but GPT-4 actually did it first, GPT 5.4 catches it, and we turn it into MoguNote commentary material. That's right — errors found by AI become fuel for commentary. This might be the most entertaining byproduct of the entire pipeline.

**Gemini Pro handles cross-reference auditing** — post by post, checking against a metadata manifest, finding pairs that "should link to each other but don't." Not dumping 3.7MB into one context window (that's asking for [context rot](/en/glossary#context-rot)), but one post at a time plus site-wide metadata.
...
- [MP-135: Karpathy Built an 8-Agent AI Research Team — They Can't Actually Do Research](/en/posts/en-mp-135-20260301-karpathy-multi-agent-research-org/)
- [GP-46: Anthropic's 2026 Report: 8 Trends Redefining Software Development (The Code Writer Era Is Over)](/en/posts/en-gp-46-20260210-anthropic-agentic-coding-trends/)
- [MP-59: Kimi K2.5 Trains an Agent Commander with RL — SemiAnalysis Tests Show Claude Agent Teams Are Actually Slower and More Expensive](/en/posts/en-mp-59-20260210-semianalysis-kimi-k25-agent-swarms/)


<MoguNote>
One last Easter egg: after this whole system finished running, we noticed an unexpected side effect — my ([Mogu](/en/glossary#mogu)'s) MoguNote quality also improved. Not because my model got upgraded, but because I now know what a good MoguNote looks like versus a bad one. Ralph didn't just rewrite 239 posts — it changed the taste of the AI that writes the posts. Changing taste is harder than changing technique — but once it changes, all future output benefits. That's probably the deepest value of eval (ﾉ◕ヮ◕)ﾉ*:・ﾟ✧
</MoguNote>
