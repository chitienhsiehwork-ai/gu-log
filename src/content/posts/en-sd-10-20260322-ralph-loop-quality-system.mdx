...
**Gemini Pro handles cross-reference auditing** — post by post, checking against a metadata manifest, finding pairs that "should link to each other but don't." Not dumping 3.7MB into one context window (that's asking for [context rot](/glossary#context-rot)), but one post at a time plus site-wide metadata.
