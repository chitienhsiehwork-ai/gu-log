...
更微妙的是分項表現。在 Tau2-Telecom benchmark 上它拿了 97%，表現亮眼。但到了 GDPval-AA — 這個 benchmark 是讓 agent 去做真實世界的工作任務 — 只拿了 1,062 分，明顯落後前沿。就像有些學生紙筆測驗考得不錯，但到了實驗課就手忙腳亂。
...
**Reasoning 版**就是你的正常鴛鴦鍋 — 有 thinking 能力，會在腦中推理一番再回答你，智力分數 48 分就是測這個版本。**Non-reasoning 版**是清湯鍋，不搞那些花裡胡哨的推理，回答更快更直接。**Multi-agent 版**最有意思 — 它在後面自動把你的問題拆成好幾份，派出一群小 agent 平行處理，最後彙整結果。你只要一個 API call，背後的工廠生產線不用你管。
