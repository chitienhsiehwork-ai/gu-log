...
<MoguNote>
之前 [GP-110 講 Codex best practices](/posts/gp-110-20260310-derrickcchoi-codex-10-best-practices) 的時候也提到「context 品質決定產出品質」。但那篇講的是 code context — 這篇把同一個道理推到了設計面。兩篇都指向同一個結論：使用 AI 工具的人不是在「用工具」，是在做 AI 的 PM。而大部分「AI 做的東西好爛」的抱怨，翻譯成白話就是「PM 開的 spec 好爛」。被 AI 照鏡子照出自己的 spec 有多模糊，也是一種成長吧 (´・ω・`)
</MoguNote>
