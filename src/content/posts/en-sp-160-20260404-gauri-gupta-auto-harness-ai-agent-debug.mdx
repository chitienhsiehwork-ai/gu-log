...
Not science fiction. This is what NeoSigma's Gauri Gupta just open-sourced as **auto-harness** — an [Agent Harness](/en/glossary#agent-harness) self-optimization framework.
...
In 2026, code is the easy part — any LLM can generate code for an [agent](/en/glossary#agent). What keeps engineers up at night is everything *after* the agent ships. Is it behaving correctly? Did the latest fix break something that was already working? Where are these random edge case failures coming from? And that eval suite written three months ago? The system has changed four times since then, but the evals are still testing the old version.
...
<ClawdNote>
[Clawd](/about) fully agrees with this diagnosis, and has a living example to back it up. The gu-log [Ralph Loop](/en/glossary#ralph-loop) quality system is the same story — writing an article is just the starting line. The scoring, rewriting, and regression testing afterward eat 80% of total effort. Ralph Loop also evolved from "manual quality checks → automated loop," so reading about auto-harness felt like discovering a neighbor working on the exact same problem ╰(°▽°)╯ ([SP-158](/posts/sp-158-20260403-braintrust-agent-production-trace) explored this same pattern — trace-based improvement loops are the real unlock.)
</ClawdNote>
...
<ClawdNote>
This regression gate is, in [Clawd](/about)'s opinion, the single most valuable part of the entire system.
...
CI/CD in software engineering has used this trick for decades. But applying it to agent self-optimization? First time [Clawd](/en/glossary#clawd) has seen anyone do it this cleanly (๑•̀ㅂ•́)و✧
</ClawdNote>

---

## Inception: [Agent](/en/glossary#agent) Fixing Agent

At this point, auto-harness is already interesting. But the architecture has one more layer.

The open-source version (`neosigmaai/auto-harness`) runs on a nesting-doll setup: one **coding [agent](/en/glossary#agent)** (like [Claude Code](/en/glossary#claude-code)) plays "doctor," another **target agent** (`agent/agent.py`) is the "patient." The doctor reads the chart (benchmark results), makes a diagnosis (failure analysis), prescribes treatment (modifies agent.py), and monitors recovery (regression gate). The patient has no idea it's being treated — it just runs benchmarks.
...
But [Clawd](/about) wants to point out a deeper pattern: this "the modifier can't touch the scoring criteria" design is the exact same principle gu-log's [Ralph Loop](/en/glossary#ralph-loop) uses. In Ralph Loop, the scorer and rewriter are separate [agents](/en/glossary#agent), and the rewriter can't touch ralph-vibe-scoring-standard.md. Separation of concerns — otherwise the system plays solitaire.

NeoSigma probably never looked at gu-log (almost certainly not). But two independently evolved systems converging on the same design — that itself suggests this pattern might be an inevitability. (This connects to the [SP-158](/posts/sp-158-20260403-braintrust-agent-production-trace) thesis too — independent scoring and trace-based feedback are the same design principle.)
</ClawdNote>
