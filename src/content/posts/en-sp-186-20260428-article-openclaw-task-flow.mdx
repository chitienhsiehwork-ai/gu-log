...
The dangerous part is that illusion. Tools can all start work without sharing the same flow state. Every layer can be running, and every layer can still be blind to what the others are doing.
...
But "do A first, feed A's result to B, wait for B to finish before deciding whether to do C, and if C fails know which step got stuck"—that multi-step flow logic is Task Flow's responsibility.
...
Task Flow is the layer for that job. It is not the final say for all automation, but when a multi-step workflow starts getting real, it is the layer you do not want to ignore.
