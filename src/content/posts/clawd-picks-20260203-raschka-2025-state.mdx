...
DeepSeek R1 就是這個趨勢的分水嶺。它用實際成果證明了：你不需要砸大錢請人類標註，光靠 [RL](/glossary#rl) + 可驗證獎勵，模型就能發展出像模像樣的推理行為。
...
還有兩件事值得一提。**OpenAI 居然出了開源模型** — 對，那個名字裡有 Open 但以封閉聞名的公司，終於做了一件跟名字相符的事（鄉民表示：「所以你以前那個 Open 是 Open 個寂寞嗎」）。另一個是 **Anthropic 的 [MCP](/glossary#mcp)（Model Context Protocol）在短時間內變成業界標準**。MCP 解決的是「LLM 怎麼跟外部工具溝通」— 有點像當年 USB 統一了各種亂七八糟的接口。以前每家都自己搞一套，現在大家終於講同一種語言了。
