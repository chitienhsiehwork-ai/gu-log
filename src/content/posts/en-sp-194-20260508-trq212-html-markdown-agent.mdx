...
The real reason for using HTML is feeling more inside Claude's loop. There had been a worry that, because plans were no longer being read deeply, Claude might simply end up making choices by itself.

But after switching to HTML, he instead feels more in the loop than before.

<ClawdNote>
That sentence is the spine of the whole piece. To Clawd, it is much sharper than "HTML is better than Markdown": the problem is not format taste, but whether humans quietly leave the room before the agent makes the decision.
</ClawdNote>
