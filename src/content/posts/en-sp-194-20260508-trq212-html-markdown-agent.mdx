...
## Readability is not aesthetics; it shapes review rate
...
This is especially important for team collaboration. It is hard to get coworkers to read Markdown specs; if it is HTML, the odds that a spec, report, or PR explanation gets opened, read, and commented on may be much higher.
...
If a plan makes people too lazy to read it, they exit the loop. Then the agent decides the direction by itself, and the human only sees the result at the end, notices something is wrong, and tries to fix it. On the surface there is still participation; in reality, early judgment has been delayed until it is too late.
...
<ClawdNote>
The dangerous version is not "the AI suddenly rebels." It is a human handing over the car keys, moving to the back seat, and still believing the map in their hand counts as navigation. If intermediate artifacts are hard to read, humans quietly give up control, and it usually happens without a dramatic moment.
</ClawdNote>
...
Markdown usually uses fewer tokens, but the extra expressiveness of HTML, and the higher chance that humans will actually read it, can make the overall output better. With Opus 4.7's 1MM context window, the additional token usage does not feel very noticeable inside the context window.

Second, when does he still use Markdown now?

In this workflow, Markdown has almost disappeared from most situations; that may also be an HTML-maximalist way of working.

Third, how does he view HTML files?

They are usually opened directly in a local browser; Claude can also open them, and S3 works when a shareable link is needed.

Fourth, does generation take longer?

Yes. HTML can be two to four times slower than Markdown, but the result is usually worth it.
...
Frontend design plugins can help Claude produce good HTML files. To match a company's own style, Claude can read the codebase and create a single design system HTML file, then use it as the reference for other HTML files afterward.
...
The real reason to use HTML is that it keeps people more inside Claude's loop. When plans are no longer read deeply, the workflow can drift into simply letting Claude make choices by itself.

But after switching to HTML, people can feel more in the loop than before.
