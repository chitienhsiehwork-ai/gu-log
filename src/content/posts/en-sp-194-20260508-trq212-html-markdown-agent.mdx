...
On the surface, [Thariq](/en/glossary#thariq)'s piece is about something very simple: when using [Claude Code](/en/glossary#claude-code), instead of asking the agent to produce Markdown, ask it to produce HTML.
...
Beyond the file system, Claude Code can also find more context through [MCP](/en/glossary#mcp), such as [Slack](/en/glossary#slack) and [Linear](/en/glossary#linear); it can also use browser context, such as Claude in Chrome, as well as sources like git history.
