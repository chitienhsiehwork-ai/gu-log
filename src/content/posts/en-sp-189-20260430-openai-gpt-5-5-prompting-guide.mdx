...
The docs include two templates. One for a "steady, task-focused" assistant:
...
Both templates are short — deliberately. These blocks **don't replace task goals, success criteria, tool rules, or stopping conditions**. Personality shapes user experience. Collaboration style shapes task behavior. Neither should reach across and define what "done" means.
...
Once you've stripped the process-heavy bullet lists, the prompt is shorter. But there are still situations where the model needs explicit guidance. The GPT-5.5 section gives three independent tricks, each fixing a specific pain point.
...
This trick doesn't make the model faster. It makes the user **feel** like the model is faster. Seeing "Got it, going to start by checking X" appear in a streaming UI cuts the waiting anxiety in half. The docs call this "improve perceived responsiveness" — refreshingly honest, no claim that reasoning is faster.
...
Following on from the outcome-first philosophy, a stopping condition tells the model "when to stop." The template:
...
The Responses API introduced a `phase` field starting with GPT-5.4. It lets long-running or tool-heavy workflows distinguish "intermediate updates" from "final answer." GPT-5.5 uses the same pattern. The rules:
...
The GPT-5.4 docs even add a troubleshooting hint: "if GPT-5.4 starts treating intermediate updates as the final answer, first check whether your integration is preserving the phase field." A lot of teams must have hit this bug for it to make it into the docs.
...
This has big consequences for coding agent decisions: if you roll your own patch tool, you need a very specific reason (like patches needing to go through some review pipeline first). The docs offer both a server-defined tool and a freeform tool with context-free grammar — both have full examples. Pick the one that fits your existing stack.
...
Result: status updates stay short and clean, code diffs come out easy to read. This is the standard solution for "the same model needs different behaviors on different surfaces." With GPT-5, `text.verbosity` is the global default, but the prompt can override it for specific contexts.
...
The GPT-5.3-Codex section hides a sneakier trick: metaprompting, which means **letting the model look at one of its unsatisfying outputs and suggest how to fix the instructions**. The template:
...
The caveats matter — don't skip them:
...
Lay the OpenAI GPT-5.5 prompting guide next to Anthropic's Opus 4.7 best practices ([SP-175](/en/posts/en-sp-175-20260416-anthropic-opus-4-7-prompting-best-practices/)) and a striking convergence appears:
...
Two top-tier vendors converging on the same direction isn't copy-paste. It's two independently-trained flagship models reducing the need for process-heavy prompts. For prompt engineers, this signal is stronger than any single doc: the old prompt style can no longer be treated as the default safe option.
