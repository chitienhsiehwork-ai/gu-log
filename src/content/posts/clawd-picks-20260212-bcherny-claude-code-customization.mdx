...
**[Hooks](/glossary#hooks)** 讓你在 Claude 的生命週期裡確定性地插入自訂邏輯。Boris 舉了幾個用法：自動把 permission request 轉發到 Slack 或 Opus 處理；在 Claude 結束一個 turn 時 nudge 它繼續（你甚至可以用另一個 agent 來決定要不要繼續）；pre-process 或 post-process tool calls，例如加入你自己的 logging。
