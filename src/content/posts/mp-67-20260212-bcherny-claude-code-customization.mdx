...
[Boris Cherny](/glossary#boris-cherny) — [Claude Code](/glossary#claude-code) 的創作者 — 在 X 上發了一個 12 則的 thread，講的就是怎麼讓 Claude Code 從「能住的房子」變成「你的家」。核心只有一個字：
...
然後是「AI 的大腦要多用力」。跑 `/model` 選 effort level — Low（省 token、快）、Medium（平衡）、High（聰明但慢）。Boris 本人？**全部開 High。** 一點都不意外，畢竟他是那個一天 ship 27 個 PR 的人（見 MP-14）。對他來說品質永遠大於速度。
...
**第一層：白名單。** 跑 `/permissions` 精確開放你需要的指令，支援 wildcard 語法 — `Bash(bun run *)` 批准所有 bun run 指令，`Edit(/docs/**)` 批准修改 docs 資料夾所有檔案。Boris 在 MP-14 就明確說過他**不推薦**用 `--dangerously-skip-permissions`。正確做法是配一把鑰匙，不是直接把門拆掉。
...
**[Hooks](/glossary#hooks)** 讓你在 Claude 的生命週期裡確定性地插入自訂邏輯。Boris 舉了幾個用法：自動把 permission request 轉發到 [Slack](/glossary#slack) 或 Opus 處理；在 Claude 結束一個 turn 時 nudge 它繼續（你甚至可以用另一個 agent 來決定要不要繼續）；pre-process 或 post-process tool calls，例如加入你自己的 logging。

Boris 在 MP-14 分享過他的 PostToolUse hook 會自動跑 `bun run format`。而現在 hook 能做的事更多了 — 包括「用另一個 AI 來決定要不要繼續」。這是 agent-of-agents 的概念，遞迴到讓人頭暈。
...
你有沒有注意到，Boris 前一個爆紅 thread（MP-14）講的是「**我**怎麼用 Claude Code」— 那是一場 open house，帶你參觀他的家。但這次的 thread，主角不是 Boris，是**你**。他在告訴你：「嘿，這面牆可以打掉、那個書架高度你隨便調、蓮蓬頭水壓你自己設。」
...
- [GP-52: 在 Claude Code 裡優雅調用 Codex](/posts/gp-52-20260212-codex-mcp-claude-code/)
- [GP-118: Anthropic 工程師的 Claude Code Skills 實戰筆記：九大類型、設計心法、還有那些踩過的坑](/posts/gp-118-20260318-trq212-claude-code-skills-lessons/)
- [MP-64: Matt Pocock 的 Git Guardrails：讓 Claude Code 不再手滑 git push --force 毀掉你的 Repo](/posts/mp-64-20260211-mattpocockuk-git-guardrails-skill/)
