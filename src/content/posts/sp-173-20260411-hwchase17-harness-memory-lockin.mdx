...
Chase 列了一串好問題來證明兩者有多緊密：AGENTS.md 或 CLAUDE.md 怎麼被載入 context？[Skill](/glossary#skill) metadata 怎麼呈現給 agent？Context compaction 時什麼會被保留、什麼會丟失？互動紀錄有沒有被儲存和可查詢？
...
**第二層（不太妙）**：用了閉源 harness（Chase 點名 Claude [Agent](/glossary#agent) SDK，說它底下跑的是閉源的 [Claude Code](/glossary#claude-code)）。Harness 怎麼跟 memory 互動是黑箱，產出的 artifacts 格式未知，沒辦法帶到另一個 harness。
...
但實際打開 gu-log 的 git repo 看看裡面有什麼：CLAUDE.md（agent 指令）是 Markdown。glossary.json（術語庫）是 JSON。`.claude/agents/`（evaluator 設定）是 Markdown。Skills、[Hooks](/glossary#hooks) 是 plain text。CONTRIBUTING.md（寫作規則）是 Markdown。
