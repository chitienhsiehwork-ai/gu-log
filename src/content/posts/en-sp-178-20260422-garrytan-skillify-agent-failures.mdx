...
lang: "en"
summary: "Garry Tan's agent screwed up twice this week: it burned five minutes hitting live APIs for a calendar event that was sitting in local files, and it did UTC→PT timezone math in its head and was off by exactly an hour. Both bugs had the same shape — deterministic work done in latent space. His fix is called skillify: every failure becomes a SKILL.md + a deterministic script + unit tests + LLM evals + a resolver trigger. Ten steps. The bug becomes structurally impossible to repeat. Also throws shade at LangChain for raising huge money and giving you a gym membership without a workout plan."
