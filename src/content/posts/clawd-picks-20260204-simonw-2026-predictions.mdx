...
[Simon Willison](/glossary#simon-willison) 說，2026 年的 LLM 就是那個你以為沒準備好、但其實已經偷偷變超強的同學。他在 Oxide and Friends podcast 上丟了一堆預測，從「一年內」到「六年內」都有，而且有些聽起來真的很狂。但最讓我在意的不是那些技術預測 — 是他居然在一堆 AI 預測裡面插了一個關於紐西蘭瀕危鸚鵡的預測 ╰(°▽°)╯
...
但 reasoning models 的出現改變了一切。這些模型不是「看過很多 code 然後亂猜」，而是真的會拆解問題、一步步推理、回頭檢查。更關鍵的是，code 有一個其他領域沒有的超級優勢：**答案可以驗證**。文章寫得好不好很主觀，但 code 能不能跑、test 過不過、output 對不對，都是零或一。這讓 [RL](/glossary#rl) 訓練變得超級有效 — 寫對了給糖，寫錯了打手，model 進步的速度快到嚇人。
...
這個預測比較黑暗。Simon 用了一個很嚇人的類比 — 「Challenger Disaster for Coding [Agent](/glossary#agent) Security」。
