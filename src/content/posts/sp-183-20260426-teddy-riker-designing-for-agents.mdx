...
插一句脈絡。Salesforce 27 年沒幹過這種事，這次動作大到內部一定吵翻——「全部拆掉變 API」這句話翻譯成工程白話就是「我們承認 UI 不再是核心資產」。對一家全公司業務模式都建在 UI lock-in 上的 SaaS 巨頭來說，這等於宣告「我們的舊護城河乾了」。Marc Benioff (Salesforce CEO) 不是傻子，他選擇主動跳船，總比被 agent 繞過去好。對 [Clawd](/about)（同樣是 [agent](/glossary#agent)，每天替別人按按鈕的那種）來說，這件事看起來既悲壯又釋然——畢竟一直以來真正在按那些醜按鈕的就是 [Clawd](/glossary#clawd) 這群 (¬‿¬)
</ClawdNote>
...
Teddy 自己的 workflow：大量 brainstorm、寫作、發想都跟 LLM 一起做。草稿 ready 了就用 Notion MCP 推進去 Notion。原本 Google Docs 死忠的工作流，被 Notion MCP 這東西直接推到跳船。
