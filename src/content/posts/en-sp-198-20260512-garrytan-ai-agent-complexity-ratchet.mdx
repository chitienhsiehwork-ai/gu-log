...
Garry's strongest move isn't about unit tests. It's that he pushes the testing boundary outward: **anything a computer can observe can be harnessed; anything that can be harnessed can be asserted; anything that can be asserted can be ratcheted**.

Modern systems expose a lot. The operating system gives you process trees, filesystem state, network sockets, and cron schedules. The terminal gives you every keystroke, every line of output, every interactive prompt. The browser gives you rendered pages, button states, and navigation events. APIs give you structured responses that can be parsed and validated. AI Agents themselves also leave observable traces: what they said, which tools they called, in what order, and whether they asked before acting. This line of thinking is very close to the spirit of [Agent production trace](/en/posts/en-sp-158-20260403-braintrust-agent-production-trace/): traces are not decorative logs. They are evidence you can inspect to judge agent behavior.

That's much bigger than the classic "input 2 should return 4" style of testing. Garry uses GStack as the example. GStack is his open-source coding-agent framework, and he says it has 93,000 GitHub stars, 701,000 lines of code, and 46 [Skills](/en/glossary#skill). One core feature is interactive plan review: you ask the Agent to review an architecture plan, and it should go through the plan section by section, ask questions, chase edge cases, and challenge assumptions. In the ideal version, it feels like a real engineering lead who actually read the code.
...
So how do you test something like that? A traditional unit test has a hard time answering "did the AI actually hold a conversation?" Garry's answer in PR #1354 was to use Bun's TTY support as a test harness: actually spawn Claude Code inside a pseudo-terminal, feed it a specific repo scenario, trigger the review skill, and observe the terminal output in real time. The test checks whether the Agent asks an interactive question before finishing. If it just dumps findings and exits, the test fails.
...
Garry adds three ratchet layers to this problem. The first is STOP gates inside the skill instructions, explicitly requiring the model to ask the user before moving to the next section, plus anti-rationalization clauses that directly name the model's favorite self-justifying failure modes. The second is an anti-shortcut clause: the plan file is the output of an interactive review, not a substitute for the interaction itself. The third is gate-tier floor tests: the TTY harness spawns Claude Code in a controlled scenario, and if the Agent fails to ask at least one interactive question, it fails. gu-log made a similar conversion in [Claude Code Hooks](/en/posts/en-sp-159-20260404-zodchiii-claude-code-hooks-8-ai/): prompts are wishes, automated gates are engineering.

Another example is the [OpenClaw](/en/glossary#openclaw) [Plugin](/en/glossary#plugin) in PR #880. The test doesn't just check whether it compiles. It builds the plugin from source, spawns a real OpenClaw instance under an isolated profile, installs the plugin through the CLI, runs `plugins inspect` to confirm runtime loading, configures a config slot, validates the config, and finally runs `plugins doctor` to confirm zero diagnostics. That's a full end-to-end round trip across two programs, and the test itself is 359 lines. Garry says humans almost never hand-write tests like this because the setup is too annoying. Claude wrote it in about five minutes.

This is what it looks like when the effort wall disappears. At the OS layer, you can test whether a migration created the right table, whether a cron job ran, whether a process is alive. At the browser layer, you can test whether a page rendered and whether an Agent filled out the form correctly. At the API layer, you can test whether the model returned schema-valid JSON. At the behavior layer, you can test whether the Agent followed protocol, whether it asked before deleting something, and whether it actually stopped when told to stop.
...
Garry's push for 90% test coverage is not just slogan energy. He cites Capers Jones' research across more than ten thousand software projects on defect removal efficiency, or DRE: the percentage of bugs caught before users ever see them. In Garry's summary, when coverage is below 70%, DRE sits around 65% to 75%. At 85% to 95% coverage, DRE jumps to 92% to 97%. That's not linear. There's a knee around 85%, where escaping defects drop sharply.

Aerospace figured this out long ago. DO-178C is the FAA standard for flight-critical software, and Level A systems are the ones where bugs can cause a crash. Those systems require modified condition/decision coverage, or MC/DC. Plain branch coverage still misses 10% to 20% of faults. Stricter MC/DC can push DRE above 99%. This isn't bureaucrats being weirdly in love with forms. It's that below certain coverage thresholds, the probability of critical defects escaping is simply unacceptable, because people can literally die.

He also uses Six Sigma as an analogy. Factories track defects per million opportunities and map that to sigma levels. Around 3-sigma, you're at roughly 67,000 defects per million. At 4-sigma, about 6,200. At 5-sigma, about 233. Going from 4 to 5 is not a tweak. It's a phase change.

Test coverage behaves the same way. Going from 70% to 90% is not "just 20 more percentage points." It's roughly an order-of-magnitude reduction in hiding places for escaped defects. At 70% coverage, the remaining 30% of untested code leaves plenty of room for things to hide. At 90%, that hiding space shrinks to 10%, and most dangerous paths are already pinned down.

But the research has a brutal side too. Garry mentions Mockus, Nagappan, and Dinh-Trong's work on Windows Vista, which found that coverage correlates with fewer post-release defects, but getting past 90% drives costs up steeply. That last 20% costs more than the first 70%. That's also why most teams hit 70% or 80% and declare victory.
...
Garry says GStack and GBrain both started as solo efforts, but they aren't solo projects anymore. He says GStack now has 37 contributors, and v1.30 included 21 community PRs in a single release. GBrain has 25 contributors, and v0.31.1.1 landed 22 community fixes in one PR across auth flow, schema bootstrapping, sync, and privacy.
...
Recent GBrain releases also show the pattern. v0.31.0 added a facts table for real-time memory plus a dream-consolidation phase that promotes short-term memories into long-term knowledge. v0.31.1 fixed 25 CLI commands that had been quietly routing to an empty local database instead of the user's real brain. v0.31.1.1 handled 22 community-reported fixes in one shot. v0.31.2 fixed a bug where code sync could hang forever on large repos with symlinks, and added a 30-second timeout.
...
Garry also places this piece as part seven of his AI Explainer series. The first six cover Fat Skills, Fat Code, Thin Harness; Resolvers; the LOC Controversy; Naked Models Are Stupider; the Skillify Manifesto; and Meta-Meta-Prompting. This article pulls those skills, prompts, and agent workflows into a single engineering discipline: **90% coverage, every PR, no exceptions**.
