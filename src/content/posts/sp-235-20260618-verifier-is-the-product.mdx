...
這個模式的學術根源是 ReAct（Princeton 和 Google）：思考、行動、觀察結果、再思考，直到完成。用寫程式的話講：理解目標、寫 code、跑起來、讀錯誤訊息、推斷原因、修掉、再跑，直到測試通過。
...
這個模式的學術根源是 Reflexion：ReAct 加上記憶。當一次嘗試失敗，agent 用白話寫下「為什麼失敗」，存起來，下次嘗試時讀出來。在現代的工具鏈裡，那個筆記住在檔案裡，不是 context window 裡。SKILL.md、AGENTS.md、CLAUDE.md 都是這種外圈記憶的具體實作。
...
然後是 Anthropic 自己在公告裡寫的那句但書：**這個移植還沒上 production。**
