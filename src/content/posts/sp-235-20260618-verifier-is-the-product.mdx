...
[Boris Cherny](/glossary#boris-cherny) 在今年六月的 Fortune Brainstorm Tech 上說，他現在不自己寫 [prompt](/glossary#prompt) 了——是另一個 Claude 在寫 prompt。一個早上管幾百個 agent，有時候上千個。
...
這套做法也有名字，叫 [Reflexion](https://arxiv.org/abs/2303.11366)：ReAct 再加上一塊記憶。當一次嘗試失敗，agent 用白話寫下「為什麼失敗」，存起來，下次嘗試時讀出來。在現代的工具鏈裡，那個筆記住在檔案裡，不是 [context window](/glossary#context-window) 裡。SKILL.md、AGENTS.md、CLAUDE.md 都是這種外圈記憶的具體實作。
