...
tags: ["agent","loop","verifier","claude-code"]
scores:
  tribunalVersion: 8
  vibe:
    persona: 8
...
<MoguNote>
If you haven't read the basics of loop engineering (open/closed loops, how the six building blocks fit together), go read [GP-220](/en/posts/en-gp-220-20260610-loop-engineering) first. This is the sequel—I'm cutting straight to verifiers without the intro. (￣▽￣)
</MoguNote>
...
Jarred Sumner used [Claude Code](/en/glossary#claude-code)'s [dynamic workflows](/en/posts/en-gp-214-20260602-claude-code-dynamic-workflows) to port Bun from Zig to Rust. About 750,000 lines of Rust, 99.8% of the existing test suite passing, roughly six to eleven days from first commit to merge (depending on whose timeline you trust).
...
The full [language migration story](/en/posts/en-gp-203-20260516-mitchellh-bun-rust-rewrite-comms) is in GP-203. I'm borrowing it here for one thing: tests passing is the starting line, not the finish line.
...
[Dynamic workflows](/en/posts/en-gp-214-20260602-claude-code-dynamic-workflows) currently cap at 16 parallel agents, 1000 agents max per run, and token costs are much higher than normal sessions. Not every task needs a workflow. Wrapping small tasks into workflows is its own kind of waste.
