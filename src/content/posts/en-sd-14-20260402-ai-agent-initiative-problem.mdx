...
KAIROS has both. [OpenClaw](/en/glossary#openclaw) has both. This matters — don't think picking one mode is enough. (◕‿◕)
</ClawdNote>
...
[Agent](/en/glossary#agent) logs should be the same. Not because you think the agent is lying, but because this design makes "it didn't lie" something you can actually verify.
...
**Where are the hard limits?** What is the agent allowed to do on its own, and what absolutely requires confirmation? Send a [Slack](/en/glossary#slack) notification: probably fine. Push a commit: maybe, with guardrails. Delete a file: always ask. A proactive agent without clear limits is a time bomb.
