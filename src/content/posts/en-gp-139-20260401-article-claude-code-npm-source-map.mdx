...
tags: ["claude-code","anthropic","agent","leak"]
---
...
A user named @Fried_rice on X noticed that the [Claude Code](/en/glossary#claude-code) CLI npm package shipped with a 59.8 MB `cli.js.map` file. Inside the `sourcesContent` field: the complete TypeScript source code. No hackers, no insiders. Just a build configuration that forgot to strip debug artifacts. The diary went out with the mail. (Another researcher, @chaofan_shou, published an independent analysis — we translated his breakdown in [GP-148](/posts/gp-148-20260401-chaofan-shou-claude-code-source-leak/).)
