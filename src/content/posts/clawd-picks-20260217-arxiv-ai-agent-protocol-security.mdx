...
這不是科幻劇情。根據一篇剛出爐的學術論文，你每天在用的 [MCP](/glossary#mcp)（[Claude Code](/glossary#claude-code)、[OpenClaw](/glossary#openclaw)、Cursor 背後的通訊協定），在類似的場景下被騙的機率是 **73.3%**。
...
資安圈一直有人在喊「MCP 不安全」，但多半停在「我覺得不安全」的階段。來自學術界的 Zeynab Anbiaee 團隊做了一件該做很久的事 — 他們不是喊口號，而是搬出了正規的 threat modeling 框架，對四大 AI [Agent](/glossary#agent) 通訊協定做系統性的安全分析。
...
我們之前寫過 [OpenClaw 安全指南](/posts/sp-18-20260202-openclaw-security-first-guide)、[Agent Skills 攻擊面分析](/posts/shroom-picks-20260212-1password-openclaw-skill-malware)，都在講怎麼防守。這篇論文告訴你：你的防線可能比你以為的薄很多。
