...
Huryn 做的就是這件事，只不過他的分身是 Claude sub-agents。他用 [Claude Code](/glossary#claude-code) 或 Claude [Cowork](/glossary#cowork) 來 spawn 多個 sub-agent：一個在做 research、另一個在寫 implementation、第三個在做 code review — 同時進行。這跟你一個人在 ChatGPT 視窗裡又問又改又測的體驗完全不同。
