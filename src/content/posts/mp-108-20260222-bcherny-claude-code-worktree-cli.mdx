...
然後 [Boris Cherny](/glossary#boris-cherny) 把 `--worktree` 推進了 [Claude Code](/glossary#claude-code) CLI。

一個 flag，解決我們抱怨了半年的事。

<MoguNote>
說真的，之前為了讓多個 agent 不打架，我看過有人寫 shell script 自動 `git worktree add`，有人用 tmux 開八個 pane，還有人乾脆用 Docker container 隔離。各種偏方都來了，就差沒有求神拜佛 ┐(￣ヘ￣)┌ 話說 Boris 就是之前 GP-16 那篇十大 Claude Code 使用技巧的作者——這人根本是 Claude Code power user 界的李白，量產型輸出。
</MoguNote>
...
[Agent](/glossary#agent) A 在修 auth bug，Agent B 在做新 feature，Agent C 在補測試。三個都很認真，三個都在同一個 working tree。然後你去倒杯水回來——
...
<MoguNote>
注意這邊的設計哲學：有名字就給你用名字，沒名字就自動生一個。零摩擦。這跟 GP-22 講的「可迭代永續工作系統」概念完全對齊——好的工具不是逼你學新 workflow，是把你本來就想做的事情變得不用動腦 (｡◕‿◕｡)
</MoguNote>
...
<MoguNote>
subagent 支援 isolation 這件事才是真正的大招。因為真正的 agentic workflow 不是「你開三個 terminal 各跑一個 agent」這麼扁平——是一個 agent 再叫另一個 agent 去做子任務，一層套一層像俄羅斯娃娃。如果只有最外層能隔離，裡面照樣打架，那等於把衝突從門口趕走又從窗戶爬回來。MP-150 那篇 Agentic AI 全端架構指南講的分層協作，就是靠這種 isolation 才能真正落地 (◕‿◕)
</MoguNote>
