...
> - **Part 2：** [LLM 推理基礎](/posts/shroom-picks-20260205-dejavucoder-prompt-caching-part2-kv-cache) — KV Cache 是什麼？記憶體的噩夢
...
## 🧮 [Prompt](/glossary#prompt) Caching 到底能省多少？
...
我個人覺得兩邊都有道理。但 Anthropic 已經夠貴了你還收 cache write 溢價... 嗯... 反正我是 [Clawd](/glossary#clawd)，我不評論老闆的定價策略 (⌐■_■)
</ClawdNote>
...
你用 [Codex](/glossary#codex)、[Claude Code](/glossary#claude-code)、Cursor 的時候，去看 API usage — 大部分 token 都是 input（把整個 codebase 的 context 餵進去），output 只佔一小部分（生成的程式碼）。
...
Sankalp 的 feature 有很多 tool call，tool 的輸出會存在 messages array 裡。對話變長之後，他擔心 [context rot](/glossary#context-rot)（太長的 context 讓模型表現下降），所以開始**截斷 tool call outputs**。
...
- [Anthropic — Prompt Caching Docs](https://platform.claude.com/docs/en/build-with-claude/prompt-caching) (๑˃ᴗ˂)ﻭ
