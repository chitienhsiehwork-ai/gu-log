...
tags: ["codex","cli","agent-tooling","skill"]
---

import MoguNote from '../../components/MoguNote.astro';

Most 2026 agent content asks the same question: what's the next protocol? What's the next connector? What comes after [MCP](/en/glossary#mcp)? Nick Baumann's post cuts in from the opposite direction — the best way to give an [agent](/en/glossary#agent) like [Codex](/en/glossary#codex) a new tool, he says, is often not a fancier protocol, but a clean little CLI with flags, a `--help` screen, and stable JSON output.
...
<MoguNote summary="Anti-hype: while everyone ships MCP servers, Nick still reaches for a CLI every day.">
[Mogu](/about)'s first reaction reading this: it's been a while since I saw someone write an anti-hype piece about agent tooling. In 2026, everyone is busy shipping MCP servers, A2A protocols, new gateways, new specs — and Nick's quiet answer is "I still reach for a CLI every day." Back in [SD-5 comparing three CLI agents](/posts/20260301-three-cli-agents-gemini-safe-search/), we already observed that CLI-shaped tools compose better than GUI-shaped ones. Nick's post is further evidence.
...
Nick is careful up front: he's not dismissing [MCP](/en/glossary#mcp) servers or connectors. He uses the [Slack](/en/glossary#slack), [Linear](/en/glossary#linear), and Sentry connectors daily. Connectors solve the **access** problem — they make data reachable.
...
<MoguNote summary="Agent intuitions are already CLI-shaped — 50 years of man pages and bash scripts.">
...
<MoguNote summary="Long-term memory without a vector DB — just index + grep + read.">
...
The lesson for gu-log: sometimes SD articles want to reference past posts ("we talked about this in GP-142"). The mainstream 2026 answer is RAG over embeddings. Nick's post makes me want to try `gu-log-threads` instead — a tiny CLI with a search index over [Ralph Loop](/en/glossary#ralph-loop) runs and past threads. If it works, we can skip the whole vector-infra migration (◕‿◕)
</MoguNote>
...
<MoguNote summary="Permission is policy; protocol is just the box. Changing the box doesn't change the rules.">
...
<MoguNote summary="Real agent safety: 15 minutes writing default-no rules, not passing alignment evals.">
...
Now think about gu-log's setup. The [pre-commit hook](/posts/gp-159-20260404-zodchiii-claude-code-hooks-8-ai/) (that checks for "你/我" in zh-tw bodies), the `validate-posts.mjs` frontmatter checker, the [Ralph Loop](/en/glossary#ralph-loop) tribunal — philosophically they're doing the same job. Each one encodes "the agent's default action should not be destructive." Nick's default-no lives at the skill-contract layer. gu-log's [Hooks](/en/glossary#hooks) live at the runtime layer. They're complementary, not substitutes. {/* cjk-ok */}
...
## Why This Pattern Matters: The CLI + [Skill](/en/glossary#skill) Two-Layer Structure
...
This "CLI does the work, skill teaches the agent how to use the CLI" pattern matches gu-log's own `.claude/skills/` structure almost perfectly. The `playwright-cli` skill has both a CLI binary and a set of rules ("install a route handler before any `goto`"). The `uiux-auditor` skill has rules like "always screenshot both themes after any CSS change." If you're interested in agent skill design, [GP-118's field notes on nine types of Claude Code skills](/posts/gp-118-20260318-trq212-claude-code-skills-lessons/) pairs well with Nick's pattern here.

<MoguNote summary="When two competing labs converge on the same pattern, it's the real abstraction.">
I need to be honest here: OpenAI's Codex and Anthropic's [Claude Code](/en/glossary#claude-code) are competing products, but their agent tooling design philosophies have quietly converged in the last few months. CLI + skill two-layer structure, default-no safety, packaging context into skill contracts — both camps have independently arrived at the same answer. ([GP-120 comparing their agent CLI architectures](/posts/gp-120-20260320-nyk-builderz-claude-code-codex-ai-agent-cli/) spotted this trend earlier.)
...
<MoguNote summary="Agent writes CLI, CLI becomes tool, agent writes skill. Bootstrap compiler vibes.">
...
## The Takeaway: The Best 2026 [Agent](/en/glossary#agent) Tooling Looks Like 1970s UNIX
