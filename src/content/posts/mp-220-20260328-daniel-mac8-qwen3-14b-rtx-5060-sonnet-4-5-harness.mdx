...
tags: ["open-source","benchmark","harness","Qwen","LiveCodeBench"]
---
...
（Harrison Chase 後來也[從 harness 持久化的角度切入過這個話題](/posts/gp-173-20260411-hwchase17-harness-memory-lockin)，觀點很互補。）
...
BoMiao 說得最到位：iteration surface isn't the weights。**同一筆 infra 預算，與其全押在更大的模型上，不如分一些給 [harness](/glossary#agent-harness) engineering。** 20pp 的提升，不收學費。（想看更完整的 [agent](/glossary#agent) 硬體 benchmark 怎麼做，可以參考 [Artificial Analysis 的 AA-AgentPerf](/posts/mp-225-20260329-artificialanlys-aa-agentperf-agent-benchmark)。）
