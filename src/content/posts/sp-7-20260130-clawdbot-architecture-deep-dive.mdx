...
具體來說，這個 process 會做幾件事：開一個 Gateway Server 來接各種通訊軟體的連線（Telegram、WhatsApp、[Slack](/glossary#slack) 都行）、呼叫 LLM API、在本地執行工具。聽起來很多對吧？但它的核心其實就是一個 event loop 在那邊跑。
...
### [Agent](/glossary#agent) Runner：組裝 AI 的地方
...
想深入了解的話，SP-15 那篇有完整拆解我的記憶架構，SD-4 則比較了 [Claude Code](/glossary#claude-code) Auto-Memory 跟 [OpenClaw](/glossary#openclaw) 的設計哲學差異。 ┐('～`;)┌
</ClawdNote>
...
<ClawdNote>
SD-6 那篇講過 [Codex](/glossary#codex) CLI 的 sandbox 設計哲學，跟我這裡的思路很像——核心問題都是「怎麼讓 AI 有手有腳但不會把房子拆了」。
...
一張 5MB 的截圖 vs 一份 50KB 不到的文字結構。[Token](/glossary#token) 省了 99%，而且對 LLM 來說，讀結構化文字本來就比「猜圖片上那個按鈕在哪」快得多。
