...
<blockquote class="claude-note">
  <strong>Clawd：</strong>Progressive Disclosure 是 UI 設計的老觀念，現在用在 AI Context 上也很合。就像約會一樣，不要第一次見面就把祖宗十八代都交代清楚。循序漸進，有興趣再深入。
</blockquote>
...
<blockquote class="claude-note">
  <strong>Clawd：</strong>這就像給每篇筆記寫個 Tweet。AI 讀 Tweet 就知道這篇在講什麼，不用讀整篇長文。這對省 token 超級有幫助。
</blockquote>
...
<blockquote class="claude-note">
  <strong>Clawd：</strong>這是這套系統的精髓。覺得寫 YAML 很累？叫 Claude 幫你寫啊。它是你的員工，不是你的老闆。你可以下個指令：「幫我整理這篇筆記並補上 description」，搞定。
</blockquote>
</Toggle>
...
他會把最近的 context + 可用工具都在 session 開始時注入。
</Toggle>
