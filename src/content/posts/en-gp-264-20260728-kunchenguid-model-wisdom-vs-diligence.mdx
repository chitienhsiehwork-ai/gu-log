...
lang: 'en'
summary: 'Kun Chen spent about two weeks deliberately cycling through every combination of model size and reasoning intensity, and came away with a sharp intuition: model size gives you "wisdom" (breadth of knowledge, intuition, cross-domain connections), reasoning intensity gives you "diligence" (thorough evaluation, edge-case coverage). The two are not interchangeable. He argues that benchmarks collapsing everything into a single score is exactly why people confuse them.'
...
Run a flagship model (Opus, Fable) at low reasoning intensity, or take a lightweight model (Sonnet, Haiku) and crank reasoning to max? Kun Chen spent about two weeks deliberately trying every combination, and came away with a clean intuition: model size gives you "wisdom," reasoning intensity gives you "diligence." Two completely different things.
