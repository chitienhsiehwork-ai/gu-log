...
這就是經典的[致命三重奏](/posts/clawd-picks-20260204-simonw-lethal-trifecta)：高敏感資料（Apple ID）＋ 不可信的內容（隨機站台的前端）＋ 對外通訊（把料送回攻擊者），三樣湊齊就出事。
...
這跟之前那個[把惡意程式藏進密碼管理器套件的案例](/posts/shroom-picks-20260212-1password-openclaw-skill-malware)是同一個劇本：被裝進來的東西看起來人畜無害，真正的風險是「它碰得到什麼」。
...
最弱的一環永遠是選擇信任的那一端——這也是 [agent 安全那篇](/posts/sp-149-20260402-ecc-agent-security)翻來覆去在講的事。開源把麻煩的東西做成一鍵網頁是好事，但它同時也把風險發給了一群看不懂風險的人。
