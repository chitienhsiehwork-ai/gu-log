...
Figma's latest move attacks this directly. Through their [MCP](/en/glossary#mcp) server, AI [agents](/en/glossary#agent) can now work inside your Figma files — not drawing random shapes, but connecting to your design system, reading your component library, and using your variables. Figma had [previously built code-to-canvas](/posts/clawd-picks-20260217-figma-claude-code-to-canvas/) as a one-way street from code to design. The new `use_figma` tool makes it a two-way operation on the entire canvas.
...
**`use_figma`** is the professor — the new feature. It lets [agents](/en/glossary#agent) work directly on the canvas the way your team defined things. Not drawing with a blank pen, but opening your component library, reading your variable names, and following your auto layout rules.
...
Okay, the [agent](/en/glossary#agent) is inside the canvas now. But here's the problem.
...
Anthropic's [Claude Code](/en/glossary#claude-code) product lead Cat Wu put it well:
...
OpenAI's [Codex](/en/glossary#codex) design lead Ed Bayes said in the article:
...
Because this capability is built natively into the [MCP](/en/glossary#mcp) server, it leverages Figma's existing security and stability while opening up access to Code Connect, Figma Draw, FigJam, and other surfaces through the [Plugin](/en/glossary#plugin) API. [MCP as a standard protocol for connecting agents to external tools](/posts/sp-180-20260423-anthropic-mcp-production-agents/) plays a clear role here: one entrance, multiple agents sharing it. The MCP clients listed in the article: Augment, [Claude Code](/en/glossary#claude-code), [Codex](/en/glossary#codex), Copilot CLI, Copilot in VS Code, Cursor, Factory, Firebender, and Warp.
...
This isn't just corporate hedging. Think about it: a human designer works eight hours a day — one seat, easy math. But an [agent](/en/glossary#agent) might cross ten files and modify three hundred components in five minutes. One seat? Ten seats? Charge by API call? Running one self-healing loop might burn through a massive number of calls. **The "seat" as a pricing unit was designed for humans. When the user is no longer human, the billing model itself needs reinventing.**
...
What Figma just did is give that nightmare a new escape route: not by relying on human discipline (that was never going to work), but by letting [agents](/en/glossary#agent) walk in carrying your design system's context, do the work, check their own output, and catch token drift automatically. `use_figma`, skills, self-healing loop, the path toward Plugin API parity — all on the same track.
