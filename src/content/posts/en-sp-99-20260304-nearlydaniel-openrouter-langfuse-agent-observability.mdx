...
This is how most people develop AI [agents](/glossary#agent).

Agent gets stuck? Edit the system prompt. Makes a dumb decision? Add a rule: "NEVER do X." Still broken? Add ten more rules. You do not even know if the problem is the prompt, the tool response, or the [context window](/glossary#context-window) overflowing. You just keep tweaking in the dark.
...
The community had other perspectives too. Some pointed out that OpenRouter can be more expensive than calling Anthropic or OpenAI APIs directly. Others proposed a more DIY approach: if you use [OpenClaw](/glossary#openclaw), you can go straight to `~/.openclaw/agents/main/sessions/` and grab the session logs, then write your own parser to read reasoning traces.
