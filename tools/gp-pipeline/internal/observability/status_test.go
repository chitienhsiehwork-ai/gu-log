...
func TestCheckPendingArtifacts_FindsPendingPostsAndProgress(t *testing.T) {
	...
	mustWrite(t, filepath.Join(root, "src", "content", "posts", "gp-pending-20260508-test.mdx"), "---\n---\n")
	mustWrite(t, filepath.Join(root, "scores", "tribunal-progress.json"), `{
  "gp-pending-20260508-test.mdx": {
    "article": "gp-pending-20260508-test.mdx",
    "stages": {}
  }
}
`)
...
func TestCollect_AllowsCurrentPendingArtifactsWhileRalphRuns(t *testing.T) {
	cfg, root := makeStatusRepo(t)
	workDir := filepath.Join(root, "tmp", "gp-pending-obs")
	mustMkdir(t, workDir)
...
`)
	...
	if err := WriteSnapshot(cfg, SnapshotInput{
		...
		RunState:       "running",
		CurrentStep:    "ralph",
		ActiveFilename: "gp-pending-20260508-test.mdx",
	}); err != nil {
		t.Fatal(err)
	}
...
`)
	...
	if err := RenameTribunalProgressEntry(cfg.RepoRoot, "gp-pending-20260508-test.mdx", "gp-201-20260508-test.mdx"); err != nil {
		t.Fatal(err)
	}
	...
	if _, ok := progress["gp-pending-20260508-test.mdx"]; ok {
		t.Fatal("old pending key still exists")
	}
	entry, ok := progress["gp-201-20260508-test.mdx"]
	if !ok {
		t.Fatal("new key missing after rename")
	}
	if entry["article"] != "gp-201-20260508-test.mdx" {
