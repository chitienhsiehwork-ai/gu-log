...
// Write is pipeline Step 2. It renders the
...
func (s *State) Write(ctx context.Context) error {
	...
	if s.PromptTicketID == "" {
		s.PromptTicketID, err = counter.PendingTicketID(s.Prefix)
		if err != nil {
			return fmt.Errorf("write: %w", err)
		}
	}
	if err := counter.ValidateTicketIDForPrefix(s.PromptTicketID, s.Prefix); err != nil {
		return fmt.Errorf("write: %w", err)
	}
