// Package deploy implements gp-pipeline Step 5 —
...
// --title-slug would silently produce a filename like "gp-252---.mdx"
...
type Options struct {
	...
	// SkipBuild disables `pnpm run build` (test convenience).
	SkipBuild bool
...
func RunExisting(ctx context.Context, opts Options) (*Result, error) {
	...
	if !opts.SkipBuild {
		if err := runPnpmBuild(ctx, opts.Cfg.RepoRoot); err != nil {
			return nil, fmt.Errorf("deploy: pnpm run build failed: %w", err)
		}
	}
...
func Run(ctx context.Context, opts Options) (*Result, error) {
	...
	// Canonical taxonomy gate: the prefix must be a live series and the
	// pending files must carry the matching canonical PENDING ticket. This
	// runs before any git / counter access so a retired-series handoff fails
	// loud without touching operator state.
	if err := counter.ValidatePrefix(opts.Prefix); err != nil {
		return nil, fmt.Errorf("deploy: %w", err)
	}
	prefixLower := strings.ToLower(opts.Prefix)
	if !strings.HasPrefix(opts.ActiveFilename, prefixLower+"-pending-") {
		return nil, fmt.Errorf("deploy: active filename %q must start with %q", opts.ActiveFilename, prefixLower+"-pending-")
	}
	if opts.ActiveENFilename != "" && opts.ActiveENFilename != "en-"+opts.ActiveFilename {
		return nil, fmt.Errorf("deploy: English pending filename %q must equal %q", opts.ActiveENFilename, "en-"+opts.ActiveFilename)
	}
	pendingTicketID, err := counter.PendingTicketID(opts.Prefix)
	if err != nil {
		return nil, fmt.Errorf("deploy: %w", err)
	}
	if err := validatePendingTicketID(filepath.Join(postsDir, opts.ActiveFilename), pendingTicketID); err != nil {
		return nil, err
	}
	hasEN := false
	if opts.ActiveENFilename != "" {
		enPath := filepath.Join(postsDir, opts.ActiveENFilename)
		if _, statErr := os.Stat(enPath); statErr == nil {
			hasEN = true
			if err := validatePendingTicketID(enPath, pendingTicketID); err != nil {
				return nil, err
			}
		} else if !os.IsNotExist(statErr) {
			return nil, fmt.Errorf("deploy: stat English pending file %s: %w", enPath, statErr)
		}
	}
	...
	if err := replacePendingTicketID(filepath.Join(postsDir, finalFilename), pendingTicketID, ticketID); err != nil {
		return nil, err
	}
	if finalEN != "" {
		if err := replacePendingTicketID(filepath.Join(postsDir, finalEN), pendingTicketID, ticketID); err != nil {
			return nil, err
		}
	}
...
// replacePendingTicketID opens a file, verifies its frontmatter ticketId still
// equals the expected canonical PENDING value, replaces it with ticketID, and
// writes the result. This is the Go port of the three-sed-call
// replace_pending_ticket_id function in bash.
func replacePendingTicketID(path, expectedPending, ticketID string) error {
	...
	value, ok := f.GetScalar("ticketId")
	if !ok {
		return fmt.Errorf("replace pending: %s has no ticketId", path)
	}
	if strings.Trim(value, `"'`) != expectedPending {
		return fmt.Errorf("replace pending: %s has ticketId %s; expected %s", path, value, expectedPending)
	}
	f.SetScalar("ticketId", `"`+ticketID+`"`)
	return os.WriteFile(path, f.Bytes(), 0o644)
}

// validatePendingTicketID confirms a pending file still carries the canonical
// PENDING ticket for its series before the counter is touched, so a handoff
// carrying a retired-series pending ticket fails loud instead of being
// silently published.
func validatePendingTicketID(path, expected string) error {
	data, err := os.ReadFile(path)
	if err != nil {
		return fmt.Errorf("deploy: read pending file %s: %w", path, err)
	}
	f, err := frontmatter.Parse(data)
	if err != nil {
		return fmt.Errorf("deploy: parse pending file %s: %w", path, err)
	}
	value, ok := f.GetScalar("ticketId")
	if !ok {
		return fmt.Errorf("deploy: pending file %s has no ticketId", path)
	}
	if strings.Trim(value, `"'`) != expected {
		return fmt.Errorf("deploy: pending file %s has ticketId %s; expected %s", path, value, expected)
	}
	return nil
}
...
func runPnpmBuild(ctx context.Context, repoRoot string) error {
