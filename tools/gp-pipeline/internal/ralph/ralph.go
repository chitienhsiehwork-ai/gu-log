...
// scripts/tribunal.sh — the standalone 4-judge tribunal that
...
// call back into a retired shell pipeline, so wrapping it is safe and the Go implementation
...
type Options struct {
	// RalphScript is the absolute path to scripts/tribunal.sh.
	RalphScript string
...
func Run(ctx context.Context, opts Options) (passed bool, err error) {
	...
	if res != nil {
		...
		// is also written to .score-loop/logs/ by tribunal.sh.
