...
func newEvalCmd(state *rootState) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "eval",
		Short: "Evaluate whether a captured source is GP-worthy",
		Long: `eval runs the pipeline's two-evaluator gate at Step 1.5.
...
compares the two verdicts:
  ...
  SKIP/SKIP → exit 12 (not GP-worthy; caller should drop from queue)
...
--force skips the gate entirely and exits 0 without calling the LLM.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			return runEval(cmd.Context(), state, sourcePath, workDir, force)
		},
	}
...
func runEval(ctx context.Context, state *rootState, sourcePath, workDir string, force bool) error {
	start := time.Now()
	...
	disp, err := buildDispatcherForRole(state, dispatcherJudge)
	if err != nil {
		return err
	}
