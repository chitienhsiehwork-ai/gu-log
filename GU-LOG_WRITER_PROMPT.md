...
- **比喻先選世界，再決定要不要開門**：比喻的工作是降低理解成本，不是替每段換造型。需要核心比喻時，動筆前先選一個能承載全文的故事視角，後文沿用同一套角色與映射；原文直接講更清楚時就不用比喻。一篇最多三套獨立比喻系統，三套是上限，不是配額；第二、第三套若不能解釋核心比喻承載不了的重要概念，就刪掉。臨時從考場跳到駕照、再跳到高速公路，只會讓讀者重新配對角色、增加認知負擔。
