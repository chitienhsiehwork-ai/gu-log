...
## GAP #2: `ralph-all-claude.sh` 失敗不應 kill loop
...
如果 Builder 照慣例加 `set -euo pipefail`（兩個 existing scripts 都用），那 `ralph-all-claude.sh` exit 1（article FAIL）會直接終止整個 loop。Systemd 會 restart（60s delay），但這代表**每個 FAIL 的 article 都浪費 60 秒 restart penalty**。

**應該**: `bash scripts/ralph-all-claude.sh "$next_article" || tlog "Article failed, continuing..."`
...
### 5. Quiet hours 依賴 `ralph-all-claude.sh` 內部處理
Spec 正確指出不需要在 loop 層重複 quiet hours logic。✓ 已驗證 `ralph-all-claude.sh` 有 `is_quiet_hours()` + `wait_for_quiet_hours_end()`。
