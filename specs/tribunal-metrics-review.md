...
- Vibe Scorer = calibrated veteran（讀過所有 golden standards，知道 GP-158 trap）
...
**Why drop pronoun clarity from Librarian?** Vibe Scorer 的 `clarity` dimension 更 well-calibrated（有 GP-158 教訓、有 rubric anchors），重複測量沒有額外 signal。

### Decision 3: 新增 `narrative` dimension（Vibe Scorer）

**Problem**: 敘事結構是 gu-log 品質的核心差異化因素（GU-LOG_WRITER_PROMPT 整個 Narrative Structure section），也是 GP-158 核心教訓（decorative persona + linear structure = trap）。但沒有任何 dimension explicitly 測結構。
...
**Vibe Scorer 新 dimensions**: `persona` · `moguNote` · `vibe` · `clarity` · `narrative`
...
- 找到 2 個真實 calibration articles：
  - **GP-14** (high anchor, 9/10): Anthropic 研究，精確 stats，完整 hedge preservation
  - **MP-153** (medium anchor, 8/10): Tweet 來源，架構正確，numbers verifiable
...
- [x] Calibration articles (GP-14, MP-153) 寫入 scoring SSOT
...
**Research completed** (see `specs/tribunal-calibration-research.md`):
- 調查了 3 篇 EN 文章（GP-14, GP-2, GP-24），全部 8/8/8 — zh-tw rubric 沒 capture EN-specific variance
...
3. **MoguNote / Vibe / Narrative**: 同 zh-tw bar，不需要 EN-specific adjustment。
...
- [ ] **MR-4**: Fact Checker has ≥ 2 calibration examples (GP-14, MP-153) with per-dimension 0-10 scores + anchor tables
