...
`PENDING` 是 gu-log 的標準新文章流程：草稿、tribunal、修稿期間都先維持 `SP-PENDING` / `CP-PENDING` / `SD-PENDING` / `Lv-PENDING`，檔名也維持 `*-pending-*`。真的要上 main 前，才跑 `node scripts/allocate-ticket.mjs` 把 frontmatter 和檔名換成真號，並在那一步 bump `scripts/article-counter.json`。
