...
3. **Mogu / MP pipeline 自動產**（VPS 上的 agent）
   - `scripts/gp-pipeline.sh`、`scripts/mogu-picks-prompt.md`、`cp-candidates-queue.yaml`
...
- **Mac**：負責把草稿 import 成 MDX（含 frontmatter、component、PENDING ticket ID）、跑品質 tribunal、merge 前配真號、push。
- **線上**：`gu-log.vercel.app` 正式版。你可以在 iPhone 上同時開 Obsidian（看原始草稿）和 Vercel（看成品），比較 frontend 帶來的加值——字型、TOC、Solarized 主題、MoguNote 的吐槽框 render、reading progress、系列導覽。
...
再建 `_templates/sp-draft.md`（GP 系列）：
...
4. 連結到其他文章用 wikilink：`[[sp-100-xxx]]`，import 時會自動轉成 `/posts/gp-100-xxx`
...
git commit -m "content(sd-pending): draft from obsidian"

node scripts/allocate-ticket.mjs      # merge 前：換真號 + bump counter
git add scripts/article-counter.json src/content/posts/
git commit -m "content(sd-20): allocate ticket"
git push
```
...
**Q: 草稿 import 後想重改怎麼辦？**
- 改 vault 裡的 `.md` → 重新 `obsidian-import.mjs`？通常不要，目標 `*-pending-*` 檔案已存在會擋住。
