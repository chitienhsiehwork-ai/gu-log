...
The tribunal scoring system SHALL assign every scoring dimension to exactly one judge. Dimension ownership SHALL be selected by the post's `tribunalVersion`: posts at `tribunalVersion >= 9` use the new ownership; posts at `tribunalVersion <= 8` use the legacy ownership unchanged.
...
- **THEN** the floor gate SHALL require all five legacy vibe dims present AND composite >= 3
...
Posts authored at `tribunalVersion <= 8` SHALL continue to validate and render with their existing frontmatter (including `clarity` under `scores.vibe`). The change SHALL NOT require any migration of, or edit to, existing post frontmatter.

#### Scenario: Legacy post with clarity under vibe still validates

- **WHEN** a `tribunalVersion <= 8` post with `scores.vibe.clarity` present is validated
- **THEN** validation SHALL pass
- **AND** the post SHALL NOT be re-gated or flagged solely because clarity lives under `scores.vibe`
