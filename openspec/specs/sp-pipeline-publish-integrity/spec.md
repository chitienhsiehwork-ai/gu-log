# sp-pipeline-publish-integrity Specification

## Purpose

`gp-pipeline`（`tools/sp-pipeline/`）發佈路徑（standalone deploy 的檔名建構、write→translate 的雙語契約、frontmatter 的 YAML 序列化）SHALL fail closed：不得產生格式錯誤的檔名、不實的 help text、或無效 YAML frontmatter。本 capability 由 gu-log issue #546 觸發，收斂三個曾在真實 pipeline 跑程（SP-252）中發生的 fail-open 缺口。
