...
Iris / Clawd 的 broad GitHub operator credentials SHALL 只作用在 dedicated AI lab organization，prefer name 為 `shroomdog-ai-lab`，並且 SHALL NOT 包含 `chitienhsiehwork-ai/gu-log`。

#### Scenario: 為 Iris 或 Clawd 建立 broad token

- **WHEN** operator 為 Iris 或 Clawd 建立 broad GitHub token
...
- **WHEN** Iris 或 Clawd 需要建立新的 experimental open-source repo
