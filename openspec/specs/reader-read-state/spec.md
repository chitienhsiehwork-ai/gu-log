...
## Purpose
Define Reader Tracker's version-aware read-state model, including per-article read records, legacy slug-list migration, stale/current/unread semantics, progress counting, and sync merge behavior that preserves the read revision associated with the latest read.
