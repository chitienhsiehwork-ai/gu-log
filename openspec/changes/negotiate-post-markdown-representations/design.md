...
- 以真實 Vercel 預覽／正式環境驗證內部改寫、HEAD、308 與 CDN 快取隔離。
...
只有沒有尾端斜線、沒有副檔名、只有一個安全 slug 區段的正式文章路徑會進行協商。帶尾端斜線的請求交回 Vercel 現有 308；跟隨轉址後才在正式路徑協商。明確 `.md`、`/api/**`、資產、列表與舊 slug 轉址都不由中介層改寫，因此狀態與 Location 契約不變。
...
3. 擴充部署 verifier，先在預覽驗 q 權重、HEAD、Vary、308、API／`.md` non-regression 與快取隔離。
