...
- [x] 1.3 在 spec 中把「重組敘事 = 改 packaging，不改 payload」寫成 SP/CP body 的單點權威定義。
...
- [ ] 2.1 封存本 change 後，把 `CONTRIBUTING.md` 中 SP/CP、Lv、系列定位與 note/body 邊界相關段落降級成 derived view，指向 `openspec/specs/editorial-charter/spec.md`。
...
- [ ] 2.5 檢查 ShroomDog / Clawd / Mogu / SP / CP / GP 品牌名與系列名的現況；若要收斂，另開 change，不在本 change 內 rename。
