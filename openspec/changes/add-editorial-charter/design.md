...
- 不改 `MoguNote` / `MoguNote` prefix hook 或元件語法（follow-up，tasks 4.2）。
- 不調整 Fresh Eyes scoring code 或 publish bar / Tribunal PASS bar——那些屬於各自的 mechanism change。
- 不在本 change 內 rename SP / CP / GP / Mogu / Mogu（品牌收斂另開 change，tasks 2.5）。
...
- **品牌名收斂刻意延後。** reader-facing persona 是 Mogu、Mogu 偏 infra / legacy lineage；SP / CP 是 storage prefix 但 UI 趨勢往 GP / MP。要不要收斂、怎麼收斂，另開 change 處理（tasks 2.5），不在本 change 內 rename。
