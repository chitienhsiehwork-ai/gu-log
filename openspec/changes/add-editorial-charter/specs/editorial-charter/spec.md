...
所有 gu-log 意見、吐槽、玩笑、外加類比與 commentary SHALL 進 MoguNote 或 MoguNote，SHALL NOT 塞進翻譯 body。
...
- **AND** writer SHALL 在發布前修正，或把 gu-log 外加材料移進 MoguNote / MoguNote
...
- SP = ShroomDog 選的外部好文，忠實翻譯並加 MoguNote commentary
- CP = Mogu / Mogu 選的 tweet 或短 source，其他 body / note 邊界同 SP
...
- **THEN** 它 SHALL 把 SP 描述為 ShroomDog-selected external good writing with faithful translation plus MoguNote commentary
- **AND** SHALL NOT 把 SP 描述成自由改寫

#### Scenario: CP identity is cited

- **WHEN** doc、prompt 或 judge 描述 CP
- **THEN** 它 SHALL 把 CP 描述為 Mogu / Mogu-selected tweets or short-form sources
...
- **AND** 這類 commentary SHALL 改放進 MoguNote 或 MoguNote
