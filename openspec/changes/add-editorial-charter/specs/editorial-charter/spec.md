...
### Requirement: SP and CP body MUST be faithful translation

SP 與 CP 的 body SHALL 是忠實翻譯。

忠實翻譯 SHALL NOT 等於逐字保留所有 inventory。writer MAY 重組敘事、刪掉不重要的專有名詞、合併重複材料，並翻成自然的 PTT 風繁中；前提是這樣做會讓 source idea 更清楚。

對 SP 與 CP body，「重組敘事」只有一個權威定義：
...
所有 gu-log 意見、吐槽、玩笑、外加類比與 commentary SHALL 進 ClawdNote 或 MoguNote，SHALL NOT 塞進翻譯 body。
...
### Requirement: SP and CP body MUST NOT alter source payload

SP 與 CP body SHALL NOT 加入 source 沒有的事實、數字、結論、因果關係或主張強度。SP 與 CP body SHALL NOT 抹掉 source 的 hedge、caveat、限制或條件。SP 與 CP body SHALL NOT 軟化、反轉或磨掉 source 真正的刀口。
...
- **AND** writer SHALL 在發布前修正，或把 gu-log 外加材料移進 ClawdNote / MoguNote
...
Lv-guided-reading SHALL 落在 SD 與 SP 中間：它是在「教」這篇 source，不是在翻譯它。它 MAY 只挑有用的一塊、簡化並大幅重組；它 SHALL NOT 被要求 cover 整篇 source。
...
- **AND** 不得因為 Lv 比 SP 自由，就扭曲掛在人家名下的 claim
...
- SP = ShroomDog 選的外部好文，忠實翻譯並加 ClawdNote commentary
- CP = Clawd / Mogu 選的 tweet 或短 source，其他 body / note 邊界同 SP
...
#### Scenario: SP identity is cited

- **WHEN** doc、prompt 或 judge 描述 SP
- **THEN** 它 SHALL 把 SP 描述為 ShroomDog-selected external good writing with faithful translation plus ClawdNote commentary
- **AND** SHALL NOT 把 SP 描述成自由改寫

#### Scenario: CP identity is cited

- **WHEN** doc、prompt 或 judge 描述 CP
- **THEN** 它 SHALL 把 CP 描述為 Clawd / Mogu-selected tweets or short-form sources
- **AND** SHALL 保留與 SP 相同的忠實 body 與 commentary-note 邊界
...
在作者優先北極星下，深 MOBA 詞，包含 Vainglory-specific terms，MAY 使用。每個非顯而易見的深詞 SHALL 能 link 到 on-site MOBA glossary。
...
- **WHEN** SP 或 CP body 翻譯 source claim
- **THEN** translated body SHALL 保持 plain
- **AND** SHALL NOT 加入 MOBA-flavored wording、玩笑或外加類比
- **AND** 這類 commentary SHALL 改放進 ClawdNote 或 MoguNote
...
#### Scenario: deep terms require glossary support

- **WHEN** note 或原創說明使用非顯而易見的 MOBA / Vainglory-specific term
...
- 對應的 mac-CC 或 CCC playbook 負責把 editorial work 指到 charter
...
#### Scenario: mac-CC starts editorial work

- **WHEN** mac-CC 開始寫文、修文、改內容規則、writer prompt、judge prompt 或 editorial workflow
- **THEN** mac-CC startup route SHALL 包含閱讀 editorial charter
