...
Raw `style`／`script` element、inline `style` attribute 與 `on*` event handler attribute 採同一個遇錯即停邊界。CSS 與 JavaScript escape 即使 source 沒有 Unicode glyph，仍可能在瀏覽器產生 emoji；validator 不建立半套 CSS／JavaScript evaluator，而是在新增或修改這類 executable markup 時要求移除。教學文章若要展示範例，照常使用 fenced code block，不會被當成執行內容。
