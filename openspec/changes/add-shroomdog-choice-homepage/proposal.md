...
首頁目前沒有一個能清楚表達 ShroomDog 親自讀過、人工挑選與排序的長期策展入口；只做討論用靜態 mock 也無法驗證它和真實首頁內容、雙語資料與主題樣式是否協調。這次直接把策展區做進實際首頁，讓 branch preview 成為產品決策與驗收的共同介面。

## What Changes

- 在繁中與英文首頁最前方加入 `ShroomDog’s Choice` 策展區，以較俐落的圓角矩形、單層內容階層呈現一篇首選與兩篇延伸選文，不使用會和 `ShroomDog Original` ticket 前綴混淆的 `SD` 圓章。
