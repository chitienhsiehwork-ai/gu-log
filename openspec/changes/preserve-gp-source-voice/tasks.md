...
- [ ] 2.5 新增各角色的 config key、dispatcher、profile validation、provider preflight、state、執行報告與可持久追溯資訊
- [ ] 2.6 將原文翻譯階段命名為 `source-translate`，和既有英文 sidecar `translate` 明確區隔；未設定完整 GP profile 的 runtime 禁止發布 GP
...
- [ ] 5.5 驗證 `--from-step`、`--file` 與 deploy 都不能沿用 stale 或缺漏的 gate verdict
