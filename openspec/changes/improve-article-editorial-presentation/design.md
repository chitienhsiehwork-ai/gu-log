...
- ClawdNote / MoguNote weight，讓 commentary 支援正文，而不是變得一樣搶戲。**附帶條款（user 拍板）**：降音量（縮字級）時 line-height SHALL 跟著收緊——user 實測回饋：小字配 1.8 行距太鬆散；收緊後的實際行高值實作時實測定案。
...
4. **標題下輕量「小卡」（✅ 選定方案，user 拍板；原 Open Q2 的 user 反提案，不在原三選項內）**：保留卡片質感（不降級成裸文字），`inline-block` 不滿版、縮 padding、小字級。user 理由：「小卡比小字有誠意」、pure HTML text 看起來像沒寫 CSS。
...
**拍板（user 同意）**：desktop 走細線導航——hairline rail＋active 亮橘標記、去掉卡片底色；mobile 照上述 spec（可發現、不佔首屏）。細線在 Dracula 深色主題的對比度（WCAG AA）列入實作驗證項（見 tasks.md）。
...
- **降音量 ≠ 去卡片化**：縮小、收窄、降 padding 都可以，但不要把卡片降級成裸文字。
- **UI chrome 一律 clean SVG line icons、不用 emoji**（📄🔧 都算 cheap）——對齊現有 nav 的 search/globe/moon 線條 icon 風格；source 小卡與技術資訊抽屜的 icon 需要設計。
