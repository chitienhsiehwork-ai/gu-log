# Proposal: Improve article editorial presentation

## Why

SP-251 的 UI/UX 差距分析把 gu-log 文章頁和 Kevin Ma 的 Fable 文章頁放在一起看，結論不是「少一個漂亮 hero」而是文章頁的 editorial presentation 還太像工具頁：首屏先露出行政資訊與工具模組，正文節奏和標題層級不夠像長文版面，正文結束後又接上一串 dashboard 式功能區。
...
- `.post-content` SHALL 是正文節奏的主錨點；H2、段落、blockquote / ClawdNote、code / prompt examples 要有可掃描的層級。
...
- 不新增或修改 SP-251 文章插圖、`PostImage`、圖片 assets。
...
- None. `zoomable-post-images` 仍只管文章圖片能力；本 change 不碰它。
...
Approving this change means gu-log should treat article pages as editorial reading surfaces first and tool dashboards second. The implementation should keep the existing reader tools, but reorganize their weight, order, and disclosure so they support the article instead of taking over the page.
