...
- **hard-block on FAIL**：dupCheck FAIL 會讓 Stage 3 整體 FAIL，**但不 loop 回 workers**（FactCorrector / Librarian 沒有 dedup 語義，重跑只是白燒 token）。Stage 3 改為標記 `needs_review` + 寫 frontmatter verdict，交 Level F gate 或人工處理。dupCheck 的「正確行為」是 `allow` 時 10 分，不是「擋不擋」。真正的 BLOCK 動作留給 Level F gate。
