...
### Requirement: Judge 比對 corpus 時限縮範圍以控成本
...
Judge MAY 透過 frontmatter 的 `clusterIds` / `seriesId` / `authorCanonical` 欄位先篩候選 corpus subset，然後只讀候選的內文 snippet。

#### Scenario: Corpus 比對限縮在 frontmatter + 首 300 字

- **WHEN** judge 比對稿件 vs. 922 篇 corpus
- **THEN** 判定 pipeline SHALL 先以 frontmatter 欄位（`clusterIds`、`seriesId`、`authorCanonical`、`sourceType`、`temporalType`）做 pre-filter
