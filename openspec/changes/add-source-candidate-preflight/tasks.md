...
- [ ] 1.2 將 YouTube 擷取改成結構化部分結果，缺 `yt-dlp` 時封閉失敗，且 metadata 缺欄位不補推測值
...
- [ ] 2.2 新增 `gp-pipeline candidate <url>`，只編排來源擷取、完整性與既有僅用 URL 的防重複檢查
...
- [ ] 4.2 新增清單原子性、穩定失敗、雜湊、過期產物、防重複 PASS／BLOCK 與逾時／中斷測試
- [ ] 4.3 新增端到端異動測試，驗證候選預審前後 HEAD、索引、計數器與文章 checksum 完全相同，且假的 LLM／git／發布執行檔被呼叫即失敗
...
- [ ] 5.2 同步 `source-candidate-preflight` delta spec、封存 change，確認 OpenSpec ownership／archive 關卡通過
