...
URL parser 先把 watch、shorts 與 `youtu.be` 形式解析成同一 video ID。Dedup gate 的 URL identity normalization 同步加入 YouTube video ID，比對 candidate 與既有文章 `source` 時不依賴原始 URL 字串形式；相同影片跨形式必須 BLOCK。Candidate 不啟動 title similarity，也不讓 LLM 覆寫結果。
...
沿用目前預設的 `$TMPDIR`，但 candidate 額外拒絕 repo root、本 repo 子目錄，以及解析 symlink 後落在 repo 內的 `--work-dir`。這避免「預審只寫 workdir」被誤用成 repo 修改。測試會在 fake repo 中對所有 tracked contents 與 untracked set 建立前後 snapshot，而不只抽查少數路徑；並讓 fake LLM／git／deploy commands 一被呼叫就失敗。
