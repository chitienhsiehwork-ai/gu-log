...
- 提供明確、可重跑且不會發布的 `candidate` 預審路徑。
- 讓 YouTube acquisition fail closed，仍能保留 metadata-only failure evidence。
- 以 manifest 清楚揭露來源完整性、字幕 provenance、hash、限制與 URL dedup 結果。
...
- 不改變 ShroomDog 直接交付 URL 時，既有完整 `gp-pipeline run` 的路由語意。
- 不在這個 change 補齊 generic fetch 的完整 redirect／DNS-rebinding SSRF 防線；candidate 仍重用現有 generic validator，YouTube 則只接受 allowlisted canonical hosts 與可解析的單一 video ID。

## Decisions

### 新增薄 `candidate` command，不重用 `run --dry-run`

`candidate <url>` 只編排 source capture、completeness 判定與 URL-only dedup，然後原子寫入 `candidate-manifest.json`。它不建立 `pipeline.State`，也不 import 或呼叫 Eval、Write、Review、Refine、Credits、Ralph、Translate、Deploy。這個獨立 entrypoint 比在 `run` 增加 `--stop-after` 更容易證明 side-effect boundary，也不會讓日後新增 step 意外越界。
...
### Manifest 是唯一 candidate artifact，capture 是其引用證據

Manifest 使用 versioned JSON schema，至少記錄 canonical URL、source kind、真實 metadata nullable fields、availability、`writeEligible`、字幕 language／kind／coverage、warnings、source／raw VTT 路徑與 SHA-256、dedup verdict／matches，以及 stable failure code／retryable。所有檔案路徑都相對於 work directory，避免搬移後失效。

`candidate-manifest.json` 先寫同目錄暫存檔、`fsync`／close 後 rename；正常成功、metadata-only、缺 dependency、timeout 與分析失敗都盡量留下完整 failure manifest，不留下看似成功的半份 JSON。只有 work directory 本身無法建立時，才允許沒有 manifest。
...
字幕選擇順序固定為 manual captions 優先，再選 automatic captions；每層先依語言偏好，再穩定選第一個可用語言。實際 language 與 manual／auto provenance 必須進 manifest。保留 raw VTT，另產供人閱讀的 timestamped transcript；不得只保留移除時間軸的純文字。Duration、raw bytes 與估算 token 都在任何後續處理前受集中常數限制，超限只留 manifest，不自動 chunk。

`yt-dlp` 缺失時 YouTube path 直接回 `dependency_missing`，不得落入 generic fetch。Doctor 將 `yt-dlp` 列為 optional capability dependency：缺少它不使整體 doctor 失敗，但 JSON／human report 必須明示 YouTube candidate unavailable。

### 預審完全不呼叫 LLM

本輪 manifest 不含摘要或價值判斷；這些欄位若沒有 citation-aware、no-tools provider 就會把不可信 transcript 送入目前的 `danger-full-access`／`bypassPermissions` agent。Candidate 只跑 deterministic acquisition 與 URL dedup，讓安全邊界可以由 import test、fake executable trap 與 repo checksum test 證明。
...
沿用目前預設的 `$TMPDIR`，但 candidate 額外拒絕 repo root、本 repo 子目錄，以及解析 symlink 後落在 repo 內的 `--work-dir`。這避免「預審只寫 workdir」被誤用成 repo 修改。測試會在 fake repo 中記錄 HEAD、index、counter 與 posts tree checksum，並讓 fake LLM／git／deploy commands 一被呼叫就失敗。
...
- [Generic candidate 仍承接現有 fetch 的 SSRF 限制] → 本 change 不擴大 generic fetch 的信任面；YouTube 路徑用 host／video ID allowlist。完整 redirect revalidation 另立安全 change，不在這裡假裝已解決。
...
2. 新增 candidate command、manifest writer、URL dedup adapter與副作用測試。
