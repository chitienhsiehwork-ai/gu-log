...
- 新增明確的 `gp-pipeline candidate <url>` 預審指令，只在 repo 外的 work directory 產生來源 capture 與 machine-readable manifest。
...
- Candidate 預審只做 acquisition、來源完整性檢查與既有內容的 deterministic URL dedup；不得呼叫 LLM、建立 MDX、配置 ticket、修改 counter／Git，或進入 write、review、refine、Ralph、translate、deploy。
- 補上 `yt-dlp` 的 doctor 診斷、hermetic fixtures 與副作用回歸測試。
- 保留目前「user 直接丟 URL 就跑完整 GP pipeline」的 trusted-owner 語意；只有明確執行 `candidate` 才進預審。
...
- `source-candidate-preflight`: 定義零副作用的來源候選預審、YouTube availability/provenance manifest、fail-closed 邊界與 promotion 前的人類檢查點。
...
- `tools/gp-pipeline/internal/dedup/` 或既有 dedup adapter：重用 URL-only dedup，不啟動 editor／LLM path。
