...
## 2. usage-monitor 去 VM 硬相依

- [ ] 2.1 Vendor `~/clawd/scripts/usage-monitor.sh` 到 `scripts/usage-monitor.sh`（從 VM 拉最新版）
- [ ] 2.2 `tribunal-quota-lib.sh` 先找 `$HOME/clawd/scripts/usage-monitor.sh`，找不到 fallback `$SCRIPT_DIR/usage-monitor.sh`
