...
- **human = coach**：定高層方向、拍板 critical design decision（產品方向 / 架構 / 對外承諾 / 品牌調性）。只停在兩個座標——`opsx explore`（釐清 intent）與審 proposal（gate）。
- **main agent（mac-CC / CCC）= controller / orchestrator**：不做實作重活，負責拆 task、給 spec、驗收、當 coach 的對話介面，context 維持乾淨。
- **subagents = workers**：builder（做實作）與 reviewer（審產出），各自獨立 context。

#### Scenario: 角色不越界

- **WHEN** 一個實作任務進入階段 5–6
- **THEN** controller SHALL 把重活外包給 builder、把驗收外包給 reviewer，自己不下海寫 code
...
- 真的無法自動化者落 **Tier-2**：LLM reviewer 照 scenario 文字打分。
...
- **THEN** reviewer SHALL 照 scenario 文字逐條判定並輸出機器可讀 verdict
- **AND** 該判定 SHALL 標記為 Tier-2（主觀），供 coach 終審時辨識

### Requirement: 正確性 reviewer SHALL 以 spec scenario 為 rubric

階段 6 的正確性 reviewer SHALL 把該 change 的 spec scenario 當評分 rubric，逐條對帳。**收斂定義 = 「所有 scenario 對上」**，取代 #481 的「reviewer 滿意」。

#### Scenario: 收斂以列舉而非感覺判定

- **WHEN** reviewer 完成一輪審查
- **THEN** 輸出 SHALL 列出每一條 scenario 的對上 / 未對上狀態
- **AND** 任一 scenario 未對上時 SHALL NOT 判為收斂
...
- **WHEN** builder↔reviewer 迭代達到 max-N 輪仍未讓所有 scenario 對上
- **THEN** 迴圈 SHALL 停止並升 coach
- **AND** 升級內容 SHALL 帶上未對上的 scenario 清單

### Requirement: Escalation SHALL 先 explore 再 propose

升 coach 浮上來的是「問題」不是「答案」。升級 SHALL 先進 `opsx explore` 釐清 intent / 範圍，確認後才 `opsx propose`，不得跳過 explore 直接生完整 change。

#### Scenario: 卡關升級的順序

- **WHEN** worker 層因 design decision 卡關而升 coach
- **THEN** 預設 SHALL 先 `opsx explore`
- **AND** 只有當解法明顯、無 ambiguity 時 MAY 直接 propose（不把 explore 儀式化）
