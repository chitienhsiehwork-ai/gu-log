...
- **不確定時找誰**（Claude subagent 路徑）：技術決策不確定用 `AskUserQuestion`（先把問題想清楚、給選項）；內容 / editorial work 先讀 `openspec/specs/editorial-charter/spec.md`，內容風格再讀 `GU-LOG_WRITER_PROMPT.md`，正式打分依上條；架構不確定 spawn `Plan` subagent 規劃再動手。
