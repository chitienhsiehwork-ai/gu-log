...
- **預設流程**：
  1. 任何「一個 task = 一組相關 commit」的工作 → 在 `main` 拉一條 branch（命名慣例：`<type>/<scope>-<short-desc>` 例如 `fix/ralph-loop-revert`、`rewrite/sp-175`、`chore/scorer-pin-4-6`）
...
- **Ralph-loop / 自動化 pipeline 也走 branch + PR**：`scripts/ralph-loop.sh` 預設 `RALPH_BRANCH_MODE=pr`，每篇 post 開自己的 `ralph/<ticket>-<date>` branch + draft PR。整群 rewrite 裝在不同 PR 裡方便個別 review / revert。開 PR 前會先 `gh pr list --search` 查是否已有同 ticket 的 open PR，防止重跑造成重複 PR。
- **直推 main 的例外（不用 branch）**：
  - Prod 炸了或 main CI broken 的緊急修復（爭取時間）
  - 純設定檔 / doc typo（完全沒 code logic 風險）
  - User 明講「直接推 main」
- **逃生艙**：不想開 PR 的單次任務 → 設 env `RALPH_BRANCH_MODE=main bash scripts/ralph-loop.sh ...`，或在手動流程裡 push main 前確認自己屬於上面三種例外。
