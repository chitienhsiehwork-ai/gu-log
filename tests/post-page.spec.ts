...
test.describe('Post Page', () => {
  ...
  test('GIVEN a post with headings WHEN rendered THEN all h2 headings should have IDs for anchoring', async ({
    page,
  }) => {
    await page.goto(testPostUrl);
  ...
  test('GIVEN a post with special characters in headings WHEN rendered THEN MDX should not break', async ({
    page,
  }) => {
...
test.describe('Post Navigation', () => {
  test('GIVEN home page WHEN clicking a post link THEN should navigate to post', async ({
    page,
  }) => {
    await page.goto('/');
