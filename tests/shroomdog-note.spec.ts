...
test.describe('ShroomDogNote auto-fold', () => {
  ...
  test('GIVEN both themes WHEN rendered THEN persona accents pass contrast and typography stays stable', async ({
    ...
    for (const theme of ['light', 'dark']) {
      ...
      const result = await page.locator('#long-note .shroomdog-note').evaluate((note) => {
        ...
        const prefixStyle = getComputedStyle(note.querySelector<HTMLElement>('.shroomdog-prefix')!);
