...
test.describe('No Tables on Mobile', () => {
  test('GIVEN the SD-2 subagent showdown post WHEN rendered on mobile THEN should not contain any <table> elements', async ({
    page,
  }) => {
  ...
  test('GIVEN the SD-2 EN subagent showdown post WHEN rendered on mobile THEN should not contain any <table> elements', async ({
    page,
  }) => {
    await page.setViewportSize(MOBILE_VIEWPORT);
  ...
  test('GIVEN any post WHEN rendered THEN no table should overflow its container on mobile', async ({
    page,
  }) => {
    await page.setViewportSize(MOBILE_VIEWPORT);
