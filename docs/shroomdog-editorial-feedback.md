...
## 2026-08-20 — MP dimensions session：`downside` 會把最佳化的方向感寫反

### Feedback: `downside` 對台灣讀者不直覺，也和尋找 minimum 的比喻衝突

- ShroomDog feedback：`downside` 對台灣讀者不直覺；更重要的是，在 energy diagram 或 loss function 裡，人本來就在找 minimum，越低越好，但日常英文的 `downside` 卻表示壞處。把兩者放進同一篇文章，會讓「down」一下是好、一下是壞，方向感整個翻轉。
- 情境：這篇 MP 以 high-dimensional optimization 解釋卡住、習得無助與重新找到可走方向。原先想用 `downside` 描述採取其他行動的壞處，卻破壞了全文最重要的幾何 mental model。
- 修法：描述負面後果時改用「風險」或「代價」；描述最佳化進展時明確寫「loss 開始下降」。若需要技術精度，則記得 `gradient` 指向最陡上升方向，最小化是沿著 `-gradient` 前進。
- Reusable lesson：延伸數學比喻時，方向詞的日常語意與技術語意必須同號。若同一個「上／下」在兩套語境裡代表相反價值，就換成不帶方向的詞，保住讀者腦中的圖。

## 2026-08-20 — MP dimensions session：「步子」不是自然的台灣用語

### Feedback: 「步子？wtf」

- 情境：初稿寫成「步子不必大」，想表達先做小幅、可承受的嘗試，但「步子」帶有明顯的中國用語與翻譯腔；台灣讀者通常不會這樣說。
- 修法：改成「一開始不用走很遠」，保留原本的空間與移動意象，同時回到自然台灣口語。全站搜尋當下沒有其他「步子」殘留。
- Reusable lesson：英文 `small steps` 不要直譯成「小步子／步子不用大」。依語境改成「先走一小段」、「一開始不用走很遠」、「先做一個小嘗試」或「慢慢來」。

## 2026-08-20 — MP dimensions session：「不是……而是……」把溫柔的意思寫成 AI 金句

### Feedback: 「不是先相信才開始走」太 AI；不相信也沒關係，可以先挑路走走看

- 情境：初稿用「不是先相信才開始走，而是先走一點點，直到自己有理由重新相信」收尾。概念符合文章，但「不是……而是……」把原本容許不確定的意思，壓成刻意對稱、像在下結論的 AI 金句。
- ShroomDog 想保留的真正意思：如果非得先相信才肯開始讓人走不動，或走起來感覺不好，那麼暫時不相信也沒關係；可以先挑一個方向試走，讓相信之後再跟上。
- 修法：改成「就算還不相信也沒關係，先挑一個方向走走看。等感覺到那些微小的變化，相信也許就會慢慢跟上來。」
- Reusable lesson：溫柔的鼓勵不要寫成反義對偶。當重點是允許猶豫、降低開始的門檻，就直接給讀者空間，不要先否定一種做法，再宣布另一種正解。

## 2026-08-20 — MP dimensions session：信心不是值得追求的終點，也不能代替 feedback

### Feedback: 有沒有信心、走起來爽不爽、事情有沒有做好，是不同的事

- ShroomDog feedback：就算一直不相信也沒差。有沒有信心、走起來心情好不好、事情有沒有做好，是彼此不同的變數；信心甚至可能只是像信仰或星座一樣沒根據的盲目相信，不必把它當成特別值得追求的目標或心態。
- 情境：初稿原本用「相信也許就會慢慢跟上來」收尾，仍偷偷把「重新獲得信心」當成小步行動最後應該換得的心理獎賞。這把文章拉回正向心理學，也模糊了最佳化比喻真正能提供的判準。
- 修法：刪掉「相信會跟上來」。結尾改為允許沒有信心也先試走，並把判斷交回可觀察的結果：「這個方向有沒有用，看 loss 有沒有真的下降就好。」
- Reusable lesson：談習得無助時，值得恢復的是「行動可能改變結果」的可感知 feedback，不是信心本身。不要把相信、舒服或有希望寫成行動的必要條件，也不要把它們當成成功的 proxy；能觀察到的結果才是判斷方向是否有用的依據。

## 2026-08-20 — MP dimensions session：「鬆一點」是沒有資訊的療癒腔

### Feedback: 「看看某件事有沒有鬆一點」不如直接寫有沒有變化

- ShroomDog feedback：「鬆一點」沒有說清楚究竟什麼改變，只憑空添加模糊的療癒感；這裡要觀察的本來就只是結果有沒有變化。
- 修法：把「看看某件事有沒有鬆一點，原本坍塌的選項有沒有重新出現」收斂成「看看結果有沒有變化」。
- Reusable lesson：不要用「鬆一點、接住、安放、長出空間」之類柔軟但無法指認的詞替代實際 feedback。原意若只是 change，就直接寫「有沒有變化」；清楚本身就可以溫柔。
