...
> 這份是「agent 怎麼做事」的跨領域行為規則全文。`AGENTS.md` 只留一條指標；完整 nuance、例外、反例都在本檔。
>
...
這條把既有的「改規則時只改 SSOT 來源檔，不要在 task prompt 裡重複定義」（見 `AGENTS.md`）從「規則文件」推廣到**所有事實**（值、設定、計數、路徑），並加上「權威端 = code/openspec」「主動偵測 + 自主收斂 + 提一聲」的義務。
