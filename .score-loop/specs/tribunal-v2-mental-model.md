...
Stage 1: Vibe（定骨架）
  ...
  MoguNote 完全免 fact-check（creative scope）
...
| FreshEyes     | 用詞、段落拆分、過渡句、術語解釋     | 骨架、段落順序、MoguNote 觀點         |
...
### 5. Component Scope（MoguNote / ShroomDogNote 分開處理）
...
2. **Contract by component** — MoguNote / ShroomDogNote 各自 opt-in/out 於特定檢查
...
- **維度**: persona, moguNote, vibe, clarity, narrative（5 維）
...
- **Writer constraints**: 只能改用詞、段落拆分、過渡句、術語解釋。不能改骨架、段落順序、MoguNote 觀點。
- **Max loops**: 2
...
- 主動掃文章 body + ShroomDogNote（**排除 MoguNote**）
...
- **維度**: 同 Stage 1（persona, moguNote, vibe, clarity, narrative）
