...
4. **先轉 ready，暫不掛 auto-merge，並排定喚醒**：非 OpenSpec PR 若 harness 強制開 draft，立刻 `mcp__github__update_pull_request` 轉 ready，讓 GitHub Codex auto-review 啟動。採用 OpenSpec 的變更依 `.agents/openspec-sdlc.md` 維持 draft，完成 preview touchpoint 與 archive 後才轉 ready。此時不得先掛 auto-merge；同一輪就用 `send_later` 或 runtime 等價的 durable wake-up 排定 15 分鐘後重查，不能假設 session 會一直醒著。
5. **讀完 auto-review 才把 merge 交給 server-side**：等 review 完成後，fresh-read 最新 review threads，逐條判斷是否成立；actionable 就修、push，然後重新走這道 gate，誤報則留下理由。沒有 unresolved actionable thread 後才 `mcp__github__enable_pr_auto_merge`。15 分鐘喚醒時仍沒有 review 就記錄第一次查詢，立刻再排 10 分鐘後的 durable wake-up；25 分鐘仍無結果，記錄 ready 時間與兩次查詢結果，視為 auto-review 本次不可用並繼續，不能無限等。Gate 通過或有不可用證據後要立刻掛 auto-merge，讓 CI 綠時 GitHub 自己合，**不依賴 session 醒著**。
...
**白話**：safe 就把 preview 當收據交付，照常 merge；不因「想讓你看一眼」停住。只有「這個 taste call 我真的不確定該不該自己拍」才值得 preview + 問。判斷不出來時，預設往「提供 preview 並繼續 merge」靠，不要往「等待批准」靠。
