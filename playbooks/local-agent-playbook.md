...
安全與使用者授權邊界見 [`AGENTS.md`〈共通硬邊界〉](../AGENTS.md#共通硬邊界)；分支、PR、CI、合併與失敗處理直接照 [`CCC-playbook`〈自動合併流程〉](CCC-playbook.md#self-merge-policy)、[〈合併方式〉](CCC-playbook.md#merge-method-選擇)與[〈失敗處理〉](CCC-playbook.md#失敗處理)。本機 agent 同樣沒有直推 `main` 的例外：
