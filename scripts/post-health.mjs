...
const posts = files.map(file => {
  ...
  const wordCount = body.replace(/<[^>]+>/g, '').replace(/import.*\n/g, '').trim().split(/\s+/).length;
...
if (RETIRE_ONLY) {
  ...
  retireCandidates.forEach(p => {
    const s = p.score;
    console.log(`  ${p.ticketId.padEnd(8)} ${s.persona}/${s.clawdNote}/${s.vibe}  ${p.title.slice(0, 60)}`);
  });
  process.exit(0);
}
...
const scored9 = posts.filter(p => p.score && p.score.persona >= 9 && p.score.clawdNote >= 9 && p.score.vibe >= 9);
const scored8 = posts.filter(p => p.score && p.score.persona >= 8 && p.score.clawdNote >= 8 && p.score.vibe >= 8 && !(p.score.persona >= 9 && p.score.clawdNote >= 9 && p.score.vibe >= 9));
const below8 = posts.filter(p => p.score && (p.score.persona < 8 || p.score.clawdNote < 8 || p.score.vibe < 8));
